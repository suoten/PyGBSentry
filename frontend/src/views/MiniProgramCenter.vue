<template>
  <div class="app-page space-y-4" v-loading="loading">
    <PageContainer>
      <template #header>
        <PageHeader :title="t('miniProgram.title')" :description="t('miniProgram.description')" />
      </template>
      <TableCard>
        <template #header><div class="font-medium">{{ t('miniProgram.overview') }}</div></template>
        <div class="space-y-3" style="color: var(--el-text-color-regular)">
        <div class="flex flex-wrap gap-3 items-center">
          <el-tag type="success">{{ t('miniProgram.latestVersion') }}：{{ version.latest_version || '-' }}</el-tag>
          <el-tag :type="version.force_update ? 'danger' : 'info'">{{ t('miniProgram.forceUpdate') }}：{{ version.force_update ? t('common.yes') : t('common.no') }}</el-tag>
          <el-tag type="warning">{{ t('miniProgram.rolloutRatio') }}：{{ version.rollout_ratio ?? 100 }}%</el-tag>
        </div>
        <div class="flex gap-2">
          <el-button v-if="version.miniprogram_url" type="primary" @click="openUrl(version.miniprogram_url)">{{ t('miniProgram.openLink') }}</el-button>
        ...
        <div class="text-sm">
          <div>{{ t('miniProgram.logs24h') }}：{{ stats.total }}</div>
          <div>{{ t('miniProgram.crashes24h') }}：{{ stats.crash_total }}</div>
        </div>
        <el-alert type="info" :closable="false" show-icon>
          <template #title>{{ t('miniProgram.remoteConfig') }}</template>
