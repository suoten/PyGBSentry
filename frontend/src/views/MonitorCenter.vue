  ...
  <div class="app-page h-full flex flex-col">
          ...
          <template #actions>
            ...
            <div class="flex gap-2">
              ...
              <el-button size="small" @click="showSidebar = !showSidebar" plain>
                ...
                {{ showSidebar ? t('monitor.hideTree') : t('monitor.showTree') }}

              </el-button>

              <el-radio-group v-model="layout" size="small">

                <el-radio-button value="1">{{ t('monitor.singleScreen') }}</el-radio-button>
              ...
              <el-button v-if="hasPlayingScreens" type="danger" size="small" @click="stopAll">{{ t('monitor.stopAll') }}</el-button>
      ...
      >
        ...
        <div class="p-4 border-b" style="border-color: var(--el-border-color-lighter); background: var(--el-fill-color-extra-light);">
          ...
          <el-radio-group v-model="treeMode" size="small" @change="fetchTree" class="mb-3">
            ...
            <el-radio-button value="business">{{ t('monitor.businessGroup') }}</el-radio-button>

            <el-radio-button value="region">{{ t('monitor.adminRegion') }}</el-radio-button>

          </el-radio-group>

            <el-input v-model="filterText" :placeholder="t('monitor.searchDevice')" size="small" clearable class="search-input" />
        ...
        <div class="flex-1 overflow-auto p-3 min-h-0" style="background: var(--el-fill-color-extra-light);">
          ...
          <div v-else-if="!treeData.length" class="py-12 text-center">
            ...
            <div class="text-sm text-slate-500">{{ t('monitor.noDevice') }}</div>

            <div class="text-xs text-slate-400 mt-1">{{ t('monitor.addDeviceHint') }}</div>
        ...
        >
          ...
          <div v-if="(screens[index-1]?.url || screens[index-1]?.hls) && !screens[index-1]?.error" class="w-full h-full relative" style="min-height: 100px;">
             ...
             <div class="absolute top-3 right-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
               ...
               <el-button size="small" type="primary" circle @click.stop="takeScreenshot(index-1)" :title="t('monitor.screenshot')">

                 <el-icon><Camera /></el-icon>

               </el-button>

               <el-button size="small" type="danger" circle @click.stop="stopScreen(index-1)" :title="t('monitor.closeChannel')">
               ...
               <div class="flex items-center justify-between text-xs text-white/80">
                 ...
                 <div class="flex items-center gap-3">
                  ...
                  <span>{{ currentTime }}</span>
                   ...
                   <button @click.stop="stopScreen(index-1)" class="hover:text-red-400 transition-colors flex items-center justify-center" :title="t('monitor.stopPlay')">
          ...
          <div v-else-if="screens[index-1]?.loading" class="w-full h-full flex flex-col items-center justify-center bg-black/40">
            ...
            <div class="text-white/80 text-sm font-medium animate-pulse">{{ t('monitor.requestingStream') }}</div>
          ...
          <div v-else-if="screens[index-1]?.error" class="w-full h-full flex flex-col items-center justify-center bg-black/40 relative group">
            ...
            <div class="text-red-400 text-sm font-medium max-w-[80%] text-center truncate" :title="screens[index-1].errorMsg">{{ screens[index-1].errorMsg || t('monitor.playFailed') }}</div>
          ...
          >
            ...
            <span class="text-white font-semibold text-lg">{{ t('monitor.dragToStartPlay') }}</span>
        ...
        <template v-if="layout === '1+3'">
          ...
          >
            ...
            <template v-if="(screens[cell.index]?.url || screens[cell.index]?.hls) && !screens[cell.index]?.error">
              ...
              <div class="w-full h-full relative" style="min-height: 100px;">
                ...
                <div class="absolute top-3 right-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
                  ...
                  <el-button size="small" type="primary" circle @click.stop="takeScreenshot(cell.index)" :title="t('monitor.screenshot')">

                    <el-icon><Camera /></el-icon>

                  </el-button>

                  <el-button size="small" type="danger" circle @click.stop="stopScreen(cell.index)" :title="t('monitor.closeChannel')">
            ...
            <div v-else-if="screens[cell.index]?.error" class="w-full h-full flex flex-col items-center justify-center bg-black/40 relative group">
              ...
              <div class="text-red-400 text-sm font-medium max-w-[80%] text-center truncate" :title="screens[cell.index].errorMsg">{{ screens[cell.index].errorMsg || t('monitor.playFailed') }}</div>
            ...
            >
              ...
              <span class="text-white font-semibold">{{ t('monitor.dragToStartPlay') }}</span>
    ...
    >
      ...
      <div
        ...
        <span>{{ t('monitor.deviceControl') }}：{{ activeScreenData.name }}</span>
      ...
      <div class="p-2 max-h-[calc(70vh-44px)] overflow-y-auto">
        ...
        <el-tabs>
          ...
          <el-tab-pane :label="t('monitor.ptzControl')" name="ptz">
          ...
          <el-tab-pane :label="t('monitor.talkback')" name="talk">
...
const { t } = useI18n()  // FIXED-P3: W-04 添加i18n支持
...
// FIXED-P2: W-02 — 将实时时钟从模板内联表达式移至响应式数据 + setInterval，避免每秒重渲染时重复调用 toLocaleTimeString
...
const getNodeStatusText = (node: TreeNode) => {
  ...
  return Number(node?.status) === 1 ? t('monitor.online') : t('monitor.offline')
...
const fetchTree = async () => {
  ...
  try {
    ...
    const msg = getApiErrorMessage(e, t('monitor.loadTreeFailed'))

    ElMessage.error(typeof msg === 'string' ? msg : t('monitor.loadTreeFailed'))
...
const handleDoubleClick = async (data: Record<string, unknown>) => {
  ...
  if (existingIndex !== -1) {
    ...
    ElMessage.warning(t('monitor.channelAlreadyPlaying', { index: existingIndex + 1 }))
...
const handleDrop = async (ev: DragEvent, index: number) => {
  ...
  try {
    ...
    ElMessage.warning(t('monitor.invalidDragData'))
