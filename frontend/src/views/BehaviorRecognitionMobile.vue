  ...
  <div class="min-h-screen p-3 space-y-3">
    ...
    <el-card shadow="never">
      <div class="flex justify-between items-center gap-2">
        <div>
          <div class="font-semibold" style="color: var(--el-text-color-primary)">{{ t('behaviorMobile.title') }}</div>
          <div class="text-xs mt-1" style="color: var(--el-text-color-secondary)">
            {{ t('behaviorMobile.currentStatus') }}
    ...
    <el-card shadow="never">
      <div class="flex justify-between items-center mb-2">
        <div class="font-semibold text-sm" style="color: var(--el-text-color-primary)">{{ t('behaviorMobile.callbackConfig') }}</div>
      ...
      <div class="text-xs space-y-1" style="color: var(--el-text-color-secondary)">
        <div>{{ t('behaviorMobile.callbackUrl') }}：{{ config.ai_callback_url || t('behaviorMobile.notConfigured') }}</div>
        <div>{{ t('behaviorMobile.pushSnapshotUrl') }}：{{ config.send_snapshot_url ? t('common.yes') : t('common.no') }}</div>
        <div>{{ t('behaviorMobile.syncNotifyUrl') }}：{{ formatSyncUrls(config.sync_urls) || t('behaviorMobile.notConfigured') }}</div>
        <div>{{ t('behaviorMobile.requestTimeout') }}：{{ config.timeout_seconds || 10 }} {{ t('behaviorMobile.secondsUnit') }}</div>
      </div>
    </el-card>

    <el-card shadow="never">
      <div class="flex justify-between items-center mb-2">
        <div class="font-semibold text-sm" style="color: var(--el-text-color-primary)">{{ t('behaviorMobile.recentPushRecords') }}</div>
      ...
      <div v-else-if="events.length === 0" class="text-xs py-4 text-center" style="color: var(--el-text-color-secondary)">{{ t('behaviorMobile.noRecords') }}</div>
    ...
    <AppDialog v-model="configDialogVisible" :title="t('behaviorMobile.configTitle')" size="medium">
      <el-form :model="configForm" label-width="90px">
        <el-form-item :label="t('behaviorMobile.callbackUrl')">
          <el-input v-model="configForm.ai_callback_url" :placeholder="t('behaviorMobile.callbackUrlPlaceholder')" clearable />
          <div class="text-[11px] mt-1" style="color: var(--el-text-color-secondary)">{{ t('behaviorMobile.callbackUrlTip') }}</div>
        </el-form-item>
        <el-form-item :label="t('behaviorMobile.syncNotify')">
          <el-input
            v-model="configForm.sync_urls"
            type="textarea"
            :rows="2"
            :placeholder="t('behaviorMobile.syncNotifyPlaceholder')"
          />
          <div class="text-[11px] mt-1" style="color: var(--el-text-color-secondary)">{{ t('behaviorMobile.syncNotifyTip') }}</div>
        </el-form-item>
        <el-form-item :label="t('behaviorMobile.pushSnapshot')">
          <el-switch v-model="configForm.send_snapshot_url" />
        </el-form-item>
        <el-form-item :label="t('behaviorMobile.requestTimeout')">
...
const statusText = computed(() => (enabled.value ? t('behaviorMobile.enabled') : t('behaviorMobile.notEnabled')))
...
async function loadConfig() {
  ...
  try {
    ...
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('behaviorMobile.loadConfigFailed'))
  } finally {
...
async function saveConfig() {
  if (enabled.value && !(configForm.value.ai_callback_url || '').trim()) {
    ElMessage.warning(t('behaviorMobile.callbackUrlRequired'))
    return
  }
  saving.value = true
  try {
    ...
    const friendly = getFriendlyError(e)
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('behaviorMobile.saveFailedRetry'))
  } finally {
    saving.value = false
  }
}
...
async function onToggleEnabled() {
  try {
    ...
    ElMessage.success(enabled.value ? t('behaviorMobile.enabled') : t('behaviorMobile.disabled'))
  } catch (e: unknown) {
    const friendly = getFriendlyError(e)
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('behaviorMobile.toggleFailed'))
