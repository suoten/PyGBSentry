  ...
  <div class="min-h-screen p-3 space-y-3">
    ...
    <el-card shadow="never">
      <div class="flex justify-between items-center gap-2">
        <div>
          <div class="font-semibold" style="color: var(--el-text-color-primary)">车牌识别</div>
    ...
    <AppDialog v-model="configDialogVisible" title="车牌识别配置" size="medium">
      <el-form :model="configForm" label-width="90px">
        <el-form-item label="回调地址">
          <el-input v-model="configForm.ai_callback_url" placeholder="http(s)://车牌识别分析服务API地址" clearable />
...
async function onToggleEnabled() {
  try {
    ...
    const friendly = getFriendlyError(e)
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || '车牌识别插件启用/停用配置失败，请检查插件服务连接')
