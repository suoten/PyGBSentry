<template>
  <TableCard class="devices-card">
    <template #header>
      <TableToolbar title="设备列表" :subtitle="deviceTableSubtitle">
        <template #actions>
          <div class="device-toolbar-actions">
            <el-button v-if="selectedDevices.length > 0" type="danger" plain size="small" @click="handleBatchDelete">
              <el-icon class="mr-1"><Delete /></el-icon>批量删除（{{ selectedDevices.length }}）
            </el-button>
            <span class="toolbar-pill"><el-icon><Document /></el-icon>共 <strong>{{ totalDisplay }}</strong> 条</span>
            <span v-if="currentDevice?.gb_id" class="toolbar-pill toolbar-pill--active"><el-icon><Monitor /></el-icon>当前设备：<strong>{{ currentDevice?.name || currentDevice?.gb_id }}</strong></span>
            <el-popover placement="bottom-end" :width="250" trigger="click">
              <template #reference><el-button size="small" plain><el-icon class="mr-1"><Setting /></el-icon>字段显示</el-button></template>
              <div class="text-xs mb-2" style="color: var(--el-text-color-secondary)">默认显示核心字段，按需勾选更多字段</div>
              ...
              <div class="mt-3 flex justify-end"><el-button size="small" text @click="resetColumns">恢复默认</el-button></div>
            </el-popover>
          </div>
        </template>
      </TableToolbar>
    ...
    <el-table v-else :data="devices" style="width: 100%" v-loading="loading" :empty-text="devicesEmptyText" class="devices-table" row-key="gb_id" size="small" header-row-class-name="devices-table-header-row" :row-class-name="getRowClassName" fit @row-dblclick="(row: Record<string, unknown>) => emit('viewDevice', row)" @selection-change="(val: Record<string, unknown>[]) => emit('selectionChange', val)">
      <template #empty>
        <EmptyStateWithAction description="暂无设备。请先确认设备已注册到本平台，也可添加多协议接入源。">
          <template #action>
            <el-button type="primary" @click="emit('openAddDialog')" class="empty-action-btn"><el-icon class="mr-1"><Plus /></el-icon>添加设备</el-button>
            <el-button @click="emit('openAccessInfoDialog')" class="empty-action-btn-secondary"><el-icon class="mr-1"><InfoFilled /></el-icon>接入信息</el-button>
          </template>
        </EmptyStateWithAction>
      </template>
      <el-table-column type="selection" width="50" align="center" />
      <el-table-column v-if="isColVisible('gb_id')" prop="gb_id" width="180">
        <template #header><div class="flex items-center gap-1"><span>国标ID</span><el-tooltip content="国标设备 ID（20 位）" placement="top"><el-icon class="cursor-help text-slate-400 hover:text-slate-600 transition-colors"><InfoFilled /></el-icon></el-tooltip></div></template>
        <template #default="scope"><span class="font-mono text-xs text-slate-500">{{ scope.row.gb_id }}</span></template>
      </el-table-column>
      <el-table-column v-if="isColVisible('name')" prop="name" label="名称" min-width="140" show-overflow-tooltip>
        <template #default="scope">
          <div class="device-name-block">
            ...
            <div class="device-name-meta">
              <span class="device-meta-chip">{{ scope.row.manufacturer || '未知厂家' }}</span>
              <span class="device-meta-chip">{{ scope.row.channel_count ?? 0 }} 路通道</span>
              <span v-if="currentDevice?.gb_id === scope.row.gb_id && dialogVisible" class="device-meta-chip device-meta-chip--active">工作区已打开</span>
            </div>
          </div>
        </template>
      </el-table-column>
      <el-table-column v-if="isColVisible('organization')" label="所属组织" width="160">
        <template #default="scope">
          <div class="org-field"><span class="field-label">归属</span>
            <el-select v-model="scope.row.organization_id" placeholder="未分配" clearable size="small" class="org-select-cell" :loading="savingOrg[scope.row.gb_id]" @change="() => emit('saveOrganization', scope.row)">
              <el-option label="未分配" value="" /><el-option v-for="opt in organizationOptions" :key="opt.id" :label="opt.label" :value="opt.id" />
            </el-select>
          </div>
        </template>
      </el-table-column>
      <el-table-column v-if="isColVisible('ip_addr')" prop="ip_addr" label="IP" width="130">
      ...
      <el-table-column v-if="isColVisible('channel_count')" label="通道数" width="100" align="center">
        <template #default="scope"><span class="text-xs font-medium text-slate-600">{{ scope.row.channel_count ?? 0 }}</span></template>
      </el-table-column>
      <el-table-column v-if="isColVisible('manufacturer')" prop="manufacturer" label="厂家" width="100" show-overflow-tooltip />
      <el-table-column v-if="isColVisible('last_active')" label="最近活跃" width="140">
        <template #default="scope">
          <div class="text-[10px] flex flex-col gap-0.5 leading-tight">
            <div class="flex items-center gap-1 text-slate-500"><span class="text-slate-400 scale-90">心跳:</span><span>{{ fmtDateTime(scope.row.last_keepalive) }}</span></div>
            <div class="flex items-center gap-1 text-slate-400"><span class="text-slate-400 scale-90">注册:</span><span>{{ fmtDateTime(scope.row.register_time) }}</span></div>
          </div>
        </template>
      </el-table-column>
      <el-table-column v-if="isColVisible('stream_mode')" label="流模式" width="130">
        <template #default="scope">
          <el-tooltip :content="savingMode[scope.row.gb_id] ? '保存中，请稍候…' : '设置流模式（全局/自动/UDP/TCP等）'" placement="top">
            <span><div class="mode-field"><span class="field-label">模式</span>
      ...
      <el-table-column v-if="isColVisible('status')" label="状态" width="100" align="center">
        <template #default="scope">
          <div class="status-cell">
            <el-tooltip :content="isOnline(scope.row.status) ? '在线' : '离线'" placement="top">
              <span class="status-pill" :class="isOnline(scope.row.status) ? 'online' : 'offline'">
                ...
                <span class="status-text">{{ isOnline(scope.row.status) ? '在线' : '离线' }}</span>
              </span>
            </el-tooltip>
          </div>
        </template>
      </el-table-column>
      <el-table-column v-if="isColVisible('subscription')" label="订阅" width="180" align="center">
        <template #default="scope">
          <div class="subscription-card">
            <div class="subscription-row">
              <div class="subscription-meta"><span class="subscription-label">目录订阅</span><span class="subscription-value">{{ fmtCycle(scope.row.catalog_subscribe_cycle_seconds, '未设置周期') }}</span></div>
              <div class="subscription-actions">
                <el-tooltip :content="savingCatalogSub[scope.row.gb_id] ? '保存中，请稍候…' : '目录订阅开关'" placement="top"><span>
                  <el-switch v-model="scope.row.catalog_subscribe_enabled" size="small" inline-prompt active-text="开" inactive-text="关" class="pretty-switch pretty-switch--success" :loading="savingCatalogSub[scope.row.gb_id]" @change="(val: boolean) => emit('toggleCatalogSubscribe', scope.row, val)" />
                </span></el-tooltip>
                <el-tooltip :content="scope.row.catalog_subscribe_cycle_seconds ? '点击修改目录订阅周期（秒）' : '点击设置目录订阅周期（秒）'" placement="top">
                  <button class="subscription-link" @click="emit('setCatalogSubscribeCycle', scope.row)">设置周期</button>
                </el-tooltip>
              </div>
            </div>
            <div class="subscription-row">
              <div class="subscription-meta"><span class="subscription-label">位置订阅</span><span class="subscription-value">{{ fmtCycle(scope.row.mobile_position_interval_seconds, '未设置间隔') }}</span></div>
              <div class="subscription-actions">
                <el-tooltip :content="savingMobileSub[scope.row.gb_id] ? '保存中，请稍候…' : '位置订阅开关'" placement="top"><span>
                  <el-switch v-model="scope.row.mobile_position_subscribe_enabled" size="small" inline-prompt active-text="开" inactive-text="关" class="pretty-switch" :loading="savingMobileSub[scope.row.gb_id]" @change="(val: boolean) => emit('toggleMobileSubscribe', scope.row, val)" />
                </span></el-tooltip>
                <el-tooltip :content="scope.row.mobile_position_interval_seconds ? '点击修改位置上报间隔（秒）' : '点击设置位置上报间隔（秒）'" placement="top">
                  <button class="subscription-link" @click="emit('setMobileSubscribeInterval', scope.row)">设置间隔</button>
                </el-tooltip>
              </div>
            </div>
          </div>
      ...
      <el-table-column label="操作" align="center" width="340" fixed="right">
        <template #default="scope">
          <div class="device-actions table-action-inline">
            <el-tooltip :content="Number(scope.row.status) !== 1 ? '设备离线，无法刷新目录' : '刷新目录'" placement="top"><span>
              <el-button size="small" plain @click="emit('syncDeviceChannels', scope.row)" :loading="catalogSyncPolling[String(scope.row.gb_id || '')]" :disabled="Number(scope.row.status) !== 1 || catalogSyncPolling[String(scope.row.gb_id || '')]" class="device-action-btn device-action-btn--refresh"><el-icon class="mr-1"><Refresh /></el-icon>刷新</el-button>
            </span></el-tooltip>
            <el-divider direction="vertical" />
            <el-tooltip content="查看通道" placement="top"><span>
              <el-button size="small" plain @click="emit('viewDevice', scope.row)" class="device-action-btn device-action-btn--view"><el-icon class="mr-1"><View /></el-icon>通道</el-button>
            </span></el-tooltip>
            <el-dropdown trigger="click" @command="(cmd: string) => handleDropdownCommand(scope.row, cmd)">
              <el-button size="small" plain class="device-action-btn device-action-btn--more table-action-more">更多<el-icon class="ml-1"><MoreFilled /></el-icon></el-button>
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="edit"><el-icon><Edit /></el-icon>编辑</el-dropdown-item>
                  <el-dropdown-item command="on"><el-icon class="text-emerald-500"><Bell /></el-icon>布防</el-dropdown-item>
                  <el-dropdown-item command="off"><el-icon class="text-amber-500"><MuteNotification /></el-icon>撤防</el-dropdown-item>
                  <el-dropdown-item command="map" divided><el-icon><Location /></el-icon>设备定位</el-dropdown-item>
                  <el-dropdown-item command="syncBasic"><el-icon><Tools /></el-icon>基础配置同步</el-dropdown-item>
                  <el-dropdown-item command="teleboot" divided><el-icon><SwitchButton /></el-icon>远程重启</el-dropdown-item>
                  <el-dropdown-item command="recordStart"><el-icon><VideoCamera /></el-icon>开始录像</el-dropdown-item>
                  <el-dropdown-item command="recordStop"><el-icon><VideoPause /></el-icon>停止录像</el-dropdown-item>
                  <el-dropdown-item command="alarmReset"><el-icon><BellFilled /></el-icon>告警复位</el-dropdown-item>
                  <el-dropdown-item command="iframe"><el-icon><Film /></el-icon>强制关键帧</el-dropdown-item>
                  <el-dropdown-item command="blacklist" divided class="text-warning"><el-icon><Warning /></el-icon>拉黑 IP</el-dropdown-item>
                  <el-dropdown-item command="delete" divided class="text-danger"><el-icon><Delete /></el-icon>删除设备</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
    ...
    <div class="flex justify-end mt-4 pagination-wrapper">
      <el-pagination v-model:current-page="page" v-model:page-size="pageSize" :total="totalDisplay" layout="total, sizes, prev, pager, next, jumper" :page-sizes="[10, 20, 50, 100]" prev-text="上一页" next-text="下一页" size="small" @current-change="emit('fetchDevices')" @size-change="emit('pageSizeChange')" />
    </div>
  </TableCard>
</template>
...
const columnOptions = [
  { key: 'gb_id', label: '国标 ID' }, { key: 'name', label: '名称' }, { key: 'status', label: '状态' },
  { key: 'channel_count', label: '通道数' }, { key: 'last_active', label: '最近活跃' },
  { key: 'organization', label: '所属组织' }, { key: 'ip_addr', label: 'IP' },
  { key: 'manufacturer', label: '厂家' }, { key: 'stream_mode', label: '流模式' }, { key: 'subscription', label: '订阅' }
] as const
...
const initColumns = () => {
  try {
    ...
    const allowed = new Set<string>(columnOptions.map(i => String(i.key)))
...
const streamModeOpts = [{ label: '全局', value: 'GLOBAL' }, { label: '自适应', value: 'AUTO' }, { label: 'UDP', value: 'UDP' }, { label: 'TCP 被动', value: 'TCP_PASSIVE' }, { label: 'TCP 主动', value: 'TCP_ACTIVE' }]
const deviceTableSubtitle = computed(() => `共 ${props.totalDisplay} 台设备`)
...
const fmtCycle = (s: Record<string, unknown>, empty = '未启用') => { const n = Number(s || 0); if (!Number.isFinite(n) || n <= 0) return empty; if (n < 60) return `${Math.round(n)}秒`; const m = n / 60; if (m < 60) return `${Math.round(m)}分钟`; return `${Math.round(m / 60)}小时` }

async function handleBatchDelete() {
  try {
    await confirmDangerous('批量删除', `${props.selectedDevices.length}台设备`)
  } catch { return }
  emit('batchDelete')
}
...
async function handleDropdownCommand(row: Record<string, unknown>, cmd: string) {
  if (cmd === 'delete') {
    try {
      await confirmDangerous('删除设备', String(row.name || row.gb_id || ''))
    } catch { return }
  }
