<template>
  <div class="app-page">
    <PageContainer flex>
      <template #header>
        <PageHeader :title="t('record.deviceTitle')" description="按设备与通道查询录像，支持回放与下载" />
      </template>
      ...
      <div class="mt-3 rounded-xl border border-slate-200 bg-white p-4 flex flex-wrap items-end gap-3">
        <div class="w-[520px] max-w-full">
          <div class="text-xs text-slate-500 mb-1">设备</div>
          <el-popover
        ...
        <div class="w-80 max-w-full">
          <div class="text-xs text-slate-500 mb-1">通道</div>
          <el-select
        ...
        <el-button @click="reloadDevices" :loading="loadingDevices">刷新设备</el-button>
      </div>

      <div class="mt-3">
        <el-empty v-if="!selectedDeviceId || !selectedChannelId" description="请选择设备与通道后查询录像" />
        <DeviceRecordList
          v-else
...
const buildDeviceTree = (devices: Array<{ value: string; label: string; name: string; deviceId: string; organizationId: string }>, orgs: Record<string, unknown>[]) => {
  ...
  const createOrgNode = (org: Record<string, unknown>): Record<string, unknown> => {
    ...
    const label = String(org?.name || id || '未命名组织').trim()
...
const reloadDevices = async () => {
  ...
  try {
    ...
    ElMessage.error(getApiErrorMessage(e, '设备列表加载失败'))
    deviceOptions.value = []
...
const loadChannels = async (deviceId: string) => {
  ...
  try {
    ...
    ElMessage.error(getApiErrorMessage(e, '通道列表加载失败'))
    channelOptions.value = []
    selectedChannelId.value = ''
  }
}
