<template>
  <div class="w-80 flex flex-col bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm transition-shadow duration-200 hover:shadow-md">
    ...
    <div class="flex-1 flex flex-col min-h-0">
      <div class="p-2 border-b border-slate-200 bg-white/90">
        ...
        <el-input v-model="treeSearchKeyword" placeholder="搜索树节点..." size="small" clearable>
  ...
  >
    <template v-if="getContextMenuTargetType(contextMenu.node) === 'channel'">
      ...
      <div class="menu-item" @click="emit('openRecordTab', contextMenu.node, 'cloud')"><el-icon class="menu-item-icon"><VideoCamera /></el-icon>云端录像</div>
      <div class="menu-item" @click="emit('openRecordTab', contextMenu.node, 'device')"><el-icon class="menu-item-icon"><Files /></el-icon>设备录像</div>
      <div class="menu-item" @click="emit('openRecordTab', contextMenu.node, 'timeline')"><el-icon class="menu-item-icon"><Timer /></el-icon>时间轴</div>
    </template>
    <template v-else>
      <div v-if="contextMenu.node?.nodeType === 'device'" class="menu-item" @click="emit('syncCatalog', contextMenu.node)"><el-icon class="menu-item-icon"><Setting /></el-icon>更新通道</div>
      <div class="menu-item" @click="emit('contextCreateChild')"><el-icon class="menu-item-icon"><Plus /></el-icon>新增子节点</div>
      <div class="menu-item" @click="emit('contextRenameNode')"><el-icon class="menu-item-icon"><Edit /></el-icon>重命名</div>
      <div class="menu-item danger" @click="handleContextDeleteNode"><el-icon class="menu-item-icon"><Delete /></el-icon>删除节点</div>
    </template>
  </div>
</template>
...
async function handleDeleteDirectory() {
  try {
    await confirmDangerous('删除目录', String(props.selectedNode?.label || ''))
  } catch { return }
