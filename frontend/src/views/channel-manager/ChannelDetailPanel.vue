  ...
  <div class="flex-1 flex flex-col bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm transition-shadow duration-200 hover:shadow-md">
    ...
    <div class="flex-1 overflow-auto">
      ...
      >
        ...
        <el-table-column v-if="isColumnVisible('device')" prop="device_name" :label="t('channelDetail.colDevice')" width="130" show-overflow-tooltip />
        <el-table-column v-if="isColumnVisible('gbId')" prop="gb_id" :label="t('channelDetail.colGbId')" width="160" show-overflow-tooltip>
          <template #default="{ row }">
        ...
        <el-table-column v-if="isColumnVisible('vendor')" :label="t('channelDetail.colVendor')" width="120" show-overflow-tooltip>
          <template #default="{ row }">
        ...
        <el-table-column v-if="isColumnVisible('type')" :label="t('common.type')">
          <template #default="{ row }">
            <el-tag size="small" effect="plain" :type="row.node_type === 'directory' ? 'warning' : 'info'">
              {{ row.node_type === 'directory' ? t('channelDetail.directory') : t('channelDetail.channel') }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column v-if="isColumnVisible('audio')" :label="t('channelDetail.colAudio')" width="60" align="center">
          <template #default="{ row }">
            <el-switch
        ...
        <el-table-column v-if="isColumnVisible('stream')" :label="t('channelDetail.colStream')" width="130" align="center">
          <template #default="{ row }">
            <el-select
            ...
            >
              <el-option :label="t('channelDetail.streamMain')" value="main" />
              <el-option :label="t('channelDetail.streamSub')" value="sub" />
              <el-option :label="t('channelDetail.streamMainGeneric')" value="stream:0" />
              <el-option :label="t('channelDetail.streamSubGeneric')" value="stream:1" />
              <el-option :label="t('channelDetail.streamMainGb2022')" value="streamnumber:0" />
              <el-option :label="t('channelDetail.streamSubGb2022')" value="streamnumber:1" />
              <el-option :label="t('channelDetail.streamMainDahua')" value="streamprofile:0" />
              <el-option :label="t('channelDetail.streamSubDahua')" value="streamprofile:1" />
              <el-option :label="t('channelDetail.streamMainMercury')" value="streamMode:MAIN" />
              <el-option :label="t('channelDetail.streamSubMercury')" value="streamMode:SUB" />
            </el-select>
          </template>
        </el-table-column>
        <el-table-column v-if="isColumnVisible('mount')" :label="t('channelDetail.colMount')" width="80" align="center">
          <template #default="{ row }">
            <el-tag size="small" effect="plain" :type="catalogParentId(row) ? 'success' : 'info'">
              {{ catalogParentId(row) ? t('channelDetail.mounted') : t('channelDetail.unmounted') }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column v-if="isColumnVisible('node')" :label="t('channelDetail.colNode')" width="120" show-overflow-tooltip>
          <template #default="{ row }">
          ...
          <template #default="{ row }">
            <div class="flex items-center justify-center gap-1">
              ...
              <el-dropdown trigger="click" @command="emit('handleMoreCommand', row, $event)">
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item v-if="filters.listScope === 'on_node'" command="remove_from_node"><el-icon><Remove /></el-icon>{{ t('channelDetail.removeFromNode') }}</el-dropdown-item>
                    <el-dropdown-item v-if="filters.listScope === 'unadded' && canAddToSelectedNode" command="add_to_node"><el-icon><Plus /></el-icon>{{ t('channelDetail.addToNode') }}</el-dropdown-item>
                    <el-dropdown-item command="cloud"><el-icon><VideoCamera /></el-icon>{{ t('channelDetail.cloudRecord') }}</el-dropdown-item>
                    <el-dropdown-item command="device"><el-icon><Files /></el-icon>{{ t('channelDetail.deviceRecord') }}</el-dropdown-item>
                    <el-dropdown-item command="timeline"><el-icon><Timer /></el-icon>{{ t('channelDetail.timeline') }}</el-dropdown-item>
                    <el-dropdown-item command="reset" divided><el-icon><RefreshRight /></el-icon>{{ t('channelDetail.resetChannelInfo') }}</el-dropdown-item>
                    <el-dropdown-item command="delete" class="danger"><el-icon><Delete /></el-icon>{{ t('channelDetail.deleteChannel') }}</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
            </div>
    ...
    <div class="p-3 border-t border-slate-200 bg-slate-50/70 flex items-center justify-between">
      <div class="text-sm text-slate-500">
        <span>{{ t('channelDetail.totalPrefix') }}</span>
        <span class="font-semibold text-slate-700">{{ total }}</span>
        <span>{{ t('channelDetail.recordsUnit') }}</span>
        <span>{{ t('channelDetail.selectedPrefix') }}</span>
        <span class="font-semibold text-sky-600">{{ selectedChannels.length }}</span>
        <span>{{ t('channelDetail.recordsUnit') }}</span>
      </div>
      <div class="flex items-center gap-2">
        <el-pagination
          ...
          :prev-text="t('channelDetail.prevPage')"
          :next-text="t('channelDetail.nextPage')"
          size="small"
...
const playTooltip = (row: Record<string, unknown>) => {
  if (row.status !== 1 && Number(row.status) !== 1) return t('channelDetail.channelOffline')
  const gbId = String(row?.gb_id || row?.channelId || row?.id || '')
  const typeCode = gbId ? gbId.substring(10, 13) : ''
  const nonVideoTypes = ['133', '134', '135', '136', '137']
  if (nonVideoTypes.includes(typeCode)) {
    if (typeCode === '133' || typeCode === '134') return t('channelDetail.audioOnlyChannel')
    if (typeCode === '135' || typeCode === '136') return t('channelDetail.alarmChannel')
    if (typeCode === '137') return t('channelDetail.envMonitorChannel')
    return t('channelDetail.nonVideoChannel')
  }
  return t('channelDetail.play')
}
