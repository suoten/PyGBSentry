  ...
  <div class="min-h-screen p-3 space-y-3">
    ...
    <el-card shadow="never">
      <div class="flex justify-between items-center gap-2">
        <div>
          <div class="font-semibold" style="color: var(--el-text-color-primary)">{{ t('faceMobile.title') }}</div>
          <div class="text-xs mt-1" style="color: var(--el-text-color-secondary)">
            {{ t('faceMobile.currentStatus') }}
    ...
    <el-card shadow="never">
      <div class="flex justify-between items-center mb-2">
        <div class="font-semibold text-sm" style="color: var(--el-text-color-primary)">{{ t('faceMobile.callbackConfig') }}</div>
      ...
      <div class="text-xs space-y-1" style="color: var(--el-text-color-secondary)">
        <div>{{ t('faceMobile.callbackUrl') }}：{{ config.ai_callback_url || t('faceMobile.notConfigured') }}</div>
        <div>{{ t('faceMobile.pushSnapshotUrl') }}：{{ config.send_snapshot_url ? t('common.yes') : t('common.no') }}</div>
        <div>{{ t('faceMobile.syncNotifyUrl') }}：{{ formatSyncUrls(config.sync_urls) || t('faceMobile.notConfigured') }}</div>
        <div>{{ t('faceMobile.requestTimeout') }}：{{ config.timeout_seconds || 10 }} {{ t('faceMobile.secondsUnit') }}</div>
      </div>
    </el-card>

    <el-card shadow="never">
      <div class="flex justify-between items-center mb-2">
        <div class="font-semibold text-sm" style="color: var(--el-text-color-primary)">{{ t('faceMobile.recentPushRecords') }}</div>
      ...
      <div v-else-if="events.length === 0" class="text-xs py-4 text-center" style="color: var(--el-text-color-secondary)">{{ t('faceMobile.noRecords') }}</div>
    ...
    <AppDialog v-model="configDialogVisible" :title="t('faceMobile.configTitle')" size="medium">
      <el-form :model="configForm" label-width="90px">
        <el-form-item :label="t('faceMobile.callbackUrl')">
          <el-input v-model="configForm.ai_callback_url" :placeholder="t('faceMobile.callbackUrlPlaceholder')" clearable />
          <div class="text-[11px] mt-1" style="color: var(--el-text-color-secondary)">{{ t('faceMobile.callbackUrlTip') }}</div>
        </el-form-item>
        <el-form-item :label="t('faceMobile.syncNotify')">
          <el-input
            v-model="configForm.sync_urls"
            type="textarea"
            :rows="2"
            :placeholder="t('faceMobile.syncNotifyPlaceholder')"
          />
          <div class="text-[11px] mt-1" style="color: var(--el-text-color-secondary)">{{ t('faceMobile.syncNotifyTip') }}</div>
        </el-form-item>
        <el-form-item :label="t('faceMobile.pushSnapshot')">
          <el-switch v-model="configForm.send_snapshot_url" />
        </el-form-item>
        <el-form-item :label="t('faceMobile.requestTimeout')">
...
async function loadConfig() {
  ...
  try {
    ...
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('faceMobile.loadConfigFailed'))
  } finally {
...
async function saveConfig() {
  if (enabled.value && !(configForm.value.ai_callback_url || '').trim()) {
    ElMessage.warning(t('faceMobile.callbackUrlRequired'))
    return
  }
  saving.value = true
  try {
    ...
    const friendly = getFriendlyError(e)
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('faceMobile.saveFailedRetry'))
  } finally {
    saving.value = false
  }
}
...
async function onToggleEnabled() {
  try {
    ...
    ElMessage.success(enabled.value ? t('faceMobile.enabled') : t('faceMobile.disabled'))
  } catch (e: unknown) {
    const friendly = getFriendlyError(e)
    ElMessage.error(friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : friendly.message || t('faceMobile.toggleFailed'))
...
async function loadEvents() {
  ...
  try {
    const res = await api.get('/api/v1/structured/search', { params: { event_type: 'face', limit: 50 } })
    events.value = res.data?.items ?? res.data ?? []
  } catch (e) {
    logger.error('加载人脸识别事件失败', e)
    events.value = []
  } finally {
