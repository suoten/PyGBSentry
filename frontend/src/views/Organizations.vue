<template>
  <div class="app-page space-y-4">
    <PageContainer>
      <template #header>
        <PageHeader :title="t('org.title')" description="管理组织树与设备归属">
          <template #actions>
    ...
    <TableCard class="org-card">
      ...
      <div v-if="!treeData.length && !loading" class="empty-state py-8 text-center">
        ...
        <div class="empty-text">暂无组织，可点击「新增根组织」创建</div>
      </div>
    ...
    >
      <el-form :model="form" label-width="100px" class="org-form">
        <el-form-item v-if="parentName" label="上级组织">
        ...
        <el-form-item :label="t('common.name')">
          <div class="input-wrapper">
            ...
            <el-input v-model="form.name" placeholder="请输入组织名称" class="form-input" />
          </div>
        </el-form-item>
        <el-form-item v-if="!editId" label="排序">
          <div class="input-wrapper">
...
const submit = async () => {
  ...
  try {
    ...
    const friendly = getFriendlyError(e)
    ElMessage.error(
      friendly.suggestion ? `${friendly.message}（${friendly.suggestion}）` : (friendly.message || (editId.value ? '保存失败' : '创建失败'))
    )
  } finally {
    saving.value = false
  }
...
const doDelete = async (data: OrgNode) => {
  try {
    await ElMessageBox.confirm(`确定删除组织「${data.name}」？若存在子组织或已关联设备将无法删除。`, '删除确认', {
      type: 'warning',
