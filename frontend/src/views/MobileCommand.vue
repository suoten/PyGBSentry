<template>
  <div class="app-page h-full">
    <PageContainer class="h-full flex flex-col">
      <template #header>
        <PageHeader :title="t('mobileCommand.title')" :description="t('mobileCommand.description')" />
      </template>
      ...
      <TableCard class="shrink-0 mb-4">
        <template #header>
          <div class="flex items-center justify-between">
            <div class="font-medium">{{ t('mobileCommand.conferenceSession') }}</div>
            <div class="flex gap-2">
              <el-button link size="small" @click="loadActiveSessions">{{ t('mobileCommand.refreshSession') }}</el-button>
              <el-button type="primary" link size="small" @click="loadRecentAlarms">{{ t('mobileCommand.refreshAlarms') }}</el-button>
            </div>
          </div>
        </template>
        <div class="flex gap-4 flex-wrap">
          <div class="min-w-[280px] max-w-md">
            <div class="text-sm mb-2" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.activeSessions') }}</div>
            <div v-if="activeSessionsLoading" style="color: var(--el-text-color-secondary)">{{ t('common.loading') }}</div>
            <ul v-else class="space-y-2 max-h-32 overflow-y-auto">
              ...
              >
                ...
                <el-tag size="small" type="success">{{ t('mobileCommand.participantCount', { count: s.participant_count || 0 }) }}</el-tag>
              </li>
              <li v-if="!activeSessions.length" class="text-sm" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.noActiveSessions') }}</li>
            </ul>
          </div>
          <div class="min-w-[280px] max-w-md">
            <div class="text-sm mb-2" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.recentAlarmsHint') }}</div>
            <div v-if="recentAlarmsLoading" style="color: var(--el-text-color-secondary)">{{ t('common.loading') }}</div>
            <ul v-else class="space-y-2 max-h-32 overflow-y-auto">
              <li
              ...
              >
                ...
                <el-button size="small" type="primary" @click.stop="openConference(a)">{{ t('mobileCommand.startConference') }}</el-button>
              </li>
              <li v-if="!recentAlarms.length" class="text-sm" style="color: var(--el-text-color-secondary)">{{ t('common.empty') }}</li>
            </ul>
          </div>
        </div>
      </TableCard>
      ...
      <div class="flex-1 flex gap-4 overflow-hidden min-h-0">
        ...
        <div class="w-80 flex flex-col rounded-lg overflow-hidden shrink-0" style="border: 1px solid var(--el-border-color-lighter); background: var(--el-bg-color);">
          <div class="p-3 font-medium flex justify-between items-center shrink-0" style="border-bottom: 1px solid var(--el-border-color-lighter); background: var(--el-fill-color-light);">
            <span>{{ t('mobileCommand.onlineDevices', { count: onlineMobileCount }) }}</span>
            <el-button type="primary" link size="small" @click="loadChannels">{{ t('mobileCommand.refresh') }}</el-button>
          </div>
          <div class="flex-1 overflow-y-auto p-2 space-y-2 min-h-0">
            <div v-if="loading" class="py-4 text-center" style="color: var(--el-text-color-secondary)">{{ t('common.loading') }}</div>
            <div v-else-if="!mobileChannels.length" class="py-4 text-center" style="color: var(--el-text-color-secondary)">
              {{ t('mobileCommand.noOnlineDevices') }}
              <div class="text-xs mt-1">{{ t('mobileCommand.gpsHint') }}</div>
            </div>
            ...
            >
              <div class="flex justify-between items-start mb-1">
                ...
                <el-tag size="small" effect="plain" type="success">{{ t('mobileCommand.online') }}</el-tag>
              </div>
              ...
              <div class="mt-2 flex gap-2">
                <el-button size="small" type="primary" plain @click.stop="callDevice(ch)">
                  <el-icon class="mr-1"><VideoCamera /></el-icon> {{ t('mobileCommand.video') }}
                </el-button>
                <el-button size="small" type="warning" plain @click.stop="trackDevice(ch)">
                  <el-icon class="mr-1"><Position /></el-icon> {{ t('mobileCommand.track') }}
                </el-button>
              </div>
            </div>
          </div>
        ...
        <div class="flex-1 relative rounded-lg overflow-hidden shadow-lg min-h-0" style="border: 1px solid var(--el-border-color-lighter); background: #0b1220">
          ...
          <div id="mc-popup" class="ol-popup mc-popup" v-show="popupVisible">
            ...
            <div class="popup-content w-[400px] h-[260px] bg-black rounded-b-lg overflow-hidden relative">
              ...
              <div v-else class="h-full flex items-center justify-center text-white/30 bg-slate-950">
                {{ popupData.loading ? t('mobileCommand.requestingStream') : t('mobileCommand.noVideo') }}
              </div>
            </div>
          </div>
          ...
          <div v-if="trackingChannel" class="absolute top-4 right-4 z-10 p-3 rounded text-sm shadow-lg" style="background: rgba(255,255,255,.92); border: 1px solid var(--el-border-color-lighter)">
            <div class="font-bold mb-1 text-primary">{{ t('mobileCommand.tracking', { name: trackingChannel.name }) }}</div>
            <div class="text-xs mb-2" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.showRecentPoints') }}</div>
            <el-button size="small" link type="info" @click="clearTrack">{{ t('mobileCommand.stopTracking') }}</el-button>
          </div>
        </div>
      </div>

      <!-- 会商抽屉 -->
      <el-drawer v-model="conferenceDrawerVisible" :title="t('mobileCommand.conferenceSession')" size="420" direction="rtl">
        <div v-if="conferenceAlarm" class="space-y-4">
          <div class="text-sm" style="color: var(--el-text-color-regular)">
            <p><strong>{{ t('mobileCommand.alarm') }}</strong> {{ conferenceAlarm.device_id }} / {{ conferenceAlarm.description }}</p>
            <p class="mt-1" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.sessionId') }}: {{ conferenceSession?.id || conferenceAlarm.id }}</p>
          </div>
          <div class="h-48 bg-black rounded overflow-hidden">
            ...
            <div v-else class="h-full flex items-center justify-center" style="color: rgba(255,255,255,.6)">{{ t('mobileCommand.alarmChannelPreview') }}</div>
          </div>
          <div>
            <div class="text-sm font-medium mb-2">{{ t('mobileCommand.participants') }}</div>
            <div class="space-y-1 max-h-24 overflow-y-auto mb-3">
              ...
              <div v-if="!conferenceParticipants.length" class="text-sm" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.noParticipants') }}</div>
            </div>
            <div class="text-sm font-medium mb-2">{{ t('mobileCommand.conferenceInstructions') }}</div>
            <div class="space-y-2 max-h-48 overflow-y-auto mb-3">
              ...
              <div v-if="!conferenceInstructions.length" class="text-sm" style="color: var(--el-text-color-secondary)">{{ t('mobileCommand.noInstructions') }}</div>
            </div>
            <el-input v-model="newInstructionText" type="textarea" :rows="2" :placeholder="t('mobileCommand.instructionPlaceholder')" />
            <div class="mt-2 flex gap-2">
              <el-button type="primary" size="small" @click="publishInstruction">{{ t('mobileCommand.publishInstruction') }}</el-button>
              <el-button type="success" size="small" @click="closeConference">{{ t('mobileCommand.endConference') }}</el-button>
            </div>
          </div>
        </div>
      </el-drawer>
...
function getLayer(provider: string, key: string) {
  if (provider === 'vector') {
    ...
    const k = key
    if (!k) {
      ElMessage.warning(t('mobileCommand.tiandituKeyMissing'))
...
async function loadChannels() {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadDevicesFailed')))
  } finally {
    loading.value = false
  }
}
...
async function callDevice(ch: Record<string, unknown>) {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.getStreamFailed')))
    popupData.value.url = ''
  } finally {
...
async function trackDevice(ch: Record<string, unknown>) {
  ...
  try {
    ...
    ElMessage.warning(t('mobileCommand.trackLoadFailed'))
  }
}
...
async function loadRecentAlarms() {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadAlarmsFailed')))
  } finally {
...
async function joinExistingSession(session: Record<string, unknown>) {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadConferenceInfoFailed')))
  }
}
...
async function loadConferenceInstructions() {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadInstructionsFailed')))
  }
}
...
async function loadConferenceParticipants() {
  ...
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadParticipantsFailed')))
  }
}
...
async function openConference(alarm: Record<string, unknown>) {
  ...
  try {
    const sessionRes = await api.post('/api/v1/command/sessions', {
      alarm_id: alarm.id,
      title: alarm.description || t('mobileCommand.alarmConferenceTitle', { id: alarm.id })
    })
    conferenceSession.value = sessionRes.data
    showSuccess(t('mobileCommand.conferenceStarted'))
    if (conferenceSession.value?.id) {
      await api.post(`/api/v1/command/sessions/${conferenceSession.value.id}/join`, { role: 'participant' })
    }
  } catch (e: unknown) {
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.startConferenceFailed')))
  ...
  try {
    ...
    ElMessage.warning(t('mobileCommand.conferenceVideoLoadFailed'))
  }
}
...
async function publishInstruction() {
  ...
  try {
    ...
    showSuccess(t('mobileCommand.instructionPublished'))
    newInstructionText.value = ''
    await loadConferenceInstructions()
  } catch (e: unknown) {
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.publishInstructionFailed')))
  }
}
...
async function closeConference() {
  ...
  try {
    await api.post(`/api/v1/command/sessions/${conferenceSession.value.id}/close`, {
      summary: t('mobileCommand.conferenceClosedSummary')
    })
    showSuccess(t('mobileCommand.conferenceEnded'))
    conferenceDrawerVisible.value = false
    conferenceSession.value = null
  } catch (e: unknown) {
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.endConferenceFailed')))
  }
}
...
onMounted(async () => {
  try {
    ...
    ElMessage.warning(getApiErrorMessage(e, t('mobileCommand.loadConfigFailed')))
  }
  initMap()
  loadChannels()
  loadRecentAlarms()
