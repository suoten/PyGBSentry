  ...
  <div class="enhanced-stream-player relative w-full h-full bg-black overflow-hidden">
    ...
    >
      ...
      <div v-if="showReconnecting" class="reconnect-overlay absolute inset-0 flex flex-col items-center justify-center bg-black/70 z-15">
        ...
        <div class="text-white text-sm font-medium">连接中断，正在重连...</div>
      ...
      >
        <div class="text-xs text-white/90 space-y-1">
          ...
          <div v-if="metrics.bitrate > 0">码率: <span class="text-blue-400">{{ formatBitrate(metrics.bitrate) }}</span></div>
          <div v-if="metrics.resolution">分辨率: <span class="text-yellow-400">{{ metrics.resolution }}</span></div>
          <div>丢帧: <span :class="metrics.droppedFrames > 10 ? 'text-red-400' : 'text-gray-400'">{{ metrics.droppedFrames }}</span></div>
          <div>缓冲: <span :class="bufferStatusClass">{{ bufferStatus }}</span></div>
        </div>
      </div>
      ...
      >
        ...
        <div class="control-buttons px-4 pb-3 flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            <!-- 截图 -->
            <button class="control-btn" @click="screenshot" title="截图">
