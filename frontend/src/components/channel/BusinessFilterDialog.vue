  ...
  >
    <el-form :model="form" label-width="100px">
      <el-form-item label="省份">
        <el-select v-model="form.province" filterable placeholder="选择省份代码" style="width: 100%">
          <el-option v-for="item in provinces" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="市级代码">
        <el-select v-model="form.city" filterable allow-create default-first-option placeholder="选择或输入两位市级代码" style="width: 100%">
          <el-option v-for="item in cityOptions" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="区县代码">
        <el-select v-model="form.district" filterable allow-create default-first-option placeholder="选择或输入两位区县代码" style="width: 100%">
          <el-option v-for="item in districtOptions" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="末尾2位编号">
        <el-input v-model="form.suffix" maxlength="2" placeholder="两位数字，如01" />
      </el-form-item>
...
const dialogTitle = computed(() => props.title || '设置行政区划')
const confirmLabel = computed(() => props.mode === 'batch_region' ? '批量设置' : '确认')
...
const namePreview = computed(() => {
  const pn = provinces.find(x => x.code === form.value.province)?.name || '未选择'
  return pn
})
...
const handleSubmit = async () => {
  ...
  try {
    if (props.mode === 'batch_region') {
      ...
      ElMessage.success(`已为 ${props.targetGbIds.length} 个通道设置行政区划`)
    } else {
