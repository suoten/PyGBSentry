<template>
  ...
  >
    ...
    <el-tree
      ...
      @node-click="onClick"
    />
    <div class="text-xs mt-2" style="color: var(--el-text-color-secondary)">{{ t('pickGroupDlg.tip') }}</div>
    <template #footer>
...
const loadTree = async () => {
  ...
  try {
    ...
    showError(t('pickGroupDlg.loadTreeError'), e)
    treeData.value = []
  } finally {
    loading.value = false
  }
...
const submit = () => {
  if (!pick.value) {
    ElMessage.warning(t('pickGroupDlg.pleasePickNode'))
    return
  }
