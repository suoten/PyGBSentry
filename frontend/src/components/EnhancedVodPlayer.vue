...
// FIX: [2026-07-17 P1] 保存 document 级监听器引用，用于 destroy 时 removeEventListener
let _docKeydownHandler: ((e: KeyboardEvent) => void) | null = null
let _docFullscreenHandler: (() => void) | null = null
...
function destroy() {
  ...
  // FIX: [2026-07-17 P1] 移除 document 级事件监听器，防止内存泄漏
  if (_docKeydownHandler) {
    document.removeEventListener('keydown', _docKeydownHandler)
    _docKeydownHandler = null
  }
  if (_docFullscreenHandler) {
    document.removeEventListener('fullscreenchange', _docFullscreenHandler)
    _docFullscreenHandler = null
  }
...
function setupEventListeners() {
  ...
  _docKeydownHandler = handleKeydown  // FIX: [2026-07-17 P1] 保存引用用于 destroy 时移除
  ...
  _docFullscreenHandler = handleFullscreenChange  // FIX: [2026-07-17 P1] 保存引用用于 destroy 时移除
}
