    ...
    <template #header>
      <div class="stream-player-dialog__header">
        <div class="flex items-center gap-2 min-w-0">
          ...
          <div class="min-w-0">
            <div class="font-semibold text-slate-900 truncate">{{ titleText }}</div>
        ...
        <div class="stream-player-dialog__main">
          <div class="stream-player-dialog__toolbar">
            <el-radio-group v-model="modeState" size="small">
              ...
              <el-radio-button v-if="urls.raw" value="raw">{{ t('streamPlayerDlg.raw') }}</el-radio-button>
            </el-radio-group>
            <div class="flex items-center gap-2">
              <el-dropdown v-if="switchStreamType" trigger="click" @command="switchStreamType">
                <el-button size="small" :loading="switchingStream" :disabled="!playUrl">{{ t('streamPlayerDlg.switchStream') }}</el-button>
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item command="main">{{ t('streamPlayerDlg.mainStream') }}</el-dropdown-item>
                    <el-dropdown-item command="sub">{{ t('streamPlayerDlg.subStream') }}</el-dropdown-item>
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
              <el-button size="small" type="danger" plain @click="handleStop" :disabled="!playUrl">{{ t('streamPlayerDlg.stop') }}</el-button>
              <el-button size="small" @click="copyPlayUrl" :disabled="!playUrl">{{ t('streamPlayerDlg.copyUrl') }}</el-button>
              <el-button size="small" @click="openPlayUrl" :disabled="!playUrl">{{ t('streamPlayerDlg.openInNewTab') }}</el-button>
            </div>
          </div>
          ...
          <div class="stream-player-dialog__stage">
            ...
            <div v-if="showStreamStats && streamMetrics" class="stream-stats-overlay">
              <div class="stats-grid">
                ...
                <span class="stats-label">{{ t('streamPlayerDlg.bitrate') }}</span>
                <span class="stats-value">{{ formatBitrate(streamMetrics.bitrate) }}</span>
                <span class="stats-label">{{ t('streamPlayerDlg.buffer') }}</span>
                <span class="stats-value" :class="bufferClass">{{ streamMetrics.buffer }}%</span>
                <span class="stats-label">{{ t('streamPlayerDlg.health') }}</span>
...
const titleText = computed(() => props.title || t('streamPlayerDlg.livePreview'))
...
const copyPlayUrl = async () => {
  ...
  try {
    ...
    ElMessage.success(t('streamPlayerDlg.copied'))
  } catch {
    ElMessage.error(t('streamPlayerDlg.copyFailed'))
  }
}
