  ...
  <el-dialog v-model="dialogVisible" width="88%" class="record-workspace-dialog" destroy-on-close>
    <template #header>
      <div class="title-wrap">
        <div class="title">{{ t('timelineRecordWs.title') }}</div>
        <div class="subtitle">{{ deviceName || deviceId || t('timelineRecordWs.noDeviceSelected') }}</div>
      </div>
    </template>
    ...
    <div class="toolbar">
      <el-select
        ...
        :placeholder="t('timelineRecordWs.selectChannel')"
      ...
      <el-button size="small" @click="switchChannel(-1)">{{ t('timelineRecordWs.prevChannel') }}</el-button>
      <el-button size="small" @click="switchChannel(1)">{{ t('timelineRecordWs.nextChannel') }}</el-button>
      <el-select :model-value="windowMinutes" size="small" class="window-select" @update:model-value="onWindowChange">
        <el-option v-for="item in [15, 30, 60, 120]" :key="item" :label="t('timelineRecordWs.windowMinutes', { count: item })" :value="item" />
      </el-select>
      <el-date-picker
        ...
        class="time-picker"
        :placeholder="t('timelineRecordWs.anchorPlaceholder')"
        @update:model-value="onAnchorChange"
      />
      <el-button size="small" @click="setAnchorNow">{{ t('timelineRecordWs.backToNow') }}</el-button>
    </div>
