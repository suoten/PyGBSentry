  ...
  <el-dialog v-model="dialogVisible" width="88%" class="record-workspace-dialog" destroy-on-close>
    <template #header>
      <div class="title-wrap">
        ...
        <div class="subtitle">{{ deviceName || deviceId || t('cloudRecordWs.noDeviceSelected') }}</div>
      </div>
    </template>
    ...
    <div class="toolbar">
      <el-select
        ...
        :placeholder="t('cloudRecordWs.selectChannel')"
      ...
      <el-button size="small" @click="switchChannel(-1)">{{ t('cloudRecordWs.prevChannel') }}</el-button>
      <el-button size="small" @click="switchChannel(1)">{{ t('cloudRecordWs.nextChannel') }}</el-button>
      <el-select :model-value="windowMinutes" size="small" class="window-select" @update:model-value="onWindowChange">
        <el-option v-for="item in [15, 30, 60, 120]" :key="item" :label="t('cloudRecordWs.windowMinutes', { count: item })" :value="item" />
      </el-select>
      <el-date-picker
        ...
        class="time-picker"
        :placeholder="t('cloudRecordWs.anchorPlaceholder')"
        @update:model-value="onAnchorChange"
      />
      <el-button size="small" @click="setAnchorNow">{{ t('cloudRecordWs.backToNow') }}</el-button>
    </div>
