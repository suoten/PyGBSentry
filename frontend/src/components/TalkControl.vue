<template>
  <div class="talk-wrap">
    <div class="talk-top">
      <span class="label">对讲(Talk)</span>
      <el-switch v-model="broadcastMode" />
      <span class="label active">喊话(Broadcast)</span>
    </div>
    ...
    >
      ...
      <div class="mic-text">{{ isTalking ? '松开停止' : '按住说话' }}</div>
      <div class="mic-sub">按住讲话，松开停止</div>
    </div>
...
const startTalk = async () => {
  ...
  try {
    ...
    ElMessage.success(broadcastMode.value ? '喊话已开始' : '对讲连接成功，可以开始说话')
  } catch (error) {
    isConnecting.value = false
...
const stopTalk = async () => {
  ...
  try {
    ...
    ElMessage.success(broadcastMode.value ? '喊话已结束' : '对讲已结束')
  } catch (error) {
    const friendly = getFriendlyError(error)
