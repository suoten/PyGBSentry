  ...
  <div class="ptz-wrap">
    ...
    <div class="ptz-right">
      <div class="ptz-method">
        <span class="ptz-method-label">{{ t('ptzCtrl.controlMethod') }}</span>
        <el-radio-group v-model="ptzMethod" size="small" class="ptz-method-radio">
          <el-radio-button value="preset">{{ t('ptzCtrl.preset') }}</el-radio-button>
          <el-radio-button value="cruise">{{ t('ptzCtrl.cruise') }}</el-radio-button>
          <el-radio-button value="scan">{{ t('ptzCtrl.scan') }}</el-radio-button>
          <el-radio-button value="wiper">{{ t('ptzCtrl.wiper') }}</el-radio-button>
          <el-radio-button value="switch">{{ t('ptzCtrl.auxSwitch') }}</el-radio-button>
        </el-radio-group>
      </div>
      <PtzPreset
...
const pressedActionLabel = computed(() => {
  const map: Record<string, string> = {
    up: t('ptzCtrl.dirUp'),
    down: t('ptzCtrl.dirDown'),
    left: t('ptzCtrl.dirLeft'),
    right: t('ptzCtrl.dirRight'),
    zoomin: t('ptzCtrl.zoomIn'),
    zoomout: t('ptzCtrl.zoomOut'),
    focus_near: t('ptzCtrl.focusNear'),
    focus_far: t('ptzCtrl.focusFar'),
    iris_in: t('ptzCtrl.irisIn'),
    iris_out: t('ptzCtrl.irisOut')
  }
...
const setPreset = async () => {
  ...
  if (!Number.isInteger(presetId) || presetId < 1 || presetId > 255) {
    ElMessage.warning(t('ptzCtrl.presetIdRangeError'))
    return
  }
  try {
    ...
    ElMessage.success(t('ptzCtrl.presetSaved'))
    await syncControlState()
    const ok = await loadControlState()
    if (!ok) await loadPresetList()
  } catch (e: unknown) {
    ElMessage.error(getApiErrorMessage(e, t('ptzCtrl.presetSaveFailed')))
  }
}
...
const callPreset = async () => {
  ...
  try {
    ...
    ElMessage.success(t('ptzCtrl.presetCalled'))
  } catch (e: unknown) {
    ElMessage.error(getApiErrorMessage(e, t('ptzCtrl.presetCallFailed')))
  }
}
...
const deletePreset = async () => {
  ...
  const presetId = Number(presetForm.value.preset_id)
  try {
    await ElMessageBox.confirm(t('ptzCtrl.confirmDeletePreset'), t('common.tips'), {
    ...
    savePresetList()
    ElMessage.success(t('ptzCtrl.presetDeleted'))
    await syncControlState()
    const ok = await loadControlState()
    if (!ok) await loadPresetList()
  } catch (e) {
...
const ptzCamera = async (command: string) => {
  ...
  try {
    ...
    ElMessage.error(t('ptzCtrl.ptzControlFailed'))
  }
}
...
const focusCamera = async (command: string) => {
  ...
  try {
    ...
    ElMessage.error(t('ptzCtrl.focusControlFailed'))
  }
}
...
const irisCamera = async (command: string) => {
  ...
  try {
    ...
    ElMessage.error(t('ptzCtrl.irisControlFailed'))
  }
}
...
const doCruise = async (action: 'add' | 'delete' | 'set_speed' | 'set_time' | 'start' | 'stop' | 'delete_group') => {
  ...
  try {
    ...
    ElMessage.success(t('ptzCtrl.cruiseCommandSent'))
    await syncControlState()
    const ok = await loadControlState()
    if (!ok) await loadCruisePointList()
  } catch (e: unknown) {
    ElMessage.error(getApiErrorMessage(e, t('ptzCtrl.cruiseControlFailed')))
  } finally {
...
const deleteCruise = async () => {
  ...
  try {
    await ElMessageBox.confirm(t('ptzCtrl.confirmDeleteCruise'), t('common.tips'), {
      confirmButtonText: t('common.ok'),
      cancelButtonText: t('common.cancel'),
      type: 'warning'
    })
    ...
    cruisePointList.value = []
    saveCruisePointList()
    ElMessage.success(t('ptzCtrl.cruiseCleared'))
    await syncControlState()
    const ok = await loadControlState()
    if (!ok) await loadCruisePointList()
  } catch {
...
const doScan = async (action: 'start' | 'stop' | 'set_left' | 'set_right' | 'set_speed') => {
  ...
  try {
    ...
    ElMessage.success(t('ptzCtrl.scanCommandSent'))
    if (action === 'set_speed') scanSetSpeedVisible.value = false
    await syncControlState()
    const ok = await loadControlState()
    if (!ok) await loadScanConfig()
  } catch (e: unknown) {
    ElMessage.error(getApiErrorMessage(e, t('ptzCtrl.scanControlFailed')))
  } finally {
