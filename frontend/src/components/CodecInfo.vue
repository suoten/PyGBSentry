<template>
  <div class="codec-info-panel">
    <div v-if="loading" class="codec-loading">
      ...
      <span class="text-slate-500 mt-2">{{ t('codecInfo.loading') }}</span>
    </div>

    <div v-else-if="!hasInfo" class="codec-empty">
      <el-icon class="text-4xl text-slate-300"><DataLine /></el-icon>
      <span class="text-slate-400 mt-2">{{ t('codecInfo.empty') }}</span>
    ...
    <div v-else class="codec-content">
      <div class="info-section">
        <div class="section-header">
          ...
          <span>{{ t('codecInfo.videoInfo') }}</span>
        </div>
        <div class="info-grid">
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.codecFormat') }}</span>
            <span class="info-value">{{ codecInfo.videoCodec || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.resolution') }}</span>
            <span class="info-value">{{ codecInfo.resolution || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.frameRate') }}</span>
            <span class="info-value">{{ codecInfo.fps ? `${codecInfo.fps} fps` : '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.bitrate') }}</span>
      ...
      <div class="info-section">
        <div class="section-header">
          ...
          <span>{{ t('codecInfo.audioInfo') }}</span>
        </div>
        <div class="info-grid">
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.codecFormat') }}</span>
          ...
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.sampleRate') }}</span>
            <span class="info-value">{{ codecInfo.sampleRate ? `${codecInfo.sampleRate} Hz` : '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.channels') }}</span>
      ...
      <div class="info-section">
        <div class="section-header">
          ...
          <span>{{ t('codecInfo.streamInfo') }}</span>
        </div>
        <div class="info-grid">
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.appName') }}</span>
            <span class="info-value font-mono">{{ app || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.streamId') }}</span>
            <span class="info-value font-mono truncate" :title="stream">{{ stream || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.mediaServer') }}</span>
            <span class="info-value font-mono truncate" :title="mediaServerId">{{ mediaServerId || '-' }}</span>
          </div>
          <div class="info-item">
            <span class="info-label">{{ t('codecInfo.onlineDuration') }}</span>
      ...
      <div class="info-section">
        <div class="section-header">
          ...
          <span>{{ t('codecInfo.realtimeStats') }}</span>
        </div>
        <div class="stats-chart">
          <div class="stat-bar">
            <div class="stat-bar-label">
              <span>{{ t('codecInfo.videoBandwidth') }}</span>
          ...
          <div class="stat-bar">
            <div class="stat-bar-label">
              <span>{{ t('codecInfo.audioBandwidth') }}</span>
          ...
          <div class="stat-bar">
            <div class="stat-bar-label">
              <span>{{ t('codecInfo.bufferFrames') }}</span>
...
const fetchInfo = async () => {
  ...
  } catch (e) { showError(t('codecInfo.fetchError'), e) } finally {
    loading.value = false
  }
}
