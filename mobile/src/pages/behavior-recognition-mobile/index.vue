...
const summaryText = computed(() => {
  return `行为识别：状态=${enabled.value ? "启用" : "停用"}；事件=${events.value.length}/${total.value}；回调=${String(config.value.ai_callback_url || "未配置")}`;
});

const nextStepAdvice = computed(() => {
  if (enabled.value && !String(config.value.ai_callback_url || "").trim()) return "下一步建议：启用状态下请先配置回调地址。";
  if (!events.value.length) return "下一步建议：先触发行为告警样本，再刷新事件列表验证回调链路。";
  return "下一步建议：抽检最新事件 payload，验证外部分析服务回执一致性。";
});
...
async function loadEvents() {
  ...
  try {
    const res = await fetchRecognitionEvents("behavior", 50);
...
function copySummary() {
  ...
  uni.setClipboardData({
    data: text,
    success: () => uni.showToast({ title: "行为识别摘要已复制", icon: "none" }),
...
<template>
  <view class="app-page app-gap-12">
    <view class="app-title">行为识别</view>
    ...
    <view class="app-card app-gap-12">
      ...
      <input v-model="editAiCallbackUrl" class="app-input" placeholder="http(s)://行为识别分析服务API地址" />
