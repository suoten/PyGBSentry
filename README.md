...
# 方式一：自动生成密钥并启动（推荐）
python tools/generate_env.py --docker          # 自动生成 .env（含全部安全密钥）
docker compose up -d                             # 启动全部服务

# 方式二：一行命令全自动部署
make docker-init                                 # 生成密钥 + 构建 + 启动

# 方式三：使用部署脚本
./deploy/setup.sh init                           # 交互式初始化 + 启动
```

访问 `http://<服务器IP>` 进入安装向导，首次使用创建管理员账号即可。

> **部署诊断**：启动后运行 `./deploy/setup.sh doctor` 检查端口、数据库、ZLM、SIP 等服务状态。
