    ...
    <PageContainer>
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-3 mb-4">
        ...
        <el-card class="stat-card stat-card--avg">
          ...
          <div class="stat-content">
            <div class="stat-label">平均确认时长（分钟）</div>
      ...
      <QueryFormSection :title="t('alarm.filterTitle')" :default-collapsed="true">
        <el-form-item :label="t('alarm.statusLabel')">
          <el-select v-model="filters.escalation_state" placeholder="全部状态" clearable class="filter-select">
      ...
      <TableCard class="alarms-card">
        ...
        <el-table v-else :data="alarms" v-loading="loading" :empty-text="'暂无告警记录'" class="alarms-table" :row-class-name="getAlarmRowClass" fit>
          <template #empty>
            <EmptyStateWithAction description="暂无告警记录，产生告警后会显示在此处；可在“监控中心”查看实时画面。">
              <template #action>
            ...
            <template #default="{ row }">
              <div class="status-cell">
                ...
                <el-tag :type="row.escalation_state === 'acknowledged' ? 'success' : 'warning'" size="small" effect="dark">
                  {{ row.escalation_state === 'acknowledged' ? t('alarm.confirmedSuccess') : '未确认' }}
                </el-tag>
              </div>
            </template>
          </el-table-column>
                ...
                <el-dropdown trigger="click" @command="(cmd: string) => handleAlarmMoreCommand(row, cmd)">
                  ...
                  <template #dropdown>
                    <el-dropdown-menu>
                      ...
                      <el-dropdown-item command="ack" :disabled="row.escalation_state === 'acknowledged'">确认</el-dropdown-item>
                      <el-dropdown-item command="escalate" :disabled="row.escalation_state === 'acknowledged'">升级</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
                </el-dropdown>
              </div>
      ...
      <el-card class="mt-4">
        <template #header>
          <div class="flex justify-between items-center">
            <span class="font-semibold">告警联动配置</span>
          </div>
        </template>
        <div class="flex items-center gap-4">
          <span class="text-sm">告警录像联动</span>
          <el-switch v-model="alarmRecordLinkEnabled" :loading="alarmConfigLoading" @change="saveAlarmConfig" />
          <span class="text-xs" style="color: var(--el-text-color-secondary)">开启后设备上报报警时自动触发录像</span>
        </div>
      </el-card>
