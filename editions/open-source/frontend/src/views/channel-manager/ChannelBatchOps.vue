  ...
  <AppDialog :model-value="props.civilCodeDialogVisible" @update:model-value="(v: boolean) => emit('update:civilCodeDialogVisible', v)" :title="civilCodeDialogTitle" size="medium">
    <el-form :model="civilCodeForm" label-width="100px">
      <el-form-item label="省份">
        <el-select v-model="civilCodeForm.province" filterable placeholder="选择省份代码" style="width: 100%">
          <el-option v-for="item in provinceOptions" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="市级代码">
        <el-select v-model="civilCodeForm.city" filterable allow-create default-first-option placeholder="选择或输入两位市级代码" style="width: 100%">
          <el-option v-for="item in cityOptions" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="区县代码">
        <el-select v-model="civilCodeForm.district" filterable allow-create default-first-option placeholder="选择或输入两位区县代码" style="width: 100%">
          <el-option v-for="item in districtOptions" :key="item.code" :label="`${item.name} - ${item.code}`" :value="item.code" />
        </el-select>
      </el-form-item>
      <el-form-item label="末尾2位编号">
        <el-input v-model="civilCodeForm.suffix" maxlength="2" placeholder="两位数字，如01" />
      </el-form-item>
    ...
    <template #footer>
      <el-button @click="emit('update:civilCodeDialogVisible', false)">取消</el-button>
...
const civilCodeDialogTitle = computed(() => {
  switch (props.civilPickerTarget) {
    case 'list_filter': return '行政区划筛选'
    case 'batch_region': return '批量设置行政区划'
    default: return '生成行政区划编码'
  }
})

const civilCodeDialogConfirmLabel = computed(() => {
  switch (props.civilPickerTarget) {
    case 'list_filter': return '确定'
    case 'batch_region': return '确定并应用'
    default: return '生成并使用'
  }
})
