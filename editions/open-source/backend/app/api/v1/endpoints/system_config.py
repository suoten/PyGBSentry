...
class SystemInfoResponse(BaseModel):
    ...
    version: str
    project_name: str
    sip_auth_skipped: bool = False  # FIXED: 暴露SIP鉴权跳过状态，便于前端安全提示
...
async def save_database_config(
    ...
    return {
        "status": "ok",
        "message": "Database config saved, restart backend to take effect"  # FIXED: hardcoded Chinese → English
    }
...
async def get_system_info(
    ...
    return {
        ...
        "sip_auth_skipped": not bool(settings.SIP_DEFAULT_PASSWORD) if current_user.is_superuser else False,  # FIXED: W45 SIP鉴权跳过状态仅管理员可见
    }
        ...
        if host:
            ...
            if candidate and not candidate.startswith("localhost"):
                if _is_ipv4(candidate):
                    ...
                    resolved = _resolve_to_ipv4(candidate)
                    sip_ip = resolved if resolved else _get_real_ip()
            else:
                sip_ip = _get_real_ip()
        else:
            sip_ip = _get_real_ip()
