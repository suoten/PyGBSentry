...
# FIXED: 缺少网络层诊断工具，仅依赖 SIP 信令层追踪无法定位网络故障
...
async def port_probe(
    ...
    # FIXED: 实现端口探测，使用 asyncio.open_connection 测试 TCP 连通性
    _validate_host(host)
...
async def sip_server_probe(
    ...
    # FIXED: 实现 SIP 服务探测，发送 SIP OPTIONS 请求并等待响应
    _validate_host(host)
...
async def media_server_probe(
    ...
    # FIXED: 实现流媒体服务探测，调用 ZLM /index/api/getServerConfig API 测试连通性
    _validate_host(host)
