...
async def get_trial_status(
    ...
    if server_config:
        ...
        enable_trial = int(getattr(settings, "TRIAL_DAYS", 7) or 0) > 0
        default_trial_days = int(getattr(settings, "TRIAL_DAYS", 7) or 0)
        plugin_trial_days = {}
...
async def list_plans(
    ...
    # 开源版：从配置文件读取
    plans_from_config = getattr(settings, "LOCAL_BILLING_PLANS", None)
...
async def create_plugin_order(
    ...
    return {
        ...
        "callback_sign_example": _sign_callback(order.order_no, "paid", order.amount, "provider-trade-no"),
...
async def get_order_status(
    ...
    result = await db.execute(stmt)
    order = get_or_404(result, detail="Order not found")  # FIXED: ORM查询结果空值判断
    return order
...
async def subscription_downgrade_preview(
    ...
    new_plan = get_or_404(new_plan_result, detail=f"BillingPlan '{payload.plan_id}' not found or inactive")  # FIXED: ORM查询结果空值判断

    if not old_plan:
        raise HTTPException(status_code=400, detail=f"Current plan '{old_plan_code}' does not exist")  # FIXED: 硬编码中文→英文
