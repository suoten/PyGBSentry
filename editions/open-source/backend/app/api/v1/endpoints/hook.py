...
# FIXED: track delayed none-reader cleanup tasks to prevent GC
...
async def verify_zlm_secret(secret: str | None = None): # FIXED: 类型注解不精确
    if not secret:
    ...
    try:
        ...
        logger.warning(f"Hook callback operation failed: {e}")  # FIXED: 硬编码中文→英文
...
async def on_stream_none_reader(request: Request):
    ...
    if none_reader_delay > 0 and app_name == "live":
        ...
        _track_none_reader_task(_t)  # FIXED: prevent GC of delayed cleanup task
        return {"code": 0, "close": False}
