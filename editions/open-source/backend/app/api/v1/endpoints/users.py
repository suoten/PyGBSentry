...
from app.api.v1.endpoints.login import _validate_password_strength  # FIXED-P2: W-11 Admin creates user must validate password strength
...
async def _ensure_system_roles(db: AsyncSession, tenant_id: str) -> None:
    ...
    changed = False
    for code, name in (("viewer", "Viewer"), ("operator", "Operator"), ("admin", "Admin"), ("owner", "Owner")):  # FIXED-P3: W-15 hardcoded Chinese→English
...
async def _validate_role(db: AsyncSession, tenant_id: str, role: str) -> None:
    r = (role or "").strip()
    if not r:
        raise HTTPException(status_code=400, detail="Role is required")  # FIXED: 中文→英文
    ...
    if not row:
        raise HTTPException(status_code=400, detail="Role not found")  # FIXED: 中文→英文
...
async def create_user(
    *,
    ...
    if result.scalars().first():
        ...
        raise HTTPException(
            status_code=400,
            detail="Username already exists",  # FIXED: hardcoded Chinese → English
        )
    ...
    try:
        ...
        raise
    # FIXED-P2: W-11 Admin creates user must validate password strength
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=400, detail=f"Create user failed: {str(e)}") from e  # FIXED: db.commit异常保护+rollback
    await safe_auth_audit(
        db,
        module="users",
        action="create_user",
...
async def enable_totp(
    ...
    if not verify_totp(payload.code, decrypt_totp_secret(encrypted_secret)):
        await safe_auth_audit(
            db,
            action="totp_enable",
            source="2fa",
...
async def disable_totp(
    ...
    encrypted_secret = getattr(current_user, "totp_secret", None)
    if not encrypted_secret or not verify_totp(payload.code, decrypt_totp_secret(encrypted_secret)):
        await safe_auth_audit(
            db,
            action="totp_disable",
            source="2fa",
...
async def unlock_user(
    ...
    stmt = select(User).where(User.id == user_id, User.tenant_id == current_user.tenant_id)  # FIXED-P2: S-05 添加租户隔离
    result = await db.execute(stmt)
