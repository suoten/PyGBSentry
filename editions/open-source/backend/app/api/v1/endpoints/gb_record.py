...
def _parse_record_time(value: str, field_name: str) -> datetime:
    ...
    if not text:
        raise HTTPException(status_code=400, detail=f"{field_name} is required")  # FIXED: hardcoded Chinese → English
    normalized = text.replace(" ", "T")
    try:
        dt = datetime.fromisoformat(normalized)
    except Exception:
        raise HTTPException(status_code=400, detail=f"{field_name} format error, expected yyyy-MM-dd HH:mm:ss or ISO8601")  # FIXED: hardcoded Chinese → English
...
async def _friendly_asset_channel_not_found_detail(
    ...
    *,
    ...
    if asset.status == 0:
        return {
            "message": f"Device {asset.name or asset.gb_id} is offline, cannot query recordings.",  # FIXED: hardcoded Chinese → English
    ...
    if not channel:
        return {
            "message": "Device exists but the channel does not belong to it. Device or channel not found.",  # FIXED: hardcoded Chinese → English
    ...
    return {
        "message": "Device or channel status has changed, please refresh and retry.",  # FIXED: hardcoded Chinese → English
...
async def query_record_compat(
    ...
    if start_dt > end_dt:
        raise HTTPException(status_code=400, detail="startTime cannot be greater than endTime")  # FIXED: hardcoded Chinese → English
...
async def stop_download_compat(
    ...
    if not task:
        raise HTTPException(status_code=404, detail="Download task not found")  # FIXED: hardcoded Chinese → English
