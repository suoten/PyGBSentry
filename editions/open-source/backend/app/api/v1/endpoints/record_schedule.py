...
async def _start_record(proxy_host: str, proxy_http_port: int, proxy_secret: str, stream: str) -> None:
    ...
    try:
        r = await (await get_http_client()).get(url, params={"secret": proxy_secret, "vhost": "__defaultVhost__", "app": "live", "stream": stream, "type": 1}, timeout=5)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except (httpx.ConnectError, httpx.TimeoutException, httpx.HTTPError) as e:
        raise RuntimeError(f"startRecord request failed: {e}") from e
    # FIXED: requests.get无try-catch，ZLM离线时ConnectionError未捕获
...
async def force_start_schedule(
    ...
    try:
        await _start_record(proxy_host, proxy_http_port, proxy_secret, stream)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
async def force_stop_schedule(
    ...
    try:
        await _stop_record(proxy_host, proxy_http_port, proxy_secret, stream)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
        rt.is_recording = False
        rt.last_action_ok = True
        rt.last_error = ""
    except Exception as e:
