...
def _build_media_server_payload(selected_node, media_host: str | None, http_port: int | None, zlm_probe_ok: bool) -> dict:
    public_host = _first_non_empty(
        ...
        getattr(settings, "STREAM_PUBLIC_HOST", ""),
        getattr(settings, "MEDIA_SERVER_HOST", ""),
    )
    https_port = _safe_int(
        _node_value(selected_node, "https_port"),
        _safe_int(getattr(settings, "MEDIA_SERVER_HTTPS_PORT", 0), 0),
    )
    rtmp_port = _safe_int(
        _node_value(selected_node, "rtmp_port"),
        _safe_int(getattr(settings, "MEDIA_SERVER_RTMP_PORT", 0), 0),
    )
    rtmps_port = _safe_int(
        _node_value(selected_node, "rtmps_port"),
        _safe_int(getattr(settings, "MEDIA_SERVER_RTMPS_PORT", 0), 0),
    )
    rtsp_port = _safe_int(
        _node_value(selected_node, "rtsp_port"),
        _safe_int(getattr(settings, "MEDIA_SERVER_RTSP_PORT", 0), 0),
    )
    rtsps_port = _safe_int(
        _node_value(selected_node, "rtsps_port"),
        _safe_int(getattr(settings, "MEDIA_SERVER_RTSPS_PORT", 0), 0),
    )
    ...
    return {
        ...
        "rtpProxyPort": _safe_int(_node_value(selected_node, "rtp_port"), _safe_int(getattr(settings, "MEDIA_SERVER_RTP_PROXY_PORT", 0), 0)) or None,
        ...
        "rtpPortRange": str(
            ...
            or getattr(settings, "MEDIA_SERVER_RTP_PROXY_PORT_RANGE", "")
            or ""
        ) or None,
...
def _build_play_success_response(
    *,
    ...
    if getattr(settings, 'TURN_SERVER', None):
        turn_username = getattr(settings, 'TURN_USERNAME', None)
        turn_password = getattr(settings, 'TURN_PASSWORD', None)
        if turn_username and turn_password:
            ...
            logger.warning("TURN_SERVER is configured but TURN_USERNAME/TURN_PASSWORD are not set; skipping TURN entry")  # FIXED: 移除默认凭证admin/admin123，未配置时不返回TURN
    ...
    return {
        "code": 0,
        "msg": "Success",  # FIXED: hardcoded Chinese → English
        "data": data,
        "app": app_name,
        "stream": stream_id,
        "codec": codec,
        ...
        "rtp_port_range": str(getattr(settings, "MEDIA_SERVER_RTP_PROXY_PORT_RANGE", "") or ""),
        "mediaServerId": media_server_id,
        "mediaInfo": media_info,
...
def _map_play_stream_error(exc: Exception) -> HTTPException:
    message = str(exc or "").strip() or "Play request failed"  # FIXED-P3: W-20 中文→英文
    ...
    if isinstance(exc, ZlmApiError):
        return _play_http_exception(
            ...
            str(getattr(exc, "user_message", message) or message),
            str(getattr(exc, "hint", "Check media node status, port config and runtime logs") or "Check media node status, port config and runtime logs"),  # FIXED-P3: W-20
            retryable=bool(getattr(exc, "retryable", True)),
            diagnostics={"operation": getattr(exc, "operation", ""), "raw": getattr(exc, "data", None)},
        )
    if "empty_secret" in lower_message:
        return _play_http_exception(
            502,
            "media_secret_invalid",
            "Media node auth secret missing",  # FIXED-P3: W-20
            "Check media node secret configuration",  # FIXED-P3: W-20
            retryable=False,
    ...
    if "no media node is ready" in lower_message:
        return _play_http_exception(
            ...
            "No available media node to receive stream",  # FIXED-P3: W-20
            "Check media node online status, port range and network connectivity",  # FIXED-P3: W-20
            retryable=True,
            diagnostics={"raw": message},
        )
    if "all connection attempts failed" in lower_message or "connection refused" in lower_message or "timeout" in lower_message:
        return _play_http_exception(
            ...
            "Media node connection failed",  # FIXED-P3: W-20
            "Check media node service status, network connectivity and firewall config",  # FIXED-P3: W-20
            retryable=True,
            diagnostics={"raw": message},
        )
    if "port" in lower_message and any(keyword in lower_message for keyword in ("exhaust", "used", "busy", "range", "occup", "占用", "耗尽", "不足")):
        return _play_http_exception(
            ...
            "Media node RTP port exhausted",  # FIXED-P3: W-20
            "Close unused streams, expand RTP port range, or add media nodes",  # FIXED-P3: W-20
            retryable=True,
            diagnostics={"raw": message},
        )
    if "openrtpserver" in lower_message:
        return _play_http_exception(
            503,
            "media_service_error",
            "Media node failed to create RTP server port",  # FIXED-P3: W-20
            "Check media node runtime status, port config and logs",  # FIXED-P3: W-20
            retryable=True,
            diagnostics={"raw": message},
        )
    return _play_http_exception(
        ...
        "play_request_failed",
        message,
        "Check device online status, media node status and playback params, then retry",  # FIXED-P3: W-20
        retryable=True,
        diagnostics={"raw": message},
    )
...
async def _build_full_play_response(
    ...
    if node_host and node_http_port:
        webrtc_supported, webrtc_hint = await _probe_webrtc_capability(node_host, int(node_http_port), app_name, stream_id)
    elif media_host and media_port:
        webrtc_supported, webrtc_hint = await _probe_webrtc_capability(media_host, int(media_port), app_name, stream_id)
    if not webrtc_supported and is_embedded_node:
        webrtc_hint = f"Built-in ZLM WebRTC not ready, check ZLM startup logs and RTC port config. {webrtc_hint}"  # FIXED-P3: i18n
    elif not webrtc_supported:
        webrtc_hint = f"Current media node may not have WebRTC enabled. {webrtc_hint}"  # FIXED-P3: i18n
    http_port = _safe_int(
        media_port,
        _safe_int(_node_value(selected_node, "public_http_port"), _safe_int(getattr(settings, "STREAM_PUBLIC_HTTP_PORT", 0), 0)),
    )
    https_port = _safe_int(_node_value(selected_node, "https_port"), _safe_int(getattr(settings, "MEDIA_SERVER_HTTPS_PORT", 0), 0))
    rtsp_port = _safe_int(_node_value(selected_node, "rtsp_port"), _safe_int(getattr(settings, "MEDIA_SERVER_RTSP_PORT", 0), 0))
    rtsps_port = _safe_int(_node_value(selected_node, "rtsps_port"), _safe_int(getattr(settings, "MEDIA_SERVER_RTSPS_PORT", 0), 0))
    rtmp_port = _safe_int(_node_value(selected_node, "rtmp_port"), _safe_int(getattr(settings, "MEDIA_SERVER_RTMP_PORT", 0), 0))
    rtmps_port = _safe_int(_node_value(selected_node, "rtmps_port"), _safe_int(getattr(settings, "MEDIA_SERVER_RTMPS_PORT", 0), 0))
