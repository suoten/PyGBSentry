...
def _parse_bootstrap_learning_state(raw_value: str | None) -> dict[str, Any]:
    ...
    for key, item in profiles.items():
        ...
        for mode_key in ("UDP", "TCP_PASSIVE", "TCP_ACTIVE"):
            ...
            clean_item[mode_key] = {"s": min(9999, s), "f": min(9999, f)}  # FIXED: 9999为SIP SN上限，与device_control保持一致
    ...
    *,
    ...
    for key, url in urls.items():
        if not url:
            ...
            continue
        if key in rtc_keys:
            # RTC 端点：POST 探测，返回非 404 即认为可用
            try:
                client = await _get_zlm_client()
                # FIXED: SDP o= line hardcoded 127.0.0.1 → use MEDIA_SERVER_HOST from settings
                _sdp_ip = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "")  # FIXED: I3 回退值不再硬编码127.0.0.1
                placeholder_sdp = (
                    "v=0\r\n"
                ...
                r = await client.post(
                    url,
                    params={"app": app, "stream": stream, "type": "play"},
                    content=placeholder_sdp.encode("utf-8"),
                    headers={"Content-Type": "text/plain;charset=utf-8"},
                    timeout=timeout,
                )
                validated[key] = r.status_code != 404
            except Exception:
                validated[key] = False
            continue
...
async def _probe_zlm_stream(
    ...
    *,
    ...
    try:
        ...
        client = await _get_zlm_client()
        r = await client.get(url, params={"secret": sec}, timeout=3.0)
...
async def _wait_zlm_stream_ready(
    ...
    *,
    ...
    _is_rtp_stream = (str(app or "").lower() == "rtp")
...
async def _probe_webrtc_capability(node_host: str, node_http_port: int, app: str, stream: str) -> tuple[bool, str]:
    """
    探测节点 WebRTC 播放能力：
    ...
    try:
        ...
        # 只要不是 404，就认为 WebRTC 接口存在（可能因 SDP 不完整返回 400/其它错误）
        if r.status_code >= 500:
            return False, f"WebRTC probe failed: server error (HTTP {r.status_code})"  # FIXED: 硬编码中文→英文
        return True, ""
    except Exception as e:
...
async def _get_gb28181_play_config(db: AsyncSession) -> dict:
    ...
    values = {item.setting_key: item.setting_value for item in result.scalars().all()}
    return {
        "ssrc_policy": _normalize_ssrc_policy(values.get("gb28181.ssrc_policy") or getattr(settings, "GB28181_SSRC_POLICY", "adaptive")),
        "ssrc_retry_on_not_ready": (
            ...
            else bool(getattr(settings, "GB28181_SSRC_RETRY_ON_NOT_READY", True))
        ),
        "ssrc_retry_order": _parse_ssrc_retry_order(values.get("gb28181.ssrc_retry_order") or getattr(settings, "GB28181_SSRC_RETRY_ORDER", "strict,off")),
        "auto_ensure_embedded_media_node": (
            ...
            else bool(getattr(settings, "GB28181_AUTO_ENSURE_EMBEDDED_MEDIA_NODE", True))
        ),
    }
...
def _public_stream_scheme() -> str:
    scheme = str(getattr(settings, "STREAM_PUBLIC_SCHEME", "") or "").strip().lower()
...
async def _find_zlm_media_item(node_host: str, node_http_port: int, secret: str, app: str, stream: str) -> dict | None:
    try:
        ...
        url = f"http://{node_host}:{int(node_http_port)}/index/api/getMediaList"
        client = await _get_zlm_client()
        r = await client.get(url, params={"secret": sec}, timeout=2.0)
        if r.status_code >= 400:
            return None
        data = r.json()
        if data.get("code") not in (0, "0"):
