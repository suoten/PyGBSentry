...
# FIXED-P3: W-12 markers for port exhaustion detection (SIP module may output either form)
...
async def stream_error_catalog(
    ...
    items = [
        {"reason_code": "media_secret_invalid", "http_status": 502, "retryable": False, "message": "Media node auth config missing"},  # FIXED: hardcoded Chinese → English
        {"reason_code": "media_node_unavailable", "http_status": 503, "retryable": True, "message": "No available media node to receive video stream"},  # FIXED: hardcoded Chinese → English
        {"reason_code": "media_node_unreachable", "http_status": 503, "retryable": True, "message": "Media node connection failed"},  # FIXED: hardcoded Chinese → English
        {"reason_code": "stream_not_ready", "http_status": 504, "retryable": True, "message": "Stream not ready"},  # FIXED: hardcoded Chinese → English
        ...
        {"reason_code": "max_concurrent_streams", "http_status": 429, "retryable": True, "message": "Concurrent stream limit reached"},  # FIXED: hardcoded Chinese → English
        {"reason_code": "play_request_failed", "http_status": 502, "retryable": True, "message": "Stream invite request failed"},  # FIXED: hardcoded Chinese → English
    ]
    data = []
    for item in items:
            ...
            if zlm_probe_ok and zlm_stream_ready:
                ...
                if matched_app == "live":
                    ...
                    def _take_background_snapshot():
                        time.sleep(3.5)
                        from app.api.v1.endpoints.devices.devices_control import _try_snap_async, _snapshot_cache_file  # FIXED-P0: C-09 _try_snap_sync不存在，改用_try_snap_async
                        ...
                        try:  # FIXED-P0: C-09 在后台线程中创建新事件循环调用async函数
                ...
                _flv_log = logging.getLogger("stream.warmup")
...
class PlayStatusRequest(BaseModel):  # FIXED-P0: S-28 用Pydantic模型替代裸dict，防止非字符串session_id导致AttributeError
    session_id: str


@router.post("/play_status")  # FIXED-P2: W-10 新增POST端点，session_id从请求体获取，防止URL泄露
...
async def _play_status_inner(session_id: str, db: AsyncSession, current_user: User):
    ...
    session_record = (await db.execute(select(StreamSession).where(StreamSession.id == session_id, StreamSession.tenant_id == (current_user.tenant_id or "default")))).scalars().first()  # FIXED-P2: M-08 统一租户隔离模式
    if not session_record:
        ...
        if reason_code == "device_transport_unavailable":
            raise _play_http_exception(
                ...
                "Please verify the device is online, registered, and SIP transport is available",  # FIXED-P2: C-28
                retryable=True,
        ...
        if reason_code == "invite_send_failed":
            raise _play_http_exception(
                503,
                "invite_send_failed",
                "Invite send failed",  # FIXED-P2: C-28 硬编码中文→英文
        ...
        if reason_code == "media_port_exhausted":
            raise _play_http_exception(
                ...
                "Media node RTP port exhausted",  # FIXED-P2: C-28 硬编码中文→英文
        ...
        raise _play_http_exception(
            ...
            "Stream session ended or stream not ready (timeout)",  # FIXED-P2: C-28 硬编码中文→英文
    ...
    if not getattr(session_record, "media_server_id", None):
        return JSONResponse(
            ...
            content={
                "code": 202,
                "msg": "Stream starting, please continue polling",  # FIXED: hardcoded Chinese → English
                "data": {
                    "status": "starting",
    ...
    if not zlm_stream_ready:
        return JSONResponse(
            ...
            content={
                "code": 202,
                "msg": "Stream not ready, please continue polling",  # FIXED-P2: C-28 中英混合→英文
                "data": {
                    "status": "waiting",
...
async def play_stream(
    ...
    try:
        ...
        if not asset.ip_addr:
            ...
            raise HTTPException(status_code=500, detail="Device network information missing")  # FIXED: 中文→英文
        ...
        if max_streams > 0:
            ...
            if cnt >= max_streams:
                ...
                raise HTTPException(status_code=429, detail=f"Concurrent stream limit reached ({max_streams}), please close some streams first")  # FIXED: 中文→英文
        ...
        if async_mode:
            ...
            if existing_ss:
                ...
                return JSONResponse(
                    ...
                    content={
                        "code": 202,
                        "msg": "Stream invite sent, please poll status",  # FIXED: hardcoded Chinese → English
                        "data": {
            ...
            placeholder = StreamSession(
                ...
                resource_id=placeholder_resource_id,
                asset_id=placeholder_asset_id,
                tenant_id=current_user.tenant_id,  # FIXED-P2: S-04 添加租户隔离
                protocol="UDP",
            )
            db.add(placeholder)
            await db.commit()
        ...
        for attempt in attempts:
            ...
            if not selected_node:
                raise HTTPException(status_code=500, detail="Media node information missing")  # FIXED: 中文→英文
...
async def switch_stream_type(
    ...
    if target_type not in ["main", "sub"]:
        raise HTTPException(status_code=400, detail="target_type must be 'main' or 'sub'")  # FIXED: 中文→英文

    stmt = select(StreamSession).where(StreamSession.stream == stream_id, StreamSession.app == "live", StreamSession.tenant_id == (current_user.tenant_id or "default"))  # FIXED-P2: M-08 统一租户隔离模式
    result = await db.execute(stmt)
    ss = result.scalars().first()
    if not ss:
        raise HTTPException(status_code=404, detail="Stream session not found")  # FIXED: 中文→英文
        
    try:
        success = await sip_invite_module.sip_invite.send_stream_switch_reinvite(ss, target_type)
        if not success:
            raise HTTPException(status_code=500, detail="Stream switch signal send failed")  # FIXED: 中文→英文
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Stream switch error: {e}")  # FIXED: 中文→英文
    ...
    await _stream_audit(
        ...
        result="success",
        status_code=200,
        detail=f"Re-INVITE sent to switch to {target_type} stream",  # FIXED: 中文→英文
        extra_summary=f"stream_id={stream_id}; target={target_type}"
    )
    
    return {"code": 0, "msg": f"Stream switch command sent for {target_type} stream"}  # FIXED: hardcoded Chinese → English
...
async def stop_stream(
    ...
    stmt = select(StreamSession).where(StreamSession.tenant_id == (current_user.tenant_id or "default"))  # FIXED-P2: M-08 统一租户隔离模式
    if payload.app and payload.stream:
        ...
        raise HTTPException(status_code=400, detail="Missing stream identifier")  # FIXED: 中文→英文
    result = await db.execute(stmt)
