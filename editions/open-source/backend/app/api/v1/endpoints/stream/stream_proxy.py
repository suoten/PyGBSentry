...
async def get_push_token(
    stream_name: str,
    expire_seconds: int = Query(300, ge=60, le=86400),  # FIXED
...
async def broadcast_start(
    ...
    if media_node:
        ...
        if payload.audio_file_url:
            # FIXED-P2: N-05 防止FFmpeg协议注入 — 仅允许http/https协议
            ...
            # FIXED: hardcoded fallback IP → use settings explicitly
            _fallback_host = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "")  # FIXED: I3 回退值不再硬编码127.0.0.1
            src_url = f"http://{media_node.host or _fallback_host}:{media_node.http_port or 80}/broadcast/{payload.channel_id}/hls.m3u8"

        # FIXED: hardcoded fallback IP and RTMP port → use settings explicitly
        _fallback_host = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "")  # FIXED: I3 回退值不再硬编码127.0.0.1
...
async def broadcast_stop(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_superuser),  # FIXED-P2: N-03 broadcast_stop权限升级为superuser，与broadcast_start一致
):
    """停止语音广播"""
    from app.sip.broadcast import broadcast
...
async def talk_start(
    ...
    try:
        ...
        zlm_http_port = 80
        zlm_rtp_port = 0  # FIXED: 对讲SDP需使用RTP端口
        ...
        if media_node:
            ...
            zlm_rtp_port = getattr(media_node, 'rtp_proxy_port', 0) or 0  # FIXED: 获取RTP端口

        # FIXED: 优先使用 SipTalk 双向对讲（a=sendrecv + ZLM WHIP 音频上行）
        ...
        if sip_talk:
            talk_result = await sip_talk.send_talk_invite(
                ...
                zlm_rtp_port=zlm_rtp_port,  # FIXED: 传入RTP端口
            )
        ...
        if payload.sdp_body:
            ...
            # FIXED: m=audio 端口应使用 RTP 端口而非 SIP 信令端口
