...
async def playback_stream(
    ...
    if max_streams > 0:
        ...
        if cnt >= max_streams:
            ...
            raise HTTPException(status_code=429, detail=f"Concurrent stream limit reached ({max_streams}), please close some playback sessions and retry")  # FIXED: 硬编码中文→英文
    ...
    if not signal_targets:
        ...
        raise _play_http_exception(
            ...
            "Please verify the device is online, registered, and the SIP transport channel is available",  # FIXED-P3: W-14 hardcoded Chinese→English
            retryable=True,
        )
    ...
    if not sip_invite_module.sip_invite:
        ...
        raise _play_http_exception(
            ...
            "Please check if the platform SIP service has started",  # FIXED-P3: W-14 hardcoded Chinese→English
            retryable=True,
        )
    ...
    stream_id = result.get("stream", "")  # FIXED: 使用.get()防止KeyError
    app_name = result.get("app", "live")  # FIXED: 使用.get()防止KeyError
...
async def download_stream(
    ...
    return {
        "app": app_name,
        "stream": stream_id,
        "msg": "Download command sent, stream is transmitting"  # FIXED: hardcoded Chinese → English
    }
...
async def control_playback_stream(
    ...
    if not row:
        ...
        raise HTTPException(status_code=404, detail="Playback stream session not found or permission denied")  # FIXED: hardcoded Chinese → English
...
async def playback_pause(
    ...
    if getattr(session, "start_time", None) and payload.seek_time is not None:
        ...
        if cached_record and cached_record.file_path:
            ...
            # FIXED: hardcoded 127.0.0.1 and 554 → use settings with explicit variable
            _media_host = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "")  # FIXED: I3 回退值不再硬编码127.0.0.1
            _rtsp_port = settings.STREAM_PUBLIC_RTSP_PORT or 554
            # FIXED-P2: S-02 防止FFmpeg参数注入 — 校验 app/stream 仅含安全字符
...
async def playback_speed(
    ...
    if payload.speed not in valid_speeds:
        ...
        raise HTTPException(status_code=400, detail=f"Invalid speed value, supported speeds: {valid_speeds}")  # FIXED: hardcoded Chinese → English
    ...
    await playback_control.send_speed(
        asset, resource.gb_id,
        ((asset.ip_addr, asset.port), asset.transport, transport),
        session.call_id,
        speed=payload.speed,
