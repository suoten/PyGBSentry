...
async def list_streams(
    limit: int = Query(100, ge=1, le=500, description="每页最大条数"),  # FIXED: 全量加载 → 添加分页限制
    offset: int = Query(0, ge=0, description="偏移量"),  # FIXED: 全量加载 → 添加分页偏移
...
async def get_webrtc_url(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user)  # FIXED: WebRTC端点缺少认证保护
):
    """
    ...
    if transcode:
        ...
        if not ffmpeg_proxy_manager.is_running(target_stream_id):
            # FIXED-P2: S-03 防止FFmpeg参数注入 — 校验 app_name/stream_id 仅含安全字符
            ...
            # 原始流地址 (ZLM 本地 RTSP)
            _media_host = getattr(settings, 'MEDIA_SERVER_HOST', '') or ''  # FIXED: I3 回退值不再硬编码127.0.0.1
    ...
    # FIXED: 移除TURN默认密码admin123，仅在配置了TURN服务器且设置了凭证时才返回
