...
async def websocket_talk(websocket: WebSocket, device_id: str, token: str = Query(default=""), db: AsyncSession = Depends(get_db)):
    # FIXED-P0: C-02 WebSocket auth — verify JWT before accepting connection
    if not token:
        await websocket.close(code=4401, reason="Authentication required")
        return
    try:
        jwt_payload = _jwt.decode(token, settings.SECRET_KEY, algorithms=["HS256"], audience="pygbsentry:access")
    except Exception:
        await websocket.close(code=4401, reason="Invalid or expired token")
        return

    # FIXED-P2: N-04 添加租户隔离 — 从JWT获取user_id，查询用户tenant_id
    ...
    # FIXED-P2: M-07 检查sip_server就绪状态
    ...
    if asset.transport == "UDP":
        ...
        transport = sip_server.tcp_server  # FIXED-P2: M-06 TCP传输时保留transport="TCP"而非设为None
...
# FIXED: 双向对讲 WebSocket 端点 — 接入 send_talk_invite 实现双向音频
@router.websocket("/talk/bidirectional/{device_id}/{channel_id}")
async def websocket_bidirectional_talk(
    websocket: WebSocket,
    device_id: str,
    channel_id: str,
    token: str = Query(default=""),  # FIXED-P1: C-14 双向对讲WebSocket添加JWT鉴权
    ...
    """
    # FIXED-P1: C-14 验证JWT token，防止未授权访问
    if not token:
        await websocket.close(code=4401, reason="Authentication required")
        return
    try:
