from fastapi import Query, APIRouter, HTTPException, Depends, Body  # FIXED-P0: S-01 添加Body导入
...
# FIXED-P2: N-09 添加PTZ命令限速，防止淹没设备
...
async def _get_asset_resource(db: AsyncSession, device_id: str, channel_id: str, current_user: User):
    ...
    result = await db.execute(stmt)
    resource = get_or_404(result, detail="Resource not found")  # FIXED: ORM查询结果空值判断
...
async def query_preset(
    ...
    # FIXED-P4: 支持向设备发送PresetQuery命令查询预置位，与wvp对齐
    if remote:
        if not asset.ip_addr:
...
class DragZoomRequest(BaseModel):
    """框选缩放请求"""  # FIXED: GB28181 DragZoom框选缩放 — API请求模型
...
async def device_guard(
    ...
    action = "SetGuard" if body.guard_cmd == "SetGuard" else "ResetGuard"  # FIXED-P3: W-19 中文→英文
    await _control_audit(
        db,
        current_user,
        action="device_guard",
...
async def device_teleboot(
    ...
    return {"status": "ok", "action": "remote reboot", "device_id": device_id}  # FIXED: 硬编码中文→英文
...
async def device_record_control(
    ...
    action = "StartRecord" if body.record_cmd == "Record" else "StopRecord"  # FIXED-P3: W-19 中文→英文
    await _control_audit(
        db,
        current_user,
        action="device_record_control",
...
async def device_alarm_reset(
    ...
    return {"status": "ok", "action": "alarm reset"}  # FIXED: 硬编码中文→英文
...
async def device_drag_zoom(
    ...
    # FIXED: GB28181 DragZoom框选缩放 — API端点
    from app.sip.device_control import device_control
    ...
    action = "DragZoomIn" if body.zoom_cmd == "DragZoomIn" else "DragZoomOut"  # FIXED-P3: W-19 中文→英文
    await _control_audit(
        db,
        current_user,
        action="device_drag_zoom",
...
async def device_iframe_request(
    ...
    return {"status": "ok", "action": "force keyframe"}  # FIXED: 硬编码中文→英文
...
async def device_home_position(
    ...
    if not sip_server:  # FIXED-P0: S-01 直接使用模块级sip_server，移除未定义的_get_sip_server()
...
async def control_iris(
    ...
    action_map = {"in": "IrisOpen", "out": "IrisClose", "stop": "IrisStop"}  # FIXED-P3: W-19 中文→英文
    await _control_audit(
        db,
        current_user,
        action="control_iris",
...
async def control_focus(
    ...
    action_map = {"near": "FocusNear", "far": "FocusFar", "stop": "FocusStop"}  # FIXED-P3: W-19 中文→英文
    await _control_audit(
        db,
        current_user,
        action="control_focus",
...
async def control_cruise(
    ...
    if body.action not in valid_actions:
        ...
        raise HTTPException(status_code=400, detail=f"action must be one of {valid_actions}")  # FIXED: hardcoded Chinese → English
    ...
    if body.action in ["add", "delete"] and not (1 <= body.preset_id <= 255):
        ...
        raise HTTPException(status_code=400, detail="preset_id must be between 1-255 for add/delete operations")  # FIXED: 硬编码中文→英文
    ...
    action_map = {  # FIXED-P3: W-19 中文→英文
        "add": "AddCruisePoint",
...
async def control_scan(
    ...
    if body.action not in valid_actions:
        ...
        raise HTTPException(status_code=400, detail=f"action must be one of {valid_actions}")  # FIXED: 硬编码中文→英文
...
async def get_control_state(
    device_id: str,
    channel_id: str,
    cruise_id: int = Query(1, ge=1),  # FIXED
    scan_id: int = Query(0, ge=0),  # FIXED
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
):
...
async def control_wiper(
    ...
    action_map = {"on": "WiperOn", "off": "WiperOff", "stop": "WiperStop"}  # FIXED-P3: W-13 中文action→英文
    await _control_audit(
        db,
        current_user,
        action="control_wiper",
...
async def control_aux_switch(
    ...
    action = "AuxSwitchOn" if body.command == "on" else "AuxSwitchOff"  # FIXED-P3: W-13 中文action→英文
    await _control_audit(
        db,
        current_user,
        action="control_aux_switch",
