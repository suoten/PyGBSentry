...
import os
import shutil
import httpx  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
async def upload_plugin(
    ...
    if not file.filename.endswith(".zip"):
        raise HTTPException(status_code=400, detail="Only .zip format is supported for plugin packages")  # FIXED: 中文→英文
    # FIXED: temp_path 缩进从 raise 后移到正确位置，避免 finally 中 NameError
    ...
    try:
        ...
        result = await _install_plugin_from_zip(temp_path, current_user.tenant_id or "default")  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
        ...
        if op_for_audit == "upgrade":
            ...
            if hook_failed:
                ...
                if bool(getattr(settings, "PLUGIN_UPGRADE_HOOK_STRICT", False)):
                    upgrade_hook_report["strict_blocked"] = True
                    raise HTTPException(
                        status_code=500,
                        detail={
                            "message": "Upgrade migration callback failed (strict mode blocked)",  # FIXED: hardcoded Chinese → English
                            "strict_blocked": True,
        ...
        await _recover_plugins_after_failed_upgrade(upgrade_snapshot)
        try:
            await plugin_manager.emit(HOOK_ON_STARTUP)  # FIXED: 安装失败后重新启动所有插件，避免全部停止
        except Exception:
        ...
        try:
            ...
            logger.warning("Failed to restart plugins after install failure")
        raise HTTPException(status_code=500, detail=f"Installation failed: {str(e)}")  # FIXED: 中文→英文
    finally:
        _cleanup_plugin_upgrade_snapshot(upgrade_snapshot or {})
...
async def install_from_marketplace(
    ...
    if payload.package_url:
        if not _is_allowed_package_url(payload.package_url):
            raise HTTPException(status_code=400, detail="Package URL domain is not allowed")  # FIXED: 中文→英文
        plugin_payload = {"id": payload.plugin_id, "package_url": payload.package_url}
    else:
        plugin_payload = await _resolve_marketplace_install_payload(payload.plugin_id)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    await _precheck_paid_plugin_install(plugin_payload or {}, _authorization_for_server_proxy(request))  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    if plugin_payload:
    ...
    try:
        # 运行时安装/升级：先停机，避免旧插件后台任务在 load_plugins 后残留/重复
        await plugin_manager.emit(HOOK_ON_SHUTDOWN)
        # FIXED: 同步requests.get→异步httpx，避免阻塞事件循环30秒
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.get(package_url)
        if response.status_code >= 400:
            raise HTTPException(status_code=502, detail=f"Download failed: {response.status_code}")  # FIXED: 中文→英文
...
async def uninstall_plugin_preview(
    ...
    if not os.path.isdir(target_dir) and not os.path.exists(target_file):
        raise HTTPException(status_code=404, detail="Plugin not installed")  # FIXED: 中文→英文
...
async def uninstall_plugin(
    ...
    if not bool(confirm):
        raise HTTPException(status_code=400, detail=f"Uninstall is a high-risk operation, please confirm first (confirm=true, confirmation phrase: {expected_phrase})")  # FIXED: 中文→英文
    if str(confirm_phrase or "").strip() != expected_phrase:
        raise HTTPException(status_code=400, detail=f"Confirmation phrase is incorrect, please enter: {expected_phrase}")  # FIXED: 中文→英文
    ...
    if not data_policy:
        data_policy = str(getattr(settings, "PLUGIN_UNINSTALL_DEFAULT_DATA_POLICY", "cascade_delete") or "cascade_delete").strip().lower()
    ...
    if data_policy == "ask" and not getattr(request.query_params, "preserve_data", None):
        ...
        if preserve_data_param not in ("true", "false"):
            raise HTTPException(
                status_code=400,
                detail="Plugin uninstall data policy is 'ask', please explicitly choose preserve_data=true/false",  # FIXED: 中文→英文
            )
    ...
    try:
        ...
        # FIXED: 卸载前仅触发目标插件的停机钩子，避免影响其他正在运行的插件
        ...
        # FIXED: 卸载后仅对仍存在的插件重新触发启动钩子（load_plugins 已重新注册所有存活插件）
        ...
        # FIXED-P2: W-11 MySQL DDL隐式提交，rollback无法回滚DROP TABLE；改为逐表执行+记录结果
        if data_policy == "cascade_delete":
            ...
            logger.info(f"Plugin {plugin_id} uninstall data policy is {data_policy}, preserving DB tables: {tables_to_drop}")  # FIXED: 硬编码中文→英文
        try:
            await db.commit()
        except Exception:
            await db.rollback()
