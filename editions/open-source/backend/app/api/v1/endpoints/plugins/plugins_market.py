...
class PurchaseConfirmPayload(BaseModel):
    order_id: str
    # FIXED: dict→Pydantic schema，支付确认接口无类型校验可注入任意字段
...
async def marketplace_proxy(
    ...
    if (settings.APP_EDITION or "oss").lower() == "server":
        return _read_marketplace_catalog()
    return await _fetch_marketplace_items()  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
async def marketplace_update_summary(
    ...
    if (settings.APP_EDITION or "oss").lower() == "server":
        ...
        market_items = await _fetch_marketplace_items()  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    by_id = {
        str(x.get("id")): x
...
async def get_marketplace_shop_url(
    ...
    shop = (getattr(settings, "PLUGIN_MARKETPLACE_SHOP_URL", None) or "").strip()
...
async def embedded_purchase(
    ...
    try:
        resp = await (await get_http_client()).post(create_url, json=order_body, headers=headers, timeout=15)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
async def embedded_purchase_confirm(
    ...
    try:
        resp = await (await get_http_client()).post(confirm_url, json=payload.model_dump(), headers=headers, timeout=15)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"Cannot connect to plugin marketplace：{exc}")
    if resp.status_code >= 400:
        try:
