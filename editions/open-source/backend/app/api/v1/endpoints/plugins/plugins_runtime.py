...
async def export_plugin_table_csv(
    ...
    async with engine.connect() as conn:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Export query failed: {e}")  # FIXED: hardcoded Chinese → English
...
async def face_db_upload(
    ...
    try:
        ...
        if (not ids) and (not force):
            raise HTTPException(status_code=400, detail="Face database empty or no valid embeddings (use force=true to force overwrite)")  # FIXED: hardcoded Chinese → English
    except HTTPException:
        raise
    except Exception as e:
        if not force:
    ...
    if not str(target).startswith(str(base)):
        raise HTTPException(status_code=400, detail="invalid path")
    # FIXED: 防止磁盘满/权限不足时500无提示
    try:
        with open(target, "wb") as f:
            f.write(content)
    except OSError as e:
        raise HTTPException(status_code=500, detail=f"Face database file write failed: {e}")  # FIXED: hardcoded Chinese → English
...
async def stream_health_health(
    ...
    low_bitrate_bytes: int = Query(1024, ge=0),  # FIXED
    min_readers: int = Query(1, ge=0),  # FIXED
    limit: int = Query(500, ge=1, le=10000),  # FIXED
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
    _: None = Depends(require_oss_paid_runtime_from_path),
):
...
async def plugin_health_status(
    ...
    """
    # FIXED: S-02 plugin_manager.list_installed() 不存在，改用 plugins 字典
    ...
    for pid_str in installed_ids:
        ...
        if "stream_health" in pid_lower or "health" in pid_lower:
            try:
                ...
                if not is_healthy:
                    last_error = "No available media node"  # FIXED: A-07 中文→英文
                    error_count = 1
            except Exception:
                is_healthy = False
                last_error = "Media node query failed"  # FIXED: A-07 中文→英文
                error_count = 1
        items.append({
            "plugin_id": pid_str,
            "healthy": is_healthy,
...
async def plugin_security_report(
    ...
    """
    # FIXED: S-06 run synchronous os.walk + file scan in thread to avoid blocking event loop
    installed_ids = list(plugin_manager.plugins.keys())
    if plugin_id:
        installed_ids = [pid for pid in installed_ids if pid == plugin_id]
