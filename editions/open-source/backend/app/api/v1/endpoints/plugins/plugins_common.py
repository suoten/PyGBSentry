...
async def _fetch_purchased_plugin_ids_uncached(request: Request, current_user: User) -> frozenset[str]:
    ...
    try:
        ...
        resp = await (await get_http_client()).get(url, headers=headers, timeout=8)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
async def get_purchased_plugin_ids_cached(request: Request, current_user: User) -> frozenset[str]:
    ttl = int(getattr(settings, "PLUGIN_PURCHASED_PROXY_CACHE_SECONDS", 45) or 0)
    if ttl <= 0:
...
def _resolve_paid_runtime_auth_mode() -> str:
    ...
    """
    raw = str(getattr(settings, "PLUGIN_PAID_RUNTIME_AUTH_MODE", "compat") or "compat").strip().lower()
    valid = {"compat", "cache_only", "online_strict", "online_fail_open", "online_prefer_cache"}
    if raw in valid and raw != "compat":
        return raw

    # compat：沿用历史字段语义
    runtime_online = bool(getattr(settings, "PLUGIN_PAID_RUNTIME_INSTALL_CHECK", False))
    strict = bool(getattr(settings, "PLUGIN_PAID_INSTALL_CHECK_STRICT", True))
...
def _runtime_online_check_cache_ttl() -> int:
    return max(0, int(getattr(settings, "PLUGIN_PAID_RUNTIME_ONLINE_CHECK_CACHE_SECONDS", 15) or 0))
...
async def _check_paid_plugin_entitlement_online_once(
    ...
    *,
    ...
    if not base:
        return (False, 503, "Server edition address not configured, cannot verify authorization online")  # FIXED: 硬编码中文→英文
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Failed to build plugin request headers: {e}")  # FIXED: 硬编码中文→英文
        ...
        if resp.status_code == 401:
            return (False, 401, "Server-side login verification failed, please login again")  # FIXED: 硬编码中文→英文
        ...
        if 200 <= resp.status_code < 300:
            if strict_response_body:
                try:
                    ...
                    logger.debug(f"Operation failed, returning default value: {e}")  # FIXED: 硬编码中文→英文
                    body = {}
                if not (isinstance(body, dict) and body.get("ok")):
                    return (False, 502, "Server edition install-check returned abnormal, please retry later or contact admin")  # FIXED: 硬编码中文→英文
            return (True, 200, "OK")
        ...
        return (False, 502, msg or f"install-check failed (HTTP {resp.status_code})")  # FIXED: 硬编码中文→英文
    except httpx.TimeoutException:
        return (False, 504, "install-check request timeout, check network to server edition and retry")  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except httpx.HTTPError as e:
        return (False, 502, f"Cannot connect to server edition for install-check: {e}")  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except Exception as e:
        return (False, 502, f"install-check exception: {e}")  # FIXED: 硬编码中文→英文
...
def _entitlement_error_detail(code: str, *, message: str | None = None) -> dict:
    ...
    if normalized == "SUBSCRIPTION_EXPIRED":
        return {
            ...
            "message": msg or "Current tenant subscription has expired, cannot install/use paid plugins",  # FIXED: hardcoded Chinese → English
            "suggestion": "Please renew/restore subscription in server edition user portal first, then retry in OSS edition",  # FIXED: 硬编码中文→英文
            "retryable": False,
        }
    if normalized == "PLUGIN_NOT_PURCHASED":
        return {
            "reason_code": "PLUGIN_NOT_PURCHASED",
            "message": msg or "Paid plugin not purchased or tenant not authorized",  # FIXED: hardcoded Chinese → English
            "suggestion": "Please complete purchase/renewal in server edition user portal first, then install in OSS edition",  # FIXED: 硬编码中文→英文
            "retryable": False,
        }
    return {
        "reason_code": normalized,
        "message": msg or "Paid plugin authorization verification failed",  # FIXED: hardcoded Chinese → English
        "suggestion": "Please retry later, or contact admin to check server edition authorization status",  # FIXED: 硬编码中文→英文
        "retryable": False,
    }
...
def _parse_face_db_identities(raw: dict) -> list[dict]:
    ...
    for it in ids:
        ...
        try:
            ...
            logger.debug(f"Skipped: {e}")  # FIXED: 硬编码中文→英文
            continue
        out.append(
            {
...
def _validate_plugin_menu_conflicts(metadata: dict, plugin_id: str) -> None:
    ...
    安装/上传时校验菜单路由冲突。
    """
    required_prefix = str(getattr(settings, "PLUGIN_MENU_PATH_REQUIRED_PREFIX", "/plugins/") or "/plugins/")
    ...
    if new_frontend_urls:
        for other_pid, other_meta in plugin_manager.metadata.items():
            ...
            if conflict_fu:
                raise HTTPException(
                    status_code=400,
                    detail=f"menu.frontend_url conflicts with installed plugin: {sorted(list(conflict_fu))[:3]}",  # FIXED: hardcoded Chinese → English
                )
...
async def _ensure_plugin_dependencies_installed(plugin_id: str, target_dir: str, metadata: dict, file_list: list[str]) -> None:
    if not bool(getattr(settings, "PLUGIN_AUTO_INSTALL_DEPENDENCIES", True)):
        return
    vendor_dir = os.path.join(target_dir, str(getattr(settings, "PLUGIN_DEPENDENCY_VENDOR_DIR_NAME", ".vendor") or ".vendor"))
    timeout_seconds = int(getattr(settings, "PLUGIN_DEPENDENCY_INSTALL_TIMEOUT_SECONDS", 300) or 300)
    ...
    for it in to_install:
        await _pip_install_into_vendor(vendor_dir, it["pip"], timeout_seconds=timeout_seconds)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环

def _plugin_upgrade_backup_root() -> FSPath:
    raw = str(getattr(settings, "PLUGIN_UPGRADE_BACKUP_DIR", "") or "").strip()
    if raw:
        p = FSPath(raw)
...
def _create_plugin_upgrade_snapshot(plugin_id: str) -> dict:
    ...
    enabled = bool(getattr(settings, "PLUGIN_UPGRADE_BACKUP_ENABLED", True))
    snap = {
        "plugin_id": pid,
        "enabled": enabled,
...
def _plan_install_or_upgrade_from_zip(
    ...
    *,
    ...
    if not re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_-]{0,63}", pid):
        raise HTTPException(status_code=400, detail="Invalid plugin id (alphanumeric, -, _, max 64 chars)")  # FIXED: hardcoded Chinese → English

    exp = str(expected_plugin_id or "").strip()
    if exp and exp != pid:
        raise HTTPException(status_code=400, detail=f"Plugin package id({pid}) does not match target plugin({exp}), installation aborted")  # FIXED: hardcoded Chinese → English

    cur_version = str((plugin_manager.metadata.get(pid, {}) or {}).get("version") or "").strip()
    new_version = str(meta.get("version") or "").strip()
    allow_downgrade = bool(getattr(settings, "PLUGIN_UPGRADE_ALLOW_DOWNGRADE", False))
    if cur_version and new_version and _is_version_lt(new_version, cur_version) and not allow_downgrade:
        raise HTTPException(
            status_code=400,
            detail=f"Downgrade detected (current {cur_version} -> new {new_version}), blocked by default. Set PLUGIN_UPGRADE_ALLOW_DOWNGRADE=true to allow.",  # FIXED: hardcoded Chinese → English
        )
...
async def _precheck_paid_plugin_install(plugin_payload: dict, authorization: str | None) -> None:
    ...
    if str(plugin_payload.get("type") or "").lower() != "paid":
        return
    strict = bool(getattr(settings, "PLUGIN_PAID_INSTALL_CHECK_STRICT", True))
    ok, status_code, detail = await _check_paid_plugin_entitlement_online_once(  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
...
def _normalize_marketplace_base() -> str:
    return (settings.PLUGIN_MARKETPLACE_BASE_URL or getattr(settings, "PLUGIN_MARKETPLACE_BASE_URL", "")).rstrip("/")
...
async def _fetch_marketplace_items() -> list[dict]:
    ...
    try:
        response = await (await get_http_client()).get(url, timeout=10)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except Exception as e:
...
async def _resolve_marketplace_install_payload(plugin_id: str) -> dict:
    items = await _fetch_marketplace_items()  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    for item in items:
...
def _ensure_oss_compatible(plugin_payload: dict) -> None:
    ...
    if status == "deprecated":
        raise HTTPException(status_code=400, detail="This plugin has been deprecated, new installations are not allowed")  # FIXED: hardcoded Chinese → English
    min_oss = str(plugin_payload.get("min_oss_version") or "").strip() or None
    if min_oss:
        current = getattr(settings, "PROJECT_VERSION", "1.0.0")
        if _is_version_lt(current, min_oss):
            raise HTTPException(status_code=400, detail=f"Current open-source version {current} is too low, plugin requires >= {min_oss}. Please upgrade first.")  # FIXED: hardcoded Chinese → English
    max_oss = str(plugin_payload.get("max_oss_version") or "").strip() or None
    if max_oss:
        current = getattr(settings, "PROJECT_VERSION", "1.0.0")
        if not _is_version_lt(current, max_oss):
            raise HTTPException(status_code=400, detail=f"Current open-source version {current} is too high, plugin only supports <= {max_oss}. Contact the plugin developer.")  # FIXED: hardcoded Chinese → English
...
async def _notify_server_plugin_event(
    ...
    url = getattr(settings, "PLUGIN_SERVER_RECORD_URL", None) or ""
    ...
    try:
        ...
        resp = await (await get_http_client()).post(url.strip(), json=body, headers=headers, timeout=10)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
        if resp.status_code == 401:
            raise HTTPException(status_code=401, detail="Server-side login verification failed, please log in again")  # FIXED: hardcoded Chinese → English
        if resp.status_code == 402:
            raise HTTPException(status_code=402, detail=_entitlement_error_detail("SUBSCRIPTION_EXPIRED"))
        if resp.status_code == 403:
            detail403 = "PLUGIN_NOT_PURCHASED"
        ...
        if resp.status_code >= 400:
            raise HTTPException(
                status_code=502,
                detail=f"Server record failed: {resp.status_code}",  # FIXED: hardcoded Chinese → English
            )
    except HTTPException:
        raise
    except Exception as e:
        ...
        if pid:
            ...
            if bad:
                raise HTTPException(
                    status_code=400,
                    detail=(
                        f"Plugin declared table names must have prefix {need} (PLUGIN_TABLES_REQUIRE_PLUGIN_ID_PREFIX enabled):"  # FIXED: 硬编码中文→英文
                        + ", ".join(bad[:8])
                    ),
                )
    return out
...
async def _ensure_plugin_tables_created(*, tables: list[str] | None) -> None:
    """
    安装成功后建表。
    FIXED-P0: S-07 — 使用tables参数仅创建指定表，而非Base.metadata.create_all创建全部表
    """
    if not tables:
        return
    try:
...
async def _install_plugin_from_zip(
    ...
    *,
    ...
    sig_required = bool(getattr(settings, "PLUGIN_PACKAGE_SIGNATURE_REQUIRED", False))
    prod_default_required = bool(getattr(settings, "PLUGIN_PACKAGE_INTEGRITY_REQUIRED_IN_PROD", True))
    ...
    if enforce_required and not expected_package_sha256:
        raise HTTPException(status_code=400, detail="Plugin package hash missing: please fill in package_sha256 in catalog")  # FIXED: hardcoded Chinese → English
    if enforce_required and not expected_package_signature:
        raise HTTPException(status_code=400, detail="Plugin package signature missing: please fill in package_signature in catalog")  # FIXED: hardcoded Chinese → English
    ...
    with zipfile.ZipFile(temp_path, "r") as zip_ref:
        ...
        if security_hits:
            ...
            if bool(getattr(settings, "PLUGIN_SECURITY_SCAN_BLOCK_ON_HIT", False)) and high_hits:
                ...
                raise HTTPException(status_code=400, detail=f"Security scan detected dangerous usage (blocked): {detail}")  # FIXED: hardcoded Chinese → English
            logger.warning("Plugin package security scan hit %d risk items: %s", len(security_hits), "; ".join(security_hits[:5]))  # FIXED: 硬编码中文→英文

        if "plugin.json" not in file_list:
            raise HTTPException(status_code=400, detail="Invalid plugin package: missing plugin.json")  # FIXED: hardcoded Chinese → English
        with zip_ref.open("plugin.json") as f:
            metadata = json.load(f)
            plugin_id = metadata.get("id")
        if not plugin_id:
            raise HTTPException(status_code=400, detail="Invalid plugin metadata: Missing id")  # FIXED: hardcoded Chinese → English
        ...
        if os.path.exists(conflict_py):
            raise HTTPException(status_code=400, detail=f"Plugin id conflicts with existing single-file plugin: {plugin_id}")  # FIXED: hardcoded Chinese → English
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Plugin package is unsafe: {e}")  # FIXED: hardcoded Chinese → English
        ...
        if expected_package_signature:
            ...
            if not pub:
                raise HTTPException(status_code=400, detail="Plugin package signature verification public key not configured: PLUGIN_PACKAGE_ED25519_PUBLIC_KEY / LICENSE_ED25519_PUBLIC_KEY")  # FIXED: hardcoded Chinese → English
            if not actual_sha:
                actual_sha = _sha256_hex_of_file(temp_path)
            ok = verify_ed25519_signature({"package_sha256": actual_sha}, expected_package_signature, pub)
            if not ok:
                raise HTTPException(status_code=400, detail="Plugin package signature verification failed: package_signature mismatch")  # FIXED: hardcoded Chinese → English

        try:
            await _ensure_plugin_dependencies_installed(plugin_id, target_dir, metadata or {}, file_list)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
        except HTTPException:
            raise
        except Exception as e:
            raise HTTPException(status_code=502, detail=f"Plugin dependency install failed: {e}")  # FIXED: hardcoded Chinese → English
        plugin_manager.load_plugins()
        ...
        tables_to_create = _sanitize_plugin_tables(
            metadata.get("tables"),
            plugin_id=plugin_id,
            enforce_plugin_id_prefix=bool(getattr(settings, "PLUGIN_TABLES_REQUIRE_PLUGIN_ID_PREFIX", False)),
        )
        return {
            "status": "success",
            "plugin_id": plugin_id,
...
def verify_plugin_license(metadata: dict, license_data: dict | None, tenant_id: str):
    if not license_data:
        raise HTTPException(status_code=403, detail="Paid plugin missing license.json")  # FIXED: hardcoded Chinese → English
    ...
    if not valid:
        raise HTTPException(status_code=403, detail=f"Plugin authorization invalid: {reason}")  # FIXED: hardcoded Chinese → English
...
async def _build_uninstall_risk_preview(db: AsyncSession, plugin_id: str) -> dict:
    ...
    warnings: list[str] = []
    if table_count > 0:
        warnings.append(f"Will delete {table_count} plugin data table(s), data cannot be recovered.")  # FIXED: 硬编码中文→英文
    if runtime_config_rows > 0:
        warnings.append(f"Will clean up {runtime_config_rows} plugin runtime config row(s).")  # FIXED: 硬编码中文→英文
    if plugin_type == "paid":
        warnings.append("This is a paid plugin, functionality will be unavailable after uninstall.")  # FIXED: 硬编码中文→英文
    if table_count == 0 and runtime_config_rows == 0:
        warnings.append("No plugin data tables or runtime config found, will only delete plugin files and menu entries.")  # FIXED: 硬编码中文→英文
