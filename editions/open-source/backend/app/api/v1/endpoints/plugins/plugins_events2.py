...
async def record_index_verifier_events(
    ...
    page_size: int = Query(50, ge=1, le=200),  # FIXED
    ...
    def _parse_dt(v: str | None) -> datetime | None:
        ...
        try:
            ...
            logger.debug(f"Operation failed, returning default: {e}")  # FIXED: A-07 中文日志→英文
            return None
    ...
    def _parse_line(line: str, file_date) -> dict | None:
        ...
        try:
            ...
            logger.debug(f"Non-critical operation failed: {e}")  # FIXED: A-07 中文日志→英文
            try:
...
async def alert_test(
    ...
    class _TestAlarm:
        def __init__(self, user: User, target: str | None = None, test_channel: str = "all"):
            ...
            self.description = f"Third-party alert channel test (triggered by={user.username})"  # FIXED: A-07 中文→英文
            if target:
    ...
    msg = "Test alert triggered, please check the corresponding channel for receipt."  # FIXED: A-07 中文→英文
    if channel != "all":
        names = {"sms_alert": "SMS", "wecom_alert": "WeCom", "feishu_alert": "Feishu"}  # FIXED: A-07 中文→英文
        msg = f"Only {names.get(channel, channel)} channel test triggered, please check that channel."  # FIXED: A-07 中文→英文
...
async def hook_timeout_test(
    ...
    if timeout_seconds <= 0:
        raise HTTPException(
            status_code=400,
            detail="PLUGIN_HOOK_EXEC_TIMEOUT_SECONDS <= 0: Hook timeout isolation not enabled, cannot verify.",  # FIXED: hardcoded Chinese → English
        )
    ...
    if hook_name not in plugin_manager.hooks:
        raise HTTPException(status_code=400, detail=f"Unknown hook_name: {hook_name}")  # FIXED: hardcoded Chinese → English
    ...
    return {
        ...
        "alarm": captured,
        "log_path": log_path,
        "note": "If no alarm was captured, it may be because HOOK_ON_ALARM has no available alert plugin configured (but this endpoint should still write to the local log).",  # FIXED: A-07 中文→英文
    }