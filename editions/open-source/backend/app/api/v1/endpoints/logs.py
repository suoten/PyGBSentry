...
# FIXED: 全量读取日志文件到内存 → 从文件末尾反向读取，使用 deque 限制最大行数
...
def get_log_lines(filepath: str, keyword: str = "", page: int = 1, page_size: int = 1000):
    ...
    # FIXED: 全量读取日志文件到内存 → 文件大小检查，超过 50MB 拒绝读取
    try:
    ...
    try:
        # FIXED: f.readlines() + lines.reverse() 全量读取 → 从文件末尾反向读取，deque 限制最大行数
...
def download_log_file(filepath: str):
    target = (LOGS_DIR / filepath).resolve()
    if not str(target).startswith(str(LOGS_DIR)):
        raise HTTPException(status_code=403, detail="Access denied")
    if not target.exists() or not target.is_file():
