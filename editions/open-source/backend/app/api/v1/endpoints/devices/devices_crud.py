...
# FIXED: 导出端点全量加载无限制 → 添加 max_rows 限制
...
async def get_devices(
    ...
    # FIXED: 添加 Query 约束，与项目其他 endpoint 保持一致
...
async def get_catalog_subscription(
    ...
    if DeviceSubscription is None:
        raise HTTPException(status_code=501, detail="Device subscription feature not enabled: DeviceSubscription model missing")  # FIXED: 中文错误消息→英文
    asset_result = await db.execute(select(Asset).where(Asset.gb_id == device_id))
    asset = get_or_404(asset_result, detail="Asset not found")  # FIXED: ORM查询结果空值判断
    if not current_user.is_superuser and asset.tenant_id != (current_user.tenant_id or "default"):
        raise HTTPException(status_code=403, detail="Permission denied")  # FIXED: 中文错误消息→英文
    sub = (
...
async def get_mobile_position_subscription(
    ...
    if DeviceSubscription is None:
        raise HTTPException(status_code=501, detail="Device subscription feature not enabled: DeviceSubscription model missing")  # FIXED: 中文错误消息→英文
    asset = get_or_404(await db.execute(select(Asset).where(Asset.gb_id == device_id)), detail="Asset not found")  # FIXED: ORM查询结果空值判断
    if not current_user.is_superuser and asset.tenant_id != (current_user.tenant_id or "default"):
        raise HTTPException(status_code=403, detail="Permission denied")  # FIXED: 中文错误消息→英文
...
async def update_mobile_position_subscription(
    ...
    if enabled:
        if not asset.ip_addr:
            raise HTTPException(status_code=500, detail="Device network info missing")  # FIXED: 中文错误消息→英文
        transport = sip_server.get_transport(asset.ip_addr, asset.port, asset.transport)
        if transport is None:
            raise HTTPException(status_code=503, detail="Device signaling transport unavailable")  # FIXED: 中文错误消息→英文
        if not sip_commander_module.sip_commander:
            raise HTTPException(status_code=503, detail="SIP service not ready")  # FIXED: 中文错误消息→英文
        try:
...
async def update_device_stream_mode(
    ...
    if stream_mode not in {"GLOBAL", "AUTO", "UDP", "TCP_PASSIVE", "TCP_ACTIVE"}:
        raise HTTPException(status_code=400, detail="Invalid stream transport mode")  # FIXED: 中文错误消息→英文
...
async def update_device_organization(
    ...
    result = await db.execute(stmt)
    asset = get_or_404(result, detail="Asset not found")  # FIXED: ORM查询结果空值判断
    ...
    if new_oid:
        ...
        if not org:
            raise HTTPException(status_code=400, detail="Organization not found or permission denied")  # FIXED: 中文错误消息→英文
...
async def create_device(
    ...
    if not gb_id or not name:
        raise HTTPException(status_code=400, detail="Device ID and device name are required")  # FIXED: 中文错误消息→英文
    if len(gb_id) > 20:
        raise HTTPException(status_code=400, detail="Device ID length cannot exceed 20")  # FIXED: 中文错误消息→英文
    ...
    if duplicate:
        raise HTTPException(status_code=400, detail="Device ID already exists")  # FIXED: 中文错误消息→英文

    transport = (payload.transport or "UDP").strip().upper()
    if transport not in {"UDP", "TCP"}:
        raise HTTPException(status_code=400, detail="Transport protocol must be UDP or TCP")  # FIXED: 中文错误消息→英文
...
async def delete_device(
    ...
    await db.commit()
    return {"status": "ok", "message": "Device deleted"}  # FIXED: hardcoded Chinese → English
...
async def batch_delete_devices(
    ...
    if not gb_ids:
        raise HTTPException(status_code=400, detail="Device ID list cannot be empty")  # FIXED: 中文错误消息→英文
...
async def export_devices(
    ...
    # FIXED: 导出端点全量加载无限制 → 添加 max_rows 限制，超过时截断并警告
    truncated = False
    ...
    if format_ == "json":
        ...
        for a in assets:
            ...
            items.append(dev)
        # FIXED: 导出端点全量加载无限制 → 截断时在 JSON 中添加警告
        ...
        # FIXED: 导出端点全量加载无限制 → CSV 行数也受 max_rows 限制
...
async def cleanup_dummy_assets(
    ...
    if not replace_asset:
        raise HTTPException(status_code=400, detail="No replaceable real asset found, cannot delete virtual device")  # FIXED: 中文错误消息→英文
