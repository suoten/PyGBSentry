...
from loguru import logger  # FIXED: 添加logger导入
...
def _set_wizard_completed():
    ...
    try:
        ...
        logger.warning("Failed to write setup wizard done marker: %s", e)  # FIXED: 异常吞没→日志记录

@router.get("/status")
async def get_setup_status(current_user: User = Depends(deps.get_current_active_user)):
    """
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if data.get("code") == 0:
                ...
                zlm_detail = data.get("msg", "ZLM returned an error")  # FIXED: hardcoded Chinese → English
        else:
...
async def complete_setup(
    ...
    if _is_wizard_completed():
        return {"status": "ok", "message": "Setup wizard completed"}  # FIXED: hardcoded Chinese → English
    ...
    return {"status": "ok", "message": "Installation wizard completed"}  # FIXED: hardcoded Chinese → English
