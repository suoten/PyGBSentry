...
async def get_platform_server_config(
    ...
    return {
        ...
        "sip_transport": getattr(settings, "SIP_TRANSPORT", "UDP"),
        "backend_public_host": getattr(settings, "BACKEND_PUBLIC_HOST", ""),
        "backend_public_port": getattr(settings, "BACKEND_PUBLIC_PORT", 0),
    }
...
async def platform_exist_check(
    ...
    if not gb_id:
        raise HTTPException(status_code=400, detail="server_gb_id is required")  # FIXED: 中文错误消息→英文
...
async def list_platform_shareable_channels_flat(
    channel_type: int = Query(0, ge=0),  # FIXED
    keyword: str = "",
    ...
    if t == 0:
        ...
        if kw:
            stmt = stmt.where(or_(Resource.gb_id.ilike(f"%{kw}%"), Resource.name.ilike(f"%{kw}%"), Asset.gb_id.ilike(f"%{kw}%"), Asset.name.ilike(f"%{kw}%")))
        if not current_user.is_superuser:
            stmt = stmt.where(Asset.tenant_id == tenant_id)
        # FIXED-P1: C-15 分页total应返回总条数而非当前页条数
        count_stmt = select(func.count()).select_from(Resource).join(Asset, Asset.id == Resource.asset_id).where(Resource.node_type == "channel")
        if kw:
            count_stmt = count_stmt.where(or_(Resource.gb_id.ilike(f"%{kw}%"), Resource.name.ilike(f"%{kw}%"), Asset.gb_id.ilike(f"%{kw}%"), Asset.name.ilike(f"%{kw}%")))
        if not current_user.is_superuser:
    ...
    if t == 1:
        ...
        if kw:
            stmt = stmt.where(or_(Resource.gb_id.ilike(f"%{kw}%"), Resource.name.ilike(f"%{kw}%"), AccessSource.name.ilike(f"%{kw}%")))
        if not current_user.is_superuser:
        ...
        if kw:
            count_stmt = count_stmt.where(or_(Resource.gb_id.ilike(f"%{kw}%"), Resource.name.ilike(f"%{kw}%"), AccessSource.name.ilike(f"%{kw}%")))
        if not current_user.is_superuser:
    ...
    raise HTTPException(status_code=400, detail="channel_type only supports 0/1/2")  # FIXED: 中文错误消息→英文


def _get_keepalive_threshold() -> int:
    try:
        v = int(getattr(settings, "SIP_PLATFORM_KEEPALIVE_MISS_THRESHOLD", 3) or 3)
    except Exception:
        v = 3
    return max(1, v)


def _build_diagnosis(p: ParentPlatform, runtime: dict) -> dict:  # FIXED-P3: N-15 诊断信息国际化
    items = []
    ...
    try:
        local_sip_port = int(getattr(settings, "SIP_PORT", 0) or 0)
    except Exception:
        local_sip_port = 0
...
async def get_platform_runtime(
    ...
    row = result.first()
    if not row:
        raise HTTPException(status_code=404, detail="Parent platform not found")  # FIXED: 中文错误消息→英文
    p, rt = row
    return {
        "platform": {
            "id": p.id,
...
async def get_inbound_cascade_diagnosis(
    ...
    if not inbound_platforms:
        diagnostics.append({
            ...
            "title": "No inbound registrations detected",  # FIXED: 硬编码中文→英文
            "detail": "No subordinate platform has registered recently. Possible causes: 1) Subordinate platform not sending REGISTER; 2) SIP link blocked (firewall/port); 3) REGISTER not reaching this side.",  # FIXED: 硬编码中文→英文
            "suggestion": "On the subordinate platform: a) Verify target IP/port points to this side; b) Ensure SIP port (5060) is open in firewall; c) Run tcpdump on this server: tcpdump -i any -n 'port 5060'.",  # FIXED: 硬编码中文→英文
        })
    else:
        diagnostics.append({
            "level": "info",
            "key": "inbound_register_seen",
            "title": f"{len(inbound_platforms)} inbound registration(s) detected",  # FIXED-P3: W-13 hardcoded Chinese→English
            "detail": "A subordinate platform has registered to this side.",  # FIXED-P3: W-13 hardcoded Chinese→English
            "suggestion": "Check the 'Inbound Platform List' below to confirm whether registration succeeded.",  # FIXED-P3: W-13 hardcoded Chinese→English
        })
    ...
    if failed_events:
        ...
        diagnostics.append({
            ...
            "title": f"{len(failed_events)} registration auth failure(s) detected",  # FIXED-P3: W-13 hardcoded Chinese→English
            "detail": f"Latest failure: {failed_events[0].created_at}, reason: {latest_fail.get('reason', 'unknown')}",  # FIXED-P3: W-13 hardcoded Chinese→English
            "suggestion": "Common causes: 1) Password mismatch; 2) Realm mismatch; 3) Username/GB ID mismatch.",  # FIXED-P3: W-13 hardcoded Chinese→English
        })
    ...
    if sip_ip in ("0.0.0.0", "") or not sip_ip:
        diagnostics.append({
            ...
            "title": "SIP_IP is configured as 0.0.0.0",  # FIXED-P3: W-13 hardcoded Chinese→English
            "detail": "SIP_IP=0.0.0.0 means listening on all addresses, but the subordinate platform must use the actual reachable address of this side.",  # FIXED-P3: W-13 hardcoded Chinese→English
            "suggestion": "If the subordinate platform is not on the same internal network, ensure this side has a public IP or correct NAT mapping.",  # FIXED-P3: W-13 hardcoded Chinese→English
        })
    ...
    if not sip_domain:
        diagnostics.append({
            ...
            "title": "SIP_DOMAIN is empty",  # FIXED-P3: W-13 hardcoded Chinese→English
            "detail": "SIP_DOMAIN is the realm for Digest Auth and must not be empty.",  # FIXED-P3: W-13 hardcoded Chinese→English
            "suggestion": "Set SIP_DOMAIN in .env (usually the area code), and ensure the subordinate platform's realm config matches.",  # FIXED-P3: W-13 hardcoded Chinese→English
        })
...
async def trigger_platform_register(
    ...
    if not p.enable:
        ...
        raise HTTPException(status_code=400, detail="Platform is disabled")  # FIXED: 中文错误消息→英文
    ...
    if not svc or not getattr(svc, "running", False):
        ...
        raise HTTPException(status_code=503, detail="Cascade service not started (SIP not running or service not initialized)")  # FIXED: 中文错误消息→英文
...
async def create_platform(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.require_roles(["owner", "admin", "operator"])),  # FIXED-P2: N-06 平台写操作权限升级
):
    tenant_id = current_user.tenant_id or "default"
    ...
    if existing.scalars().first():
        ...
        raise HTTPException(status_code=400, detail="Parent platform GB ID already exists")  # FIXED: 中文错误消息→英文
    p = ParentPlatform(
        tenant_id=tenant_id,
        ...
        password=(payload.password.strip() if payload.password and payload.password.strip() else (settings.SIP_DEFAULT_PASSWORD or "")),  # FIXED: 移除弱密码回退"12345678"
...
async def delete_platform(
  ...
  current_user: User = Depends(deps.require_roles(["owner", "admin", "operator"])),  # FIXED-P2: N-06 平台写操作权限升级
    ...
    await db.delete(p)
    await db.commit()
    # FIXED-P2: C-27 删除平台时清理_catalog_ack_counter残留条目
    try:
...
async def set_platform_catalog_resources(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.require_roles(["owner", "admin", "operator"])),  # FIXED-P2: M-05 目录推送范围修改权限升级
):
