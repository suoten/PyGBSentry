...
async def reports_summary(
    ...
    return {
        "items": items,
        "stats": stats,
        "message": "Reports are extended by the report center plugin. Install it to configure templates and data integration here.",  # FIXED: hardcoded Chinese → English
    }
...
async def report_data_alarms(
    ...
    if start_time:
        try:
            ...
            raise HTTPException(status_code=400, detail=f"start_time date format error: {e}")  # FIXED: hardcoded Chinese → English
    if end_time:
        try:
            et = datetime.fromisoformat(end_time.replace('Z', '+00:00'))
            stmt = stmt.where(Alarm.time <= et)
        except (ValueError, TypeError) as e:
            raise HTTPException(status_code=400, detail=f"end_time date format error: {e}")  # FIXED: hardcoded Chinese → English
...
async def report_data_traffic(
    ...
    limit: int = Query(1440, ge=1, le=10000, description="Maximum data points to return"),  # FIXED-P2: W-07 添加分页限制，防止全量加载
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
):
    """
    ...
    stmt = (
        ...
        .limit(limit)  # FIXED-P2: W-07 添加limit限制，防止全量加载OOM
    )
    result = await db.execute(stmt)
...
async def reports_export(
    ...
    if not (cfg.get("enabled") and cfg.get("connector_url")):
        raise HTTPException(status_code=400, detail="Only built-in summary report export supported, other types require the report_suite plugin")  # FIXED: hardcoded Chinese → English
    ...
    try:
        ...
        r = await (await get_http_client()).get(connector_url, params=params, timeout=60)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except Exception as e:
...
async def test_report_suite_connector(
    ...
    try:
        r = await (await get_http_client()).get(target, params={"type": "healthcheck", "tenant_id": current_user.tenant_id or "default"}, timeout=10)  # FIXED: 同步requests→异步httpx，避免阻塞事件循环
    except Exception as e:
...
async def reports_export_pdf(
    ...
    if type != "summary":
        return {"detail": "Only type=summary supported, other types are extended by report plugins"}  # FIXED: 硬编码中文→英文
    stats, items = await _get_summary_stats(db, current_user)
    now = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC")
    lines: list[str] = []
