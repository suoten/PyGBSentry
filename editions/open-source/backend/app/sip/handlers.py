...
async def _cleanup_device_resources(gb_id: str) -> None:
    # FIXED: REGISTER expires=0 should clean up subscriptions, stream sessions, and SSRC
    try:
        from app.sip.subscribe_manager import subscribe_manager
        subscribe_manager.remove_all_for_device(gb_id)
    except Exception as e:
        logger.warning(f"Failed to remove subscriptions for deregistered device {gb_id}: {e}")
    # FIXED-P2: S-09 防止double-free — release_stream_session已删除DB记录，
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            # FIXED: StreamSession has no device_id column; join Asset via asset_id to match gb_id
            ...
            for ss in sessions:
                try:
                    await release_stream_session(session, ss, wait_bye_response=False, reason="device_deregistered")
                    # FIXED-P2: S-09 release_stream_session已删除DB记录，标记为已清理
                    _released_session_ids.add(str(ss.id))
                except Exception as e:
                    logger.warning(f"Failed to release stream session {ss.call_id} for deregistered device {gb_id}: {e}")
            await session.commit()
    except Exception as e:
        logger.warning(f"Failed to cleanup stream sessions for deregistered device {gb_id}: {e}")
    # FIXED: GB28181协议 — 设备离线后清理活跃StreamSession
    # FIXED-P2: S-09 只删除release_stream_session未能成功清理的会话，避免double-free
    try:
        async with AsyncSessionLocal() as session:
            from app.models.stream_session import StreamSession
            from app.models.asset import Asset
            ...
            if asset:
                ...
                for ss in sessions_result.scalars().all():
                    if str(ss.id) not in _released_session_ids:
                        ...
                        logger.debug(f"Skipping already released StreamSession {ss.id} for device {gb_id}")
                await session.commit()
    except Exception as e:
...
async def _validate_digest_replay(auth_params: dict, fallback_user: str) -> tuple[bool, str]:
    ...
    非生产环境默认宽松，仅警告，不阻断。
    """
    # FIXED: default to strict mode for all environments, not just production
    ...
    if current_ts - ts > _DIGEST_NONCE_TTL_SECONDS:
        ...
        if _strict:
            return False, reason
        logger.warning(f"[DigestAuth] stale nonce (age={current_ts - ts}s > {_DIGEST_NONCE_TTL_SECONDS}s), allowing in non-strict mode")
        return True, reason  # 返回 stale 让客户端刷新 nonce

    # 3. HMAC 签名校验：确保 nonce 是我们签发的
    ...
    if not hmac.compare_digest(sig, expected_sig):
        ...
        logger.warning("[DigestAuth] nonce HMAC mismatch, allowing in non-strict mode (possible replay)")
        return True, ""

    # 4. nc（nonce count）重放检查
    try:
        nc_val = int(nc, 16)
    except ValueError:
        if _strict:
...
async def send_response(transport, proto: str, addr: tuple, response: SipMessage):
    ...
    try:
        ...
        server_tx_manager.update_state(key, response)
    except Exception as e:
        logger.warning(f"Failed to cache SIP response or update transaction state: {e}")
    data = response.to_bytes()
    # FIXED: for critical 2xx responses over TCP, use await_drain to ensure delivery
...
def _rewrite_register_contact(contact_value: str, received_addr: tuple, fallback_user: str) -> str:
    if not received_addr:
        return contact_value
    raw = (contact_value or "").strip()
    if not raw:
        user = (fallback_user or "").strip() or "unknown"
        return f"<sip:{user}@{received_addr[0]}:{received_addr[1]}>"
    ...
    return f"{prefix}{received_addr[0]}:{received_addr[1]}{suffix}"
    ...
    async with AsyncSessionLocal() as session:
        ...
        if runtime and runtime.data:
            try:
                ...
                logger.debug(f"Failed to parse catalog runtime JSON: {_json_err}")  # FIXED: W-10 吞异常改为日志
                data = {}
        ...
        try:
            ...
            logger.warning(f"Failed to commit catalog runtime: {_commit_err}")  # FIXED: W-10 吞异常改为日志
...
async def _send_register_401(message: SipMessage, transport, proto: str, addr: tuple, stale: bool = False):
    ...
    # FIXED: GB12 SIP认证SHA-256支持 — RFC 8760 同时提供SHA-256和MD5算法
    ...
    # Add second WWW-Authenticate header for MD5 backward compatibility
    resp.headers["WWW-Authenticate-2"] = auth_header_md5
...
async def _push_catalog_to_platform(
    *,
    ...
    if batch_size <= 0:
        # FIXED: 级联目录推送默认分批（100条/批），避免大目录产生巨型XML报文
...
"""
        ...
        req = SipMessage()
        # 根据 GB28181 标准，目录订阅的增量同步应该使用 NOTIFY 方法
        # 此处我们判断是否是被上级订阅触发的（通常会有特定的标识，开源版此处简化为统一按 NOTIFY 发送带 Subscription-State 的报文，如果上级未订阅也兼容 MESSAGE）
        # 这里统一重构为 GB28181 级联目录订阅与增量推送机制
        req.method = "NOTIFY"
        ...
        req.headers["CSeq"] = f"{sn} NOTIFY"
        req.headers["Event"] = "catalog"
        req.headers["Subscription-State"] = "active"
        # FIXED: RFC 3265 requires NOTIFY CSeq to increment within a subscription dialog.
        # Using sn (which increments per request) satisfies this requirement.
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to parse alarm time '{alarm_time_str}': {_alarm_time_err}")  # FIXED: W-10 吞异常改为日志
            ...
            try:
                ...
                if global_enabled:
                    ...
                    try:
                        ...
                        logger.debug(f"Failed to parse alarm priority: {_prio_err}")  # FIXED: W-10 吞异常改为日志
                        p = 4
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not sip_default_password:
            ...
            if _app_env in {"prod", "production"}:
                ...
                # FIXED: dev environment skipping SIP auth entirely - now only allow loopback
                # FIXED: 增加醒目启动告警，防止开发环境误暴露到公网
        ...
        if pf and pf.password:
            candidate_passwords.append(pf.password)
        ...
        if not candidate_passwords:
            ...
            if pf2 and pf2.password:
                candidate_passwords.append(pf2.password)
        ...
        if asset and asset.password:
            candidate_passwords.append(asset.password)
        ...
        # FIXED: GB12 SIP认证SHA-256支持 — 检测客户端使用的算法并验证
        auth_algorithm = auth_params.get("algorithm", "MD5").upper()
            ...
            if skip_auth:
                ...
                _sip_debug_log(
                    ...
                    message,
                    {
                        ...
                        "auth.cnonce": _mask_sensitive_value(str(auth_params.get("cnonce") or "")),  # FIXED: cnonce脱敏
                ...
                try:
                    ...
                    # FIXED: 鉴权失败计数后端异常时使用高值触发拉黑保护（fail-safe）
                ...
                if fail_count >= 5:
                    ...
                    if not exist_bl:
                        session.add(IpBlacklist(ip=real_ip, reason="Auto-blocked: 5+ auth failures within 5 minutes"))  # FIXED: 国际化
                        await session.commit()
                        from app.sip.server import sip_server
        ...
        try:
            ...
            logger.debug(f"Invalid expires value: {_expires_err}")  # FIXED: W-10 吞异常改为日志
            expires_int = 3600
                    ...
                    if is_renewal:
                        if platform and platform.enable:
                            try:
                                ...
                                logger.warning(f"Renewal expires update failed: {renew_err}")  # FIXED: 续注册时keepalive/IP更新失败仅debug日志，提升为warning
                            ...
                            contact_in = (message.get_header("Contact") or message.get_header("m") or "").strip()
                            resp.headers["Contact"] = _rewrite_register_contact(contact_in, (real_ip, real_port), gb_id)
                            resp.headers["Server"] = settings.PROJECT_NAME
                            await send_response(transport, proto, addr, resp)
                                ...
                                try:
                                    async with AsyncSessionLocal() as renew_session:
                                        ...
                                        if a:
                                            ...
                                            # FIXED: registration renewal should update IP/port for NAT traversal
                                    ...
                                    logger.warning(f"Device renewal expires update failed: {renew_err}")  # FIXED: 续注册时keepalive/IP更新失败仅debug日志，提升为warning
                            resp = create_response(message, 200, received_addr=addr)
                            resp.headers["Date"] = _sip_date_gmt(now)
                            resp.headers["Expires"] = str(expires_int)
                            contact_in = (message.get_header("Contact") or message.get_header("m") or "").strip()
        ...
        if platform and platform.enable:
            ...
            resp.headers["Expires"] = str(expires_int)
            contact_in = (message.get_header("Contact") or message.get_header("m") or "").strip()
            contact_out = _rewrite_register_contact(contact_in, (real_ip, real_port), gb_id)
        ...
        if expires_int == 0:
            ...
            if not asset:
                ...
                if recent_devices_count >= 10:
                    ...
                    exist_bl = await session.scalar(select(IpBlacklist).where(IpBlacklist.ip == real_ip))
                    if not exist_bl:
                        session.add(IpBlacklist(ip=real_ip, reason="Auto-blocked: 10+ device registrations from same IP within 1 hour"))  # FIXED: 国际化
                        await session.commit()
                ...
                asset = Asset(gb_id=gb_id, name=f"Auto_Discovered_{gb_id}", password=getattr(settings, "SIP_DEFAULT_PASSWORD", "") or "", tenant_id="default")  # FIXED: GB28181协议 — 自动发现设备使用SIP默认密码
                session.add(asset)
                is_new_asset = True
...
async def handle_message_request(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    body = message.body
    looks_like_xml = bool(
        body and body.strip() and (  # FIXED-P2: C-30 增加body.strip()检查，防止空字符串误判为XML
        ...
        if cmd_type == "Keepalive":
            ...
            if is_query:
                ...
                try:
                    ...
                    logger.debug(f"Failed to parse SN value: {_sn_err}")  # FIXED: W-10 吞异常改为日志
                    sn_override = None
            ...
            if root_name == "query":
                ...
                try:
                    ...
                    # FIXED: 级联目录查询异常不再静默吞没，记录warning日志
            ...
            if ptz_cmd_elem is not None:
                ...
                try:
                    ...
                    # FIXED: 级联设备控制查询异常不再静默吞没，记录warning日志
                ...
                if is_cascade:
                    ...
                    try:
                        from app.sip.catalog import catalog_data_manager
                        catalog_data_manager.put(device_id_xml, "DeviceControl", body)
                    except Exception as e:
            ...
            # FIXED: 处理 ConfigDownload 响应，记录配置内容
    ...
    if longitude and latitude:
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to parse position time: {_pos_time_err}")  # FIXED: W-10 吞异常改为日志
                pos_time = datetime.datetime.now(datetime.timezone.utc)
...
async def handle_invite_request(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not asset:
            ...
            if bool(getattr(settings, "CASCADE_RTP_MEDIA_BYPASS", True)):
                ...
                try:
                    ...
                    # FIXED: check invite_ok before using sdp_response - if bypass failed, reply error to upstream
        ...
        local_port = zlm_ret.get("local_port", recv_port) if zlm_ret and zlm_ret.get("code") == 0 else recv_port  # FIXED-P0: S-03 ZLM失败时回退到recv_port而非硬编码10000
...
async def handle_options(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    # FIXED: GB1 SIP OPTIONS handler — RFC 3261 Section 11 可达性检测
    _bg_create_task(plugin_manager.emit(HOOK_ON_SIP_RECEIVE, message, addr, proto))
    resp = create_response(message, 200, received_addr=addr)
...
async def handle_update(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    # FIXED: GB1 SIP UPDATE handler — RFC 3311 会话参数更新
    call_id = message.call_id
    if not call_id:
...
async def handle_subscribe(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    try:
        ...
        logger.debug(f"Invalid subscribe expires value: {_sub_exp_err}")  # FIXED: W-10 吞异常改为日志
        expires_int = 0
    ...
    try:
        ...
        # FIXED: GB5 SUBSCRIBE Event头验证 — 仅允许GB28181支持的事件类型
    ...
    try:
        ...
        # FIXED: handle_subscribe 支持 Alarm 事件
    ...
    async def _after_subscribe() -> None:
        try:
            ...
            try:
                ...
                logger.debug(f"Invalid platform subscribe expires value: {_plat_exp_err}")  # FIXED: W-10 吞异常改为日志
                expires_int = 0
            ...
            if expires_int == 0:
                try:
                    ...
                    branch = f"z9hG4bK{secrets.token_hex(6)}" # FIXED: __import__ 反模式改为标准 import
    ...
    try:
        ...
        if "sip:" in from_uri:
            gb_id = from_uri.split("sip:")[1].split("@")[0]
        if gb_id and event:
            # FIXED-P2: M-03 SUBSCRIBE验证订阅方身份
            ...
            try:
                async with AsyncSessionLocal() as _sub_id_session:
                    ...
                    if _plat:
                        ...
                        _dev = (
                            await _sub_id_session.execute(select(Asset).where(Asset.gb_id == gb_id))
                        ).scalars().first()
                        if _dev:
                            _subscriber_known = True
            except Exception as _sub_id_err:
                logger.warning(f"[SUBSCRIBE] Identity lookup failed for {gb_id}: {_sub_id_err}")
            if not _subscriber_known:
                logger.warning(f"[SUBSCRIBE] Rejected: unknown subscriber gb_id={gb_id} from {addr}")
                try:
                    from app.sip.send import send_sip_bytes
                    from app.sip.message import SipMessage as _SipMsg
            ...
            try:
                ...
                logger.debug(f"Invalid NOTIFY subscribe expires value: {_notify_exp_err}")  # FIXED: W-10 吞异常改为日志
                expires_int = 0
...
async def handle_notify(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    if event_hdr and call_id_hdr:
        try:
            ...
            logger.debug(f"Failed to check subscription existence: {_sub_find_err}")  # FIXED: W-10 吞异常改为日志
            subscription_found = False
    else:
        subscription_found = False
    ...
    async def _after_notify() -> None:
        try:
            ...
            # FIXED: MobilePosition NOTIFY处理 — 解析设备上报的移动位置通知并更新数据库
...
async def handle_cancel(message: SipMessage, addr: tuple, proto: str, transport):
    _bg_create_task(plugin_manager.emit(HOOK_ON_SIP_RECEIVE, message, addr, proto))
    call_id = message.call_id
    # FIXED: S-05 CANCEL 未验证 Via branch 是否与原始 INVITE 匹配（RFC 3261 Section 9.1）
...
async def handle_ack(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    if call_id:
        from app.sip.invite_server_state import invite_server_state
        # FIXED: GB28181协议 — 验证ACK的From/To tag是否与原始INVITE匹配
        async with invite_server_state._lock:
            ctx = invite_server_state._items.get(call_id)
        if ctx and ctx.message:
            ack_from_tag = ""
        ...
        if ok:
            ...
            # FIXED: W-01 confirm_transaction 未被调用 — 收到ACK后需将事务状态从 Accepted 推进到 Confirmed
            try:
                from app.sip.transactions import server_tx_manager
                from app.sip.server import sip_server
                tx_key = sip_server._tx_key_from_request(message)
                server_tx_manager.confirm_transaction(tx_key)
...
async def handle_info(message: SipMessage, addr: tuple, proto: str, transport):
    _bg_create_task(plugin_manager.emit(HOOK_ON_SIP_RECEIVE, message, addr, proto))
    # FIXED: 解析 MANSRTSP 内容，提取 NPT 播放进度
    ...
    if body and "MANSRTSP" in body:
        ...
        if npt_match:
            ...
            if call_id:
                from app.sip.playback_control import _npt_results_put  # FIXED: W-30 使用带大小限制的写入方法
...
async def handle_bye(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    async with AsyncSessionLocal() as session:
        ...
        if stream_session:
            ...
            # FIXED: S-06 BYE tag为空时降级为Call-ID-only匹配存在伪造风险，增加源IP+端口校验
            if not tag_matched and (not ss_from_tag or not ss_to_tag):
                ...
                _registered_port = 0
                try:
                    # FIXED-P2: M-04 消除BYE处理嵌套DB会话，使用外层session查询
                ...
                if _registered_ip and client_ip != _registered_ip:
                    ...
                    await send_response(transport, proto, addr, resp)
                    return
                # FIXED: S-06 增加源端口校验 — 同NAT下不同设备端口不同
            ...
            try:
                ...
                # FIXED-P0: S-04 移除重复SSRC释放，finalize_stream_session内部已释放
                try:
            ...
            # FIXED-P0: BYE处理后取消流切换看门狗，防止超时后对已终止会话发送Re-INVITE
            try:
            ...
            # FIXED: GB28181协议 — BYE处理后清理流切换全局字典
            try:
                from app.sip.invite import _STREAM_SWITCH_PENDING, _STREAM_SWITCH_PENDING_TIMESTAMPS, _STREAM_SWITCH_ROLLBACK_DEPTH, _STREAM_SWITCH_ROLLBACK_DEPTH_TIMESTAMPS
                ...
                _STREAM_SWITCH_ROLLBACK_DEPTH_TIMESTAMPS.pop(call_id, None)
            except Exception as _switch_err:
                logger.debug(f"Failed to cleanup stream switch state: {_switch_err}")  # FIXED: W-10 吞异常改为日志
            return
...
def init_handlers():
    ...
    # FIXED: 启动时从DB恢复活跃SSRC，防止进程重启后SSRC冲突
    ...
    sip_server.register_handler("UPDATE", handle_update)  # FIXED: GB1 SIP UPDATE handler注册
