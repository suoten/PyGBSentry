...
def parse_sdp(body: str, fallback_ip: str | None = None) -> dict:
    ...
    for raw in text.split("\n"):
        ...
        if k == "m":
            ...
            if len(parts) >= 3:
                ...
                # FIXED: GB9 SDP port=0处理 — RFC 4566 Section 5.14 port=0表示拒绝媒体流
                if port == 0:
        ...
        if k == "a" and current_media is not None:
            ...
            # FIXED: GB9 SDP fmtp解析 — H264 profile兼容性检测
    ...
    # FIXED: GB9 SDP fmtp解析 — 收集所有媒体流的fmtp参数
...
def build_sdp(
    *,
    ...
    # FIXED: GB9 SDP GB28181必需属性 — a=tool 和 a=type
