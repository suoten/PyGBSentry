...
class SsrcManager:
    ...
    # FIXED: 启动时从DB StreamSession恢复活跃SSRC，防止进程重启后SSRC冲突
    ...
    async def allocate_specific_ssrc(self, ssrc: str, is_playback: bool = False) -> bool:
        ...
        # FIXED: allocate_specific_ssrc — 设备在200 OK中修改SSRC时需要注册新SSRC
        async with self._lock:
            try:
                ssrc_int = int(ssrc)
            except (ValueError, TypeError):
                return False
            # FIXED: GB10 SSRC跨集合冲突检查 — 确保SSRC不在另一集合中已分配
            ...
            if ssrc_int in self._playback_allocated and not is_playback:
                ...
                return False
            # FIXED-P2: 仅使用is_playback参数决定集合，不根据SSRC首字符推断
    ...
    # FIXED: release_ssrc — alias for release() to fix method name mismatch in response_handler.py
    ...
    async def cleanup_stale(self, max_age_seconds: int = 86400) -> int:
        # FIXED: cleanup_stale now checks DB StreamSession activity instead of allocation time alone
        ...
        async with self._lock:
            ...
            try:
                ...
                return 0  # FIXED: DB不可用时跳过清理，避免误清活跃流的SSRC
