...
async def should_skip_keepalive_db_update(gb_id: str, ip: str, port: int) -> bool:
    ...
    if redis_client is not None:  # FIXED: 显式 None 守卫避免 pyright Never
        try:
...
def enqueue_keepalive_update(gb_id: str, ip: str, port: int, proto: str):
    ...
    try:
        ...
        _keepalive_cache[gb_id] = (time.time(), ip, port)  # FIXED: 队列满时至少刷新内存缓存，防止设备误判离线
...
async def _process_batch(batch: list):
    ...
    # FIXED: single session for entire batch instead of one session per item
    try:
