...
def _create_basic_response(request: SipMessage, status_code: int, reason: str, received_addr: tuple | None) -> SipMessage:
    ...
    return create_response(request, status_code, reason, received_addr or ("0.0.0.0", 0))  # FIXED: tuple|None → tuple
...
class SipServer:
    def __init__(self):
        ...
        self._ip_rate_lock = threading.Lock()  # FIXED: threading.Lock for sync _schedule_process
        self._inflight = 0
    ...
    def get_transport(self, ip: str, port: int, proto: str):
        ...
        if protocol == "TCP":
            ...
            if ip_only_matches:
                ...
                _max_fallback = int(getattr(settings, "SIP_TCP_IP_ONLY_FALLBACK_MAX", 100) or 100)  # FIXED: 可配置阈值
                ...
                try:
                    ...
                    if idle_time >= tcp_keepalive_interval:
                        try:
                            ...
                            logger.debug(f"TCP keepalive send failed for {addr}: {e}")
                            break
                    continue
    ...
    async def _send_overload_response(self, data: bytes, addr: tuple, proto: str, transport) -> None:
        try:
            ...
            # FIXED: SIP过载时解析失败不再静默丢弃，记录warning日志
    ...
    def _schedule_process(self, data, addr, proto, transport):
        ...
        # IP黑名单检查
        if addr and addr[0]:
            # FIXED: IP rate limiting with lock to prevent race conditions
            now = time.time()
            ip = addr[0]
            ...
            if blacklist_ttl > 0 and (now - self._ip_blacklist_last_reload) > blacklist_ttl:
                asyncio.create_task(self.reload_ip_blacklist())
        ...
        if int(self._inflight) >= int(self._max_inflight) and self._task_queue.full():
            asyncio.create_task(self._send_overload_response(data, addr, proto, transport))
            return
    ...
    async def _prune_loop(self):
        while self.running:
            try:
                ...
                server_tx_manager.prune()  # FIXED: W21 确认：server_tx_manager.prune() 已在此定时调用（每5秒）
                client_tx_manager.prune()  # FIXED: W21 确认：client_tx_manager.prune() 已在此定时调用（每5秒）
            except Exception as e:
                logger.debug(f"Prune loop error: {e}")
            # FIXED: I10 定期清理 invite.py 中的全局字典，防止内存泄漏
            try:
                from app.sip.invite import _cleanup_global_dicts
                _cleanup_global_dicts()
            except Exception as e:
                logger.warning(f"Global dicts cleanup error: {e}")  # FIXED: GB28181协议 — 全局字典清理失败应warning级别
            # FIXED: 设备离线检测 — 定期扫描超时设备并标记离线
            try:
                await self._check_device_offline()
            except Exception as e:
                logger.debug(f"Device offline check error: {e}")
            await asyncio.sleep(5)

    # FIXED: 设备离线检测 — 扫描 last_keepalive + expires < now 的设备，标记离线
    # FIXED: 使用设备注册的 Expires 值作为基准，而非固定 60s 宽限时间
    async def _check_device_offline(self) -> int:
        try:
            ...
            # FIXED: use per-device expires for grace calculation: grace = max(expires, default) * 1.5
            ...
            async with AsyncSessionLocal() as session:
                ...
                # FIXED-P2: S-08 移除last_keepalive.isnot(None)条件，同时检测last_keepalive为NULL但status=1的设备
                ...
                for row in devices_result:
                    device_expires = int(row.expires or 3600)  # FIXED: 双重 or 3600 冗余，第二个永远不执行
                    # FIXED: GB13 心跳保活宽限时间 — 使用注册有效期而非固定300秒上限
                    grace = device_expires * 1.2
                    # FIXED-P2: W-05 Cap max grace to 600s (10 min) instead of device_expires * 2
                    # which can be 7200s for long-expires devices, causing stale devices to stay online
                    max_grace = min(max(device_expires * 2, 300), 600)  # At least 300s, at most 600s
                    grace = min(grace, max_grace)
                    cutoff = now - datetime.timedelta(seconds=grace)
                    if row.last_keepalive and row.last_keepalive < cutoff:
                        offline_ids.append(row.id)
                    elif not row.last_keepalive:
                        # FIXED-P2: S-08 last_keepalive为NULL但status=1的设备，
                        # 若注册时间超过宽限期仍未收到心跳，应标记离线
                        register_time = row.register_time
                        if register_time and register_time < cutoff:
                            offline_ids.append(row.id)
                count = 0
                if offline_ids:
                    ...
                    # FIXED: 设备离线后通道状态未更新 — 同时将关联 Resource 状态设为 0
            ...
            logger.debug(f"Device offline check failed (non-critical): {e}")
            return 0
        ...
        try:
            ...
            if settings.CLUSTER_ENABLED:
                try:
                    ...
                    logger.debug(f"Cluster route check failed: {e}")
            ...
            if "sipvicious" in ua or "friendly-scanner" in ua or "sipcli" in ua or "vaxip" in ua:
                ...
                # 记录拉黑
                await self._auto_blacklist_ip(client_ip, f"L7 firewall blocked: malicious scanner User-Agent '{ua}'")  # FIXED: A-04 中文日志→英文
                return
            ...
            # Emit SIP TRACE
            asyncio.create_task(plugin_manager.emit("ON_SIP_RECV", msg, addr, proto))

            if msg.method: # Request
                # FIXED: GB28181协议 — 入站请求递减Max-Forwards，防止信令路由循环
                ...
                if max_forwards is not None:
                    try:
                        ...
                        # FIXED: Max-Forwards递减到0时未拒绝请求 — GB28181/SIP规范要求返回483 Too Many Hops
                        if mf_val - 1 <= 0:
                        ...
                        logger.debug(f"Invalid Max-Forwards header: {e}")  # FIXED: W-09 Max-Forwards 解析失败不再静默跳过
                        msg.headers["Max-Forwards"] = "70"  # FIXED: W-09 解析失败时设置默认值 70

                # FIXED: 信令路由循环防护 — 重复请求检测（Call-ID + CSeq）
                from app.sip.handlers import _SEEN_REQUESTS, _SEEN_REQUESTS_MAX, _SEEN_REQUESTS_TTL
                call_id = msg.get_header("Call-ID") or ""
                cseq = msg.get_header("CSeq") or ""
                dedup_key = f"{call_id}~{cseq}"
                now = time.time()
                # FIXED-P0: INVITE重传必须交给事务层处理(RFC 3261 Section 17.2.1)，
                ...
                if msg.method != "ACK":
                    tx, is_new = server_tx_manager.get_or_create(msg)
                    if not is_new:
    ...
    async def reload_tls_cert(self):
        ...
        try:
            ...
            logger.info(f"[TLS] TLS server restarted on {sip_ip}:{tls_port}")
        except Exception as e:
            logger.error(f"[TLS] Failed to reload TLS certificate: {e}")
    ...
    async def start(self):
        ...
        try:
            ...
            # FIXED: W-05 cleanup_loop 未启动 — 启动 DialogManager 和 SsrcManager 的清理循环
            try:
                from app.sip.dialog_manager import dialog_manager
                asyncio.create_task(dialog_manager.cleanup_loop())
            except Exception as e:
                logger.warning(f"Failed to start DialogManager cleanup_loop: {e}")
            try:
                from app.sip.ssrc_manager import ssrc_manager
                asyncio.create_task(ssrc_manager.cleanup_loop())
            except Exception as e:
