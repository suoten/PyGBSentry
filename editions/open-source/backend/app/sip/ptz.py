...
# FIXED: GB28181协议 — PTZ指令限流器，防止SIP消息洪泛
...
class _PtzRateLimiter:
    ...
    def __init__(self, min_interval: float = 0.1):
        ...
        self._cancelled: set[str] = set()  # FIXED: PTZ最新指令优先策略 — 被取消的device_id集合

    def cancel_pending(self, device_id: str) -> None:
        """Mark a device's pending command as cancelled so the next acquire skips it."""
        self._cancelled.add(device_id)  # FIXED: PTZ最新指令优先策略 — 新指令到达时取消旧指令
    ...
    async def acquire(self, device_id: str) -> bool:
        ...
        if wait > 0:
            # FIXED: PTZ最新指令优先策略 — 等待前检查是否已被新指令取消
            ...
            # FIXED: PTZ最新指令优先策略 — 等待后再次检查是否已被新指令取消
            if device_id in self._cancelled:
                self._cancelled.discard(device_id)
...
def _sip_trace_should_log() -> bool:
    ...
    try:
        ...
        logger.warning("SIP_TRACE_SAMPLE_RATE config parse failed")  # FIXED: 国际化
        rate = 1.0
...
class SipPtz:
    ...
    async def _send_device_control_xml(self, asset, channel_id: str, transport_info: tuple, xml_body: str, call_suffix: str, log_label: str):
        ...
        sn = _next_sn()  # FIXED: SN序列号使用递增计数器替代hash
        req = SipMessage()
        req.method = "MESSAGE"
    ...
    async def send_ptz(self, asset, resource, transport_info: tuple, command: str, speed: int = 50, drag_data: dict = None):
        ...
        _ptz_rate_limiter.cancel_pending(device_id)  # FIXED: PTZ最新指令优先策略 — 新指令取消旧指令
        if not await _ptz_rate_limiter.acquire(device_id):  # FIXED: PTZ最新指令优先策略 — 被新指令取代则跳过
            return
...
"""
            ...
            # FIXED: GB4 PTZ扩展命令 — GB28181标准命令集
            cmd_code = 0x00
            param1 = 0 # Pan Speed
            param2 = 0 # Tilt Speed
            ...
            if command == "stop":
                logger.debug("PTZ stop command received, sending zero-velocity stop")  # FIXED: PTZ stop 命令为空操作
            elif command == "right":
                cmd_code = 0x01
                param1 = speed
            elif command == "left":
            ...
            # FIXED: GB4 PTZ扩展命令 — 预置位/巡航/扫描/雨刷/辅助/录像/布防/复位
            ...
            # FIXED: GB4 PTZ扩展命令 — 录像/布防/复位（委托device_control模块）
...
"""
    ...
    async def send_preset(self, asset, resource, transport_info: tuple, preset_id: int):
        ...
        addr, proto, transport = transport_info
        channel_id = resource.gb_id
        _ptz_rate_limiter.cancel_pending(asset.gb_id)  # FIXED: PTZ最新指令优先策略 — 新指令取消旧指令
        if not await _ptz_rate_limiter.acquire(asset.gb_id):  # FIXED: PTZ最新指令优先策略 — 被新指令取代则跳过
            return
...
"""
    ...
    def _get_iris_cmd(self, command: str, speed: int = 128) -> str:
        ...
        if command == 'in':
            # 光圈? A5 0F 01 02 [speed] 00
            cmd = [0xA5, 0x0F, 0x01, 0x02, speed, 0x00]
        elif command == 'out':
            # 光圈? A5 0F 01 04 [speed] 00
            cmd = [0xA5, 0x0F, 0x01, 0x04, speed, 0x00]
        else:  # stop
            # 停止: A5 0F 01 00 00 00
            cmd = [0xA5, 0x0F, 0x01, 0x00, 0x00, 0x00]
...
"""
    ...
    def _get_focus_cmd(self, command: str, speed: int = 128) -> str:
        ...
        if command == 'near':
            # 近焦: A5 0F 01 01 [speed] 00
            cmd = [0xA5, 0x0F, 0x01, 0x01, speed, 0x00]
        elif command == 'far':
            # 远焦: A5 0F 01 03 [speed] 00
            cmd = [0xA5, 0x0F, 0x01, 0x03, speed, 0x00]
        else:  # stop
            # 停止: A5 0F 01 00 00 00
            cmd = [0xA5, 0x0F, 0x01, 0x00, 0x00, 0x00]
...
"""
    ...
    def _get_cruise_cmd(self, cruise_id: int, preset_id: int, action: str, speed: int = 128, stay_time: int = 5) -> str:
        ...
        if action == 'add':
            # 添加巡航? A5 0F 01 82 [cruise_id] [preset_id]
            cmd = [0xA5, 0x0F, 0x01, 0x82, cruise_id, preset_id]
        elif action == 'delete':
            # 删除巡航? A5 0F 01 83 [cruise_id] [preset_id]
            cmd = [0xA5, 0x0F, 0x01, 0x83, cruise_id, preset_id]
            ...
            # 开始巡? A5 0F 01 86 [cruise_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x86, cruise_id, 0x00]
        elif action == 'stop':
            # 停止巡航: A5 0F 01 87 [cruise_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x87, cruise_id, 0x00]
        elif action == 'delete_group':
            # 删除巡航? A5 0F 01 88 [cruise_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x88, cruise_id, 0x00]
        else:
            cmd = [0xA5, 0x0F, 0x01, 0x00, 0x00, 0x00]
...
"""
    ...
    def _get_scan_cmd(self, scan_id: int, action: str, speed: int = 128) -> str:
        ...
        if action == 'start':
            # 开始扫? A5 0F 01 99 [scan_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x99, scan_id, 0x00]
        elif action == 'stop':
            # 停止扫描: A5 0F 01 9A [scan_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x9A, scan_id, 0x00]
        elif action == 'set_left':
            # 设置左边? A5 0F 01 9B [scan_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x9B, scan_id, 0x00]
        elif action == 'set_right':
            # 设置右边? A5 0F 01 9C [scan_id] 00
            cmd = [0xA5, 0x0F, 0x01, 0x9C, scan_id, 0x00]
        elif action == 'set_speed':
...
"""
    ...
    def _get_wiper_cmd(self, command: str) -> str:
        ...
        """
        if command == "on":
            cmd = [0xA5, 0x0F, 0x01, 0x30, 0x00, 0x01]
        elif command == "off":
            cmd = [0xA5, 0x0F, 0x01, 0x30, 0x00, 0x00]
        else:  # stop
            cmd = [0xA5, 0x0F, 0x01, 0x00, 0x00, 0x00]
        checksum = sum(cmd) % 256
        cmd.append(checksum)
    ...
    async def send_wiper(self, asset, resource, transport_info: tuple, command: str):
        ...
        _wiper_sn = _next_sn()  # FIXED: SN序列号使用递增计数器替代hash
        xml_body = f"""<?xml version="1.0" encoding="GB2312"?>
<Control>
<CmdType>DeviceControl</CmdType>
<SN>{_wiper_sn}</SN>
...
"""
    ...
    def _get_aux_switch_cmd(self, aux_id: int, command: str) -> str:
        ...
        cmd = [0xA5, 0x0F, 0x01, 0x31, aux_id, state]
        checksum = sum(cmd) % 256
        cmd.append(checksum)
        return "".join([f"{b:02X}" for b in cmd])
    ...
    async def send_aux_switch(self, asset, resource, transport_info: tuple, aux_id: int, command: str):
        ...
        _aux_sn = _next_sn()  # FIXED: SN序列号使用递增计数器替代hash
        xml_body = f"""<?xml version="1.0" encoding="GB2312"?>
<Control>
<CmdType>DeviceControl</CmdType>
<SN>{_aux_sn}</SN>
