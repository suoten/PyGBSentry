...
class SipRecord:
    ...
    async def query_device_record(self, asset, resource, transport_info: tuple, start_time: datetime.datetime, end_time: datetime.datetime, page_size: int = 50, page_num: int = 1):  # FIXED: datetime module vs datetime class, 录像查询分页支持
        """
        ...
        sn = next(_record_sn_counter) % 9999 + 1  # FIXED: P4 SN唯一性 — random→递增计数器
        # FIXED: 录像查询分页支持 — 添加RecNum/SumNum/PageNum分页参数，避免大量录像一次性返回
...
"""
        ...
        req.headers["Via"] = f"SIP/2.0/{proto} {sip_host_for_contact()}:{settings.SIP_PORT};rport;branch=z9hG4bK{secrets.token_hex(5)}"  # FIXED: P4 安全随机数 — random→secrets
        req.headers["From"] = f"<sip:{settings.SIP_ID}@{settings.SIP_DOMAIN}>;tag={secrets.token_hex(4)}"  # FIXED: P4 安全随机数 — random→secrets
        req.headers["To"] = f"<sip:{device_id}@{settings.SIP_DOMAIN}>"
        req.headers["Call-ID"] = f"{sn}_rec@{sip_host_for_contact()}"
    ...
    async def send_record_control(self, asset, resource, transport_info: tuple, action: str = "Record", stream_id: str = "") -> int:
        """
        FIXED: 录像控制命令 — 发送 Record/StopRecord 控制指令。
...
"""
        ...
        req.headers["Via"] = f"SIP/2.0/{proto} {sip_host_for_contact()}:{settings.SIP_PORT};rport;branch=z9hG4bK{secrets.token_hex(5)}rc"  # FIXED: P4 安全随机数 — random→secrets
        req.headers["From"] = f"<sip:{settings.SIP_ID}@{settings.SIP_DOMAIN}>;tag={secrets.token_hex(4)}rc"  # FIXED: P4 安全随机数 — random→secrets
        req.headers["To"] = f"<sip:{device_id}@{settings.SIP_DOMAIN}>"
        req.headers["Call-ID"] = f"{sn}_rc@{sip_host_for_contact()}"
        req.headers["CSeq"] = f"1 MESSAGE"
        req.headers["Content-Type"] = "Application/MANSCDP+xml"
        req.headers["Max-Forwards"] = "70"
        req.headers["User-Agent"] = settings.PROJECT_NAME
