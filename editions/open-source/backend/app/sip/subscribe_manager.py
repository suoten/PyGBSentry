...
_CHUNK_SIZE = 100  # FIXED: GB3 Catalog NOTIFY分页 — 每个NOTIFY最多100个通道
...
class SubscribeManager:
    def __init__(self):
        ...
        self._on_alarm: list = []  # FIXED: 新增报警订阅回调列表
        self._inbound_renew_tasks: dict[str, asyncio.Task] = {}  # FIXED: 目录订阅自动续期 — 跟踪inbound续期任务
    ...
    # FIXED: GB13 CSeq持久化 — 进程重启后CSeq从DB恢复
    ...
    async def _restore_cseq(self, call_id: str) -> int:
        ...
        try:
            ...
            logger.warning(f"Failed to restore CSeq: {e}")  # FIXED: W-08 _restore_cseq 吞异常 — 改为 warning 日志
        return 1

    def on_alarm(self, callback):
        """Register a callback for alarm notifications. FIXED: GB5 报警订阅注册方法"""
    ...
    async def stop(self):
        ...
        # FIXED: 目录订阅自动续期 — stop时取消所有inbound续期任务
    ...
    # FIXED: 目录订阅自动续期 — 启动/重启inbound订阅续期任务
    ...
    # FIXED: 目录订阅自动续期 — 在订阅过期前20%时间发送SUBSCRIBE续期
    async def _inbound_renew_loop(self, key: str, sub: SubscribeInfo):
        try:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    await self._send_inbound_subscribe_renew(current)
                    current.last_renewed = time.monotonic()
                    logger.info(f"[SubscribeManager] Renewed inbound subscribe: device={current.device_id} event={current.event} attempt={attempt}")
                    # 续期成功后启动下一轮续期
                    self._start_inbound_renew(key, current)
                    return
                except Exception as e:
            ...
            # FIXED-P1: W-02 续期全部失败后移除订阅条目，触发重新订阅流程
    ...
    # FIXED: 目录订阅自动续期 — 发送SUBSCRIBE续期请求
    ...
    async def put_inbound(self, subscribe: SubscribeInfo) -> SubscribeInfo | None:
        ...
        async with self._lock:
            ...
            if existing and existing.call_id != subscribe.call_id:
                ...
                # FIXED: 目录订阅自动续期 — 替换时取消旧续期任务
                old_task = self._inbound_renew_tasks.pop(key, None)
                if old_task and not old_task.done():
                    old_task.cancel()
            # FIXED: GB13 CSeq持久化 — 新订阅时尝试从Redis恢复CSeq
    ...
    def get_inbound(self, device_id: str, event: str) -> SubscribeInfo | None:
    ...
    def put_outbound(self, subscribe: OutboundSubscribe) -> OutboundSubscribe:
        key = f"{subscribe.device_id}:{subscribe.event}"
    ...
    def get_outbound(self, device_id: str, event: str) -> OutboundSubscribe | None:
        return self._outbound.get(f"{device_id}:{event}")

    def remove_outbound(self, device_id: str, event: str) -> OutboundSubscribe | None:
    ...
    async def notify_catalog_change(self, device_id: str, channels: list):
        ...
        # FIXED: GB28181协议 — 支持通配订阅
        if not sub:
            sub = self._inbound.get("*:Catalog")
        if not sub:
            return
        # FIXED: 目录订阅NOTIFY — 变更后主动向订阅方发送SIP NOTIFY消息
        try:
    ...
    async def _send_catalog_notify(self, sub: SubscribeInfo, channels: list) -> None:
        ...
        # FIXED: GB3 Catalog NOTIFY分页 — 通道数超过_CHUNK_SIZE时分多个NOTIFY发送
        from app.sip.server import sip_server
        from app.sip.message import SipMessage
        from app.sip.send import send_sip_bytes
        from app.core.config import settings, sip_host_for_contact
        ...
        def _build_items_xml(ch_list):
            ...
            for ch in ch_list:
                ...
                # FIXED: GB28181协议 — Catalog Item补全Table 9必填字段
                items_xml += "<Item>"
...
"""
        ...
        if total <= _CHUNK_SIZE:
            ...
            sub.cseq += 1  # FIXED-P2: M-02 单条Catalog NOTIFY CSeq递增顺序修正为先递增后构建
            await self._persist_cseq(sub.call_id, sub.cseq)  # FIXED-P2: M-01 Catalog/Alarm NOTIFY CSeq持久化
...
"""
    ...
    async def notify_alarm(self, device_id: str, alarm_info: dict):
        """Dispatch alarm notification to registered callbacks. FIXED: GB5 报警通知分发"""
        # FIXED: GB5 向已订阅 Alarm 事件的订阅方发送报警 NOTIFY
        for key, sub in list(self._inbound.items()):
            if sub.event.lower() != "alarm" or sub.device_id != device_id:
                continue
            if sub.is_expired:
                continue
            try:
    ...
    async def _send_alarm_notify(self, sub: SubscribeInfo, alarm_info: dict):
        ...
        alarm_xml = (
            ...
            f'<SN>{int(time.time() * 1000) % 100000}</SN>\n'  # FIXED-P2: M-12 Alarm NOTIFY SN扩大值域
    ...
    # FIXED: MobilePosition出站订阅 — 启动移动位置订阅，发送SUBSCRIBE并自动续期
...
"""
    ...
    # FIXED: MobilePosition出站订阅 — 停止移动位置订阅，发送expires=0取消
    async def stop_mobile_position_subscribe(self, device_id: str) -> bool:
        ...
        if not asset:
            ...
            self.remove_outbound(device_id, "MobilePosition")
            return False
...
"""
    ...
    def remove_all_for_device(self, device_id: str) -> int:
        removed = 0
        ...
        for k in inbound_keys:
            ...
            # FIXED: 目录订阅自动续期 — 清理设备时取消inbound续期任务
            renew_task = self._inbound_renew_tasks.pop(k, None)
            if renew_task and not renew_task.done():
                renew_task.cancel()
            removed += 1
...
"""
    ...
    subscribe_manager.put_outbound(sub)
    try:
        await subscribe_manager.start_outbound_renew(  # FIXED: async function call must be awaited
            device_id=device_id,
            event=event,
