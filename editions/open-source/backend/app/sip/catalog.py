...
async def _catalog_agg_prune_loop():
    # FIXED: periodic cleanup of stale catalog aggregation entries to prevent memory growth
...
async def handle_catalog_response(xml_body: str, device_id: str):
    ...
    try:
        ...
        sum_num = None  # FIXED: SumNum解析异常时不应设为0（会导致多分片被丢弃），设为None
    ...
    for item in item_nodes:
        channel_id = (get_xml_text(item, "DeviceID") or "").strip()  # FIXED: None.strip()空指针异常防护
        ptz_type_str = get_xml_text(item, "PTZType")
        video_opt_mask_str = get_xml_text(item, "VideoOptMask")
        
        # 解析视频能力：VideoOptMask ?bit0 表示是否有视?        # 0 = 无视频能力，> 0 = 有视频能?        has_video: bool = True  # FIXED: 显式类型注解避免 possibly unbound
        ...
        items_data.append({
            ...
            "name": (get_xml_text(item, "Name") or "").strip(),  # FIXED: None.strip()空指针异常防护
    ...
    total_sum = sum_num
    # FIXED: SumNum解析异常时total_sum为None，使用极大值确保收集所有分片
    _total_sum_effective = total_sum if total_sum is not None else 0x7FFFFFFF
    all_items = items_data

    # Redis 分片聚合逻辑
    if redis_client is not None and sn and _total_sum_effective > 0:  # FIXED: 显式 None 守卫避免 pyright Never
        ...
        if items_data:
            ...
            # FIXED: Redis 操作添加 try-except，避免 Redis 不可用时整个 catalog 同步失败
            try:
        ...
        # FIXED: all_raw was undefined - restored lrange call with try-except
        try:
    ...
    async with AsyncSessionLocal() as session:
        ...
        for item in all_items:
            ...
            caps = {}  # FIXED: caps={} was inside comment line
            existing_caps = {}
            if resource:
