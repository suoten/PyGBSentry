...
import random
import time  # FIXED-P0: N-01 _cleanup_stale_call_ids使用time.time()但未导入
import secrets
import string
...
class SipCascadeCommander:
    ...
    def __init__(self):
        # FIXED: GB8 级联事务跟踪 — 记录级联REGISTER的Call-ID
        self._cascade_call_ids: set[str] = set()
        self._cascade_call_id_timestamps: dict[str, float] = {}  # FIXED-P2: W-18 记录call_id时间戳，用于TTL清理

    def _cleanup_stale_call_ids(self, max_age: float = 60.0) -> None:
        """FIXED-P2: W-18 清理超过max_age秒的call_id，防止内存泄漏"""
        now = time.time()
    ...
    def handle_any_response(self, message: SipMessage, addr: tuple = None, proto: str = "", transport=None) -> bool:
        """Handle cascade SIP responses. FIXED: GB8 级联响应处理"""
        ...
        if call_id and call_id in self._cascade_call_ids:
            ...
            if status_code == 401:
                ...
                if www_auth:
                    # FIXED: GB8 处理401摘要认证挑战 — 委托给PlatformService重新注册
    ...
    async def send_cascade_register(self, platform_config: dict) -> bool:
        """Send REGISTER to upstream platform. FIXED: GB8 上级平台级联注册"""
        ...
        req.headers["From"] = f"<sip:{username}@{settings.SIP_DOMAIN}>;tag={from_tag}"
        req.headers["To"] = f"<sip:{server_gb_id}@{settings.SIP_DOMAIN}>"
        call_id = f"cascade_{secrets.token_hex(8)}@{sip_host_for_contact()}"
        req.headers["Call-ID"] = call_id
        req.headers["CSeq"] = "1 REGISTER"
        ...
        self._cascade_call_id_timestamps[call_id] = time.time()  # FIXED-P2: W-18
        self._cleanup_stale_call_ids()  # FIXED-P2: W-18
    ...
    async def send_cascade_catalog_query(self, platform_config: dict, device_id: str = ""):
        """Query catalog from upstream platform. FIXED: GB8 级联目录同步"""
        from app.core.config import settings, sip_host_for_contact
        from app.sip.send import send_sip_bytes
        from app.sip.server import sip_server
        ...
        sn = secrets.randbelow(65535) + 1  # FIXED-P2: C-26 random.randint→secrets，与项目安全规范一致
        xml_body = (
        ...
        call_id = f"cascade_cat_{secrets.token_hex(8)}@{sip_host_for_contact()}"
        req.headers["Call-ID"] = call_id
    ...
    async def send_cascade_playback_invite(
        ...
        branch = f"z9hG4bK{secrets.token_hex(8)}"
        from_tag = secrets.token_hex(4)
        call_id = f"cascade_pb_{secrets.token_hex(8)}@{sip_host_for_contact()}"
        ...
        try:
            await send_sip_bytes(proto, transport, (server_ip, server_port), req.to_bytes())
