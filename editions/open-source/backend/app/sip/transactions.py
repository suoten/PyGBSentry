...
class SipServerTransaction:
    ...
    ttl: float = 32.0  # FIXED: 动态属性 _ttl 改为正式 dataclass 字段
...
class SipServerTransactionManager:
    ...
    def get_or_create(self, request: SipMessage) -> tuple[SipServerTransaction, bool]:
        ...
        tx = SipServerTransaction(key=key, created_at=time.monotonic(), request=request, timers=[], ttl=ttl)  # FIXED: 使用正式字段替代动态属性
        self._tx[key] = tx
        return tx, True

    # FIXED: INVITE事务状态机不完整 — 添加 Confirmed/Terminated 状态转换
    def update_state(self, key: str, response: SipMessage):
        tx = self._tx.get(key)
        if not tx:
            return
        ...
        if 100 <= status <= 199:
            ...
            # FIXED-P2: Confirmed是终态，不可被重传的200 OK回退，符合RFC 3261 Section 17.2.1
    ...
    # FIXED: GB7 服务端INVITE事务Timer G/H/I — RFC 3261 Section 17.2.1
    ...
    def start_timer_g(self, key: str, response: SipMessage, addr: tuple, proto: str, transport):
    ...
    def start_timer_h(self, key: str, timeout: float = 64.0):
    ...
    def start_timer_i(self, key: str, timeout: float = 5.0):
        """Timer I: Wait for ACK retransmission timeout on UDP, then delete transaction.
        Called after confirm_transaction (ACK received for 2xx)."""
        tx = self._tx.get(key)
        if not tx:
            return
        loop = asyncio.get_running_loop()
    ...
    # FIXED: INVITE事务状态机不完整 — 收到ACK时调用，Accepted -> Confirmed
    def confirm_transaction(self, key: str):
        tx = self._tx.get(key)
        if not tx:
            return
        if tx.state == "Accepted":
            tx.state = "Confirmed"
            # FIXED: GB7 收到ACK后启动Timer I（UDP下等待ACK重传超时后删除事务）
            # Timer G/H 由 invite_server_state 管理，收到ACK时其 mark_acked 会取消重传
            self.start_timer_i(key, timeout=float(getattr(settings, "SIP_TRANSACTION_T1_SECONDS", 1.0) or 1.0) * 5.0)

    # FIXED: INVITE事务状态机不完整 — Completed/Terminated 转换
    def terminate_transaction(self, key: str):
        tx = self._tx.get(key)
        if not tx:
            return
        if tx.state in ("Completed", "Confirmed", "Accepted"):
            tx.state = "Terminated"
        # FIXED: GB7 终止事务时取消所有定时器
        if tx.timers:
            for t in tx.timers:
                t.cancel()
            tx.timers = []
    # FIXED: INVITE事务状态机不完整 — Confirmed状态不再重传200 OK
    ...
    # FIXED: INVITE事务状态机不完整 — 过期事务先设为Terminated再移除
    def prune(self):
        now = time.monotonic()
        expired = []
        for k, tx in self._tx.items():
            ttl = tx.ttl  # FIXED: 直接访问 dataclass 字段
...
class SipClientTransaction:
    ...
    timers: list[asyncio.Handle] | None = None
    # FIXED-P2: S-06 分离最终超时定时器(Timer B/F)与重传定时器(Timer A/E)，
    # 1xx临时响应只取消重传定时器，保留超时定时器继续运行（RFC 3261 Section 17.1.1.2）
    timeout_timer: asyncio.Handle | None = None
    state: str = "Trying"  # FIXED: GB6 非INVITE客户端事务状态机 — RFC 3261 Section 17.1.2
...
class SipClientTransactionManager:
        ...
        *,
        ...
        for cycle in range(0, max(0, int(retries)) + 1):
            ...
            try:
                ...
                # FIXED: GB6 非INVITE事务状态转换 — 收到最终响应后设为 Completed
                async with self._lock:
    ...
    def resolve_from_response(self, response: SipMessage) -> bool:
        ...
        if method.upper() == "INVITE" and 100 <= status_code < 200:
            # FIXED: S-09 1xx 临时响应应向上传递给 TU（RFC 3261 Section 17.1.1.2）
            # FIXED-P2: S-06 只取消重传定时器(Timer A/E)，保留最终超时定时器(Timer B/F)继续运行
        ...
        if tx.timers:
            for h in tx.timers:
                h.cancel()
        # FIXED-P2: S-06 最终响应时同时取消超时定时器(Timer B/F)
        if tx.timeout_timer:
            tx.timeout_timer.cancel()
    ...
    async def send_request(self, request: SipMessage, addr: tuple, proto: str, transport) -> None:
        ...
        if is_udp:
                ...
                async def _clean():
                    async with self._lock:
                        ...
                        if tx and tx.timers:
                            for h in tx.timers:
                                h.cancel()
                        # FIXED-P2: S-06 超时定时器已触发，无需再cancel自身
            ...
            # FIXED-P2: S-06 将最终超时定时器(Timer B/F)存储在timeout_timer而非timers列表，
