...
class LocalSipStateBackend:
    def __init__(self):
        ...
        self._register_call_ids_ts: dict[str, float] = {}  # FIXED: 记录call_id写入时间戳，支持TTL过期
    ...
    async def consume_invite_rate(
        ...
        async with self._invite_rate_lock:
            # FIXED: 全局过期清理，防止 _invite_rate_buckets 字典无限增长
    ...
    async def check_nonce_nc(self, user: str, nonce: str, nc: int) -> bool:
        ...
        if nc <= last_nc:
            return False
        self._nonce_nc_tracker[key] = (nc, time.time())
    ...
    async def record_register_call_id(self, gb_id: str, call_id: str, ttl: int = 3660) -> None:
        ...
        # FIXED: _register_call_ids添加TTL过期清理，防止已注销设备的call_id永久残留
        if len(self._register_call_ids) > 10000:
            now = time.time()
            expired_keys = [k for k, ts in self._register_call_ids_ts.items() if now - ts > 7200]
            for k in expired_keys:
...
def get_sip_state_backend() -> SipStateBackend:
    ...
    with _backend_lock:
        ...
        # FIXED: 多实例部署检测告警 — CLUSTER_ENABLED但使用local后端时发出ERROR
