...
# FIXED: GB2 3xx重定向自动跟随 — 重定向计数器，防止循环
...
async def _record_stream_health(session, stream_session: StreamSession, status_code: int):
    ...
    if is_success:
        ...
        health.last_mode = _normalize_mode(stream_session.protocol)  # FIXED-P1: C-23 成功时也更新last_mode
        return
...
async def handle_invite_response(message: SipMessage, addr: tuple, proto: str, transport):
    ...
    if call_id in invite_module._CASCADE_CALL_IDS:
        return

    if 200 <= status_code < 300:
        logger.info(f"Received 200 OK for INVITE from {message.from_header}")
    elif 300 <= status_code < 400:
        # FIXED-P1: W-01 RFC 3261要求对3xx最终响应发送ACK
        try:
            _ack_req = SipMessage()
            _ack_req.method = "ACK"
        ...
        # FIXED: GB2 3xx重定向自动跟随 — RFC 3261 Section 8.1.3.4
            ...
            if contact_uri:
                ...
                if _redirect_count <= 5:
                    ...
                    try:
                        ...
                        # FIXED-P0: CSeq序列号非数字时ValueError崩溃，改为安全解析
                        try:
                        ...
                        # FIXED-P1: C-04 3xx重定向INVITE携带原始SDP，而非空body
                        _pending_entry = invite_module._INVITE_PENDING.get(call_id)
                        original_sdp = _pending_entry[1].get("original_sdp", "") if _pending_entry else ""
                        redirect_req.body = original_sdp

                        await send_sip_bytes(proto, transport, addr, redirect_req.to_bytes())
                        # FIXED-P2: S-07 3xx重定向后取消原watchdog并注册新watchdog，
    ...
    if status_code < 200:
        ...
        # FIXED: PRACK可靠1xx支持（RFC 3262）— 对带Require:100rel的1xx响应发送PRACK
    ...
    # FIXED-P1: C-21 对讲INVITE的ACK由on_talk_200_ok统一发送，此处跳过避免双重ACK
    ...
    try:
        ...
        # FIXED-P2: 对讲模块导入失败时记录日志，否则双重ACK问题无法诊断
    ...
    if not _is_talk_invite:
        ...
        if contact and "sip:" in contact:
            try:
                uri = contact.split("<")[1].split(">")[0] if "<" in contact else contact  # FIXED: Contact头解析IndexError防护
            except (IndexError, ValueError):
        ...
        req.headers["From"] = message.get_header("From") or ""  # FIXED: str|None → str
        req.headers["To"] = message.get_header("To") or ""  # FIXED: str|None → str
        ...
        try:
            ...
            if 200 <= status_code < 300:
                async def _ack_with_log():
                    try:
                        await tx_manager.send_request(req, addr, proto, transport)
                    except Exception as ack_err:
                        logger.warning(f"ACK send_request failed: {ack_err}, falling back to direct send")
                        await send_sip_bytes(proto, transport, addr, data)

                task = asyncio.create_task(_ack_with_log())
    ...
    to_tag = _extract_tag(message.get_header("To"))
    if call_id:
        _REDIRECT_COUNTS.pop(call_id, None)  # FIXED-P2: C-16 INVITE最终完成时清理重定向计数器，防止内存泄漏
        try:
            invite_module.cancel_invite_watchdog(call_id)
        except Exception as e:
            logger.warning(f"Error: {e}")
        try:
            ...
            logger.warning(f"Error: {e}")
        invite_module._STREAM_SWITCH_PENDING.pop(call_id, None)
        invite_module._STREAM_SWITCH_ROLLBACK_DEPTH.pop(call_id, None)
        invite_module._STREAM_SWITCH_PENDING_TIMESTAMPS.pop(call_id, None)

        # FIXED: 广播INVITE 200 OK响应处理 — 清理pending并记录广播会话状态
        pending_entry = invite_module._INVITE_PENDING.get(call_id)  # FIXED-P0: C-18 用get而非pop，让on_invite_response负责最终消费
        if pending_entry:
            _, pending_result = pending_entry  # FIXED-P0: C-08 _INVITE_PENDING值是tuple[Event,dict]，需解包后取dict
        ...
        invite_module.on_invite_response(
            ...
            message.body or "",
            to_tag=to_tag,
            record_route=message.get_header("Record-Route"),  # FIXED: GB28181协议 — 传递Record-Route头到dialog
        )
        # FIXED-P1: C-19 4xx-6xx错误处理 — 原elif分支因call_id非空为死代码，移入此分支
        if status_code >= 400:
            # FIXED-P1: C-25 清理对讲pending条目，否则_talk_pending永不释放
            try:
                from app.sip import talk as _talk_mod
                ...
                if _talk_entry:
                    ...
                    if _talk_ssrc:
                        try:
                            ...
                            # FIXED-P2: SSRC释放失败记录日志，否则泄漏无法追踪
            ...
            is_stream_switch = call_id in invite_module._STREAM_SWITCH_PENDING
            saved_target_type = invite_module._STREAM_SWITCH_PENDING.get(call_id)
            invite_module._STREAM_SWITCH_PENDING.pop(call_id, None)
            invite_module._STREAM_SWITCH_ROLLBACK_DEPTH.pop(call_id, None)
            invite_module._STREAM_SWITCH_PENDING_TIMESTAMPS.pop(call_id, None)
            try:
                ...
                async with AsyncSessionLocal() as session:
                    ...
                    if stream_session:
                        ...
                        if stream_session.protocol == "UDP" and dev_ip and dev_port > 0:
                            try:
                                # FIXED-P2: C-29 NAT打洞使用线程池避免阻塞事件循环
                                def _send_dummy_rtp():
                        ...
                        if dev_ssrc and dev_ssrc != stream_session.ssrc:
                            ...
                            # FIXED: SSRC冲突处理 — 释放旧SSRC并在ssrc_manager中注册新SSRC
                            try:
                                from app.sip.ssrc_manager import ssrc_manager
                                if ssrc_manager and old_ssrc:
                                    await ssrc_manager.release_ssrc(old_ssrc)  # FIXED-P0: C-07 补充await，否则协程不执行致SSRC泄漏
                                if ssrc_manager and dev_ssrc:
                                    # FIXED-P2: S-16 传递is_playback参数，根据SSRC前缀判断：以"1"开头为回放
                                    _is_playback = str(dev_ssrc).startswith("1")
                                    await ssrc_manager.allocate_specific_ssrc(dev_ssrc, is_playback=_is_playback)  # FIXED-P0: C-07
                            ...
                            try:
                                ...
                                # FIXED: removed redundant inner import of MediaNode, using module-level import
                                ...
                                if dev_port > 0:
                                    ...
                                    for attempt in range(_tcp_connect_retries):
                                        try:
                                            ...
                                            # FIXED: removed redundant inner import of MediaNode
                                            if stream_session.media_server_id:
                                                node_result = await session.execute(select(MediaNode).where(MediaNode.id == stream_session.media_server_id))
                                                node = node_result.scalars().first()
                                                if node:
                            ...
                            if node and node.ip and node.http_port:
                                ...
                                async def _on_stream_found(registered_app: str, registered_stream: str, media_item: dict):
                                    try:
                                        ...
                                        logger.warning(f"[StreamWait] Failed to update StreamSession {_session_id}: {e}")  # FIXED: update_session possibly unbound in except
                ...
                if not stream_session:
                    try:
                        ...
                        if call_id in talk_module._talk_pending:
                            # FIXED: on_talk_200_ok 已改为 async，需要 await
        ...
        call_id = message.get_header("Call-ID")
        if call_id:
            _REDIRECT_COUNTS.pop(call_id, None)  # FIXED-P2: C-16 INVITE失败时也清理重定向计数器
            # FIXED-P2: 对讲INVITE 4xx-6xx时清理_talk_pending和释放SSRC，否则资源泄漏
            try:
            ...
            if is_stream_switch and invite_module.sip_invite:
                try:
                    inviter = invite_module.sip_invite
                    if inviter:  # FIXED: pyright None narrowing
                        await inviter._do_stream_switch_rollback(call_id, saved_target_type=saved_target_type)
                except Exception as rollback_err:
                    logger.warning(f"[Stream Switch] Rollback after failure failed for {call_id}: {rollback_err}")
                return
        if call_id and invite_module.sip_invite:
            retried = False  # FIXED: retried possibly unbound
            inviter = invite_module.sip_invite
            if inviter:
                ...
                try:
                    async with AsyncSessionLocal() as session:
                        ...
                        if stream_session:
                            ...
                            await _record_stream_health(session, stream_session, message.status_code)
                            from app.services.stream_session_service import finalize_stream_session
                            await finalize_stream_session(session, stream_session, reason=f"invite_failed_{int(message.status_code or 0)}")  # FIXED: 移除内部 loguru import，统一使用模块级 logger
                            await session.commit()
                except Exception as e:
                    logger.error(f"invite_failed cleanup DB error for {call_id}: {e}")