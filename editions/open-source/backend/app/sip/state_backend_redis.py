...
class RedisSipStateBackend:
    ...
    def _get_redis(self) -> Any:  # FIXED: 添加返回类型注解，避免 pyright 推断为 Never
        try:
    ...
    async def register_ssrc_waiter(self, ssrc: str) -> None:
        r = self._get_redis()
        if r is None:  # FIXED: 显式 None 守卫，避免 pyright reportGeneralTypeIssues
    ...
    async def check_register_renewal(self, gb_id: str, call_id: str) -> bool:
        ...
        try:
            ...
            if existing and (existing.decode("utf-8") if isinstance(existing, bytes) else str(existing)) == call_id:  # FIXED: 运算符优先级导致注册续期判断错误
                return True
            return False
        except Exception:
