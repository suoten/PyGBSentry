...
if not shutil.which("ffmpeg"):
    _safe_print("\n" + "="*65)
    _safe_print("WARNING: FFmpeg not detected. Snapshots, AI inference and recording may be limited!")  # FIXED: 硬编码中文→英文
    _safe_print("SUGGEST: Install FFmpeg, e.g. `apt install ffmpeg` (Debian/Ubuntu).")  # FIXED: 硬编码中文→英文
    _safe_print("="*65 + "\n")
...
logger.add(f"{_log_dir}/app.log", rotation="50 MB", retention="7 days", level="INFO", format=_log_format, enqueue=True)
...
async def lifespan(app: FastAPI):
    ...
    if use_alembic:
        ...
        alembic_cfg.set_main_option("sqlalchemy.url", app_settings.SQLALCHEMY_DATABASE_URI)
    ...
    if not bool(getattr(settings, "RUN_SPLIT_CATALOG_MIGRATIONS_ON_STARTUP", False)):
        logger.info(
            ...
            "To migrate an old database, set to true in .env or run: python scripts/run_split_catalog_migrations.py"  # FIXED: i18n - replaced hardcoded Chinese with English
        )
    else:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    ...
                    "or increase SQLITE_CONNECT_TIMEOUT_SECONDS in .env."  # FIXED: i18n - replaced hardcoded Chinese with English
                )
                resource_rows = -1
            except Exception as e:
                logger.warning(
    ...
    if not bool(getattr(settings, "RUN_REGION_SEED_ON_STARTUP", False)):
        logger.info(
            ...
            "To use built-in regions, set to true in .env or run: python scripts/seed_regions.py"  # FIXED: i18n - replaced hardcoded Chinese with English
        )
    else:
    ...
    if not bool(getattr(settings, "ENSURE_EMBEDDED_MEDIA_NODE_ON_STARTUP", False)):
        logger.info(
            ...
            "If initial_data has not been run or node needs to be populated: python scripts/ensure_embedded_media_node.py"  # FIXED: i18n - replaced hardcoded Chinese with English
        )
    else:
        ...
        try:
            ...
            logger.warning(
                ...
                "If this occurs frequently, check: lsof ./pygbsentry.db for multiple processes using the database"  # FIXED: i18n - replaced hardcoded Chinese with English
            )
        except Exception as e:
    ...
    if not bool(getattr(settings, "INIT_REDIS_ON_STARTUP", False)):
        logger.info(
            ...
            "Set INIT_REDIS_ON_STARTUP=true in .env and start redis-server when Redis is needed"  # FIXED: 硬编码中文→英文
        )
    else:
    ...
    # FIXED: start catalog aggregation periodic prune to prevent memory growth
    try:
    ...
    if not getattr(settings, "PLUGIN_LICENSE_MACHINE_CODE_ENABLED", False):
        _security_warnings.append("PLUGIN_LICENSE_MACHINE_CODE_ENABLED=False: machine code binding disabled, license can be copied across machines")  # FIXED: 硬编码中文→英文
    if not getattr(settings, "PLUGIN_LICENSE_ACTIVATION_TOKEN_ENABLED", False):
        _security_warnings.append("PLUGIN_LICENSE_ACTIVATION_TOKEN_ENABLED=False: activation token disabled, trial period can be reset")  # FIXED: 硬编码中文→英文
    if not getattr(settings, "PLUGIN_PACKAGE_INTEGRITY_REQUIRED_IN_PROD", False):
        _security_warnings.append("PLUGIN_PACKAGE_INTEGRITY_REQUIRED_IN_PROD=False: package signature verification disabled, plugin packages can be tampered")  # FIXED: 硬编码中文→英文
    if _security_warnings and (getattr(settings, "APP_ENV", "dev") or "dev").lower() in {"prod", "production"}:
        for _w in _security_warnings:
            logger.warning("[Security] %s", _w)  # FIXED: 硬编码中文→英文
        logger.warning("[Security] The above anti-piracy layers are disabled by default. Enable them in .env for production. See BUSINESS_MODEL_FIXES.md FIX-02")  # FIXED: 硬编码中文→英文
...
_enable_docs = bool(getattr(settings, "ENABLE_OPENAPI_DOCS", True))
_docs_enabled = _enable_docs and _env not in {"prod", "production"}
...
app = FastAPI(
    ...
    version=settings.PROJECT_VERSION,  # FIXED: 版本号从settings读取，与config.py保持一致
    lifespan=lifespan,
...
# 使用运行时生成的指纹代替硬编码密钥值，避免源码中暴露真实密钥
import hashlib as _hashlib
import os as _os

def _compute_default_key_fingerprint(key_value: str) -> str:
    return _hashlib.sha256(key_value.encode()).hexdigest()[:16]

# FIXED: 默认密钥指纹 — 使用运行时计算替代硬编码密钥值，避免源码暴露真实密钥
_DEFAULT_SECRET_KEY_FINGERPRINT = "e8f3a1b9c7d24e60"
# FIXED: 不在源码中硬编码默认ZLM密钥值，仅从环境变量读取；未设置则跳过指纹检测
_DEFAULT_ZLM_SECRET_FOR_FINGERPRINT = _os.environ.get("_PYGBSENTRY_DEFAULT_ZLM_SECRET", "")
_DEFAULT_MEDIA_SECRET_FINGERPRINT = _compute_default_key_fingerprint(_DEFAULT_ZLM_SECRET_FOR_FINGERPRINT) if _DEFAULT_ZLM_SECRET_FOR_FINGERPRINT else ""
_DEFAULT_PASSWORDS = {
...
def _is_using_default_secret(actual_key: str, fingerprint: str) -> bool:
    if not actual_key:
        return False
    return _hashlib.sha256(actual_key.encode()).hexdigest()[:16] == fingerprint

if (getattr(settings, "APP_ENV", "dev") or "dev").lower() in {"prod", "production"}:
    if _is_using_default_secret(settings.SECRET_KEY, _DEFAULT_SECRET_KEY_FINGERPRINT):
        raise RuntimeError("SECURITY: SECRET_KEY is using the repository default. Please set SECRET_KEY via environment variable in production.")  # FIXED: i18n - replaced hardcoded Chinese with English
    if _DEFAULT_MEDIA_SECRET_FINGERPRINT and _is_using_default_secret(getattr(settings, "MEDIA_SERVER_SECRET", ""), _DEFAULT_MEDIA_SECRET_FINGERPRINT):
        raise RuntimeError("SECURITY: MEDIA_SERVER_SECRET is using the repository default. Please set MEDIA_SERVER_SECRET via environment variable in production.")  # FIXED: i18n - replaced hardcoded Chinese with English
    # 仅当实际使用需要密码的数据库时才强制检查默认密码；SQLite 不应阻断启动
    ...
    if _db_type not in {"sqlite"}:
        for _pwd_key in ["POSTGRES_PASSWORD", "DATABASE_PASSWORD"]:
            ...
            if _pwd_val in _DEFAULT_PASSWORDS or _pwd_val == "":
                raise RuntimeError(f"SECURITY: {_pwd_key} is using a default or empty password. Please set a strong password in production.")  # FIXED: i18n - replaced hardcoded Chinese with English
    # 检查 SIP 默认密码
    _sip_pwd = str(getattr(settings, "SIP_DEFAULT_PASSWORD", "") or "").strip().lower()
    if _sip_pwd in _DEFAULT_PASSWORDS or _sip_pwd == "":
        raise RuntimeError("SECURITY: SIP_DEFAULT_PASSWORD is using a default or empty password. Please set a strong password in production.")  # FIXED: i18n - replaced hardcoded Chinese with English
    # 检查是否开启公开注册
    if bool(getattr(settings, "ALLOW_PUBLIC_REGISTRATION", False)):
        raise RuntimeError("SECURITY: ALLOW_PUBLIC_REGISTRATION is enabled, which may allow unauthorized user registration. It is recommended to disable public registration in production.")  # FIXED: i18n - replaced hardcoded Chinese with English
else:
    # 非生产环境：SECRET_KEY 未配置或使用默认值时发出警告
    if _is_using_default_secret(settings.SECRET_KEY, _DEFAULT_SECRET_KEY_FINGERPRINT):
        import logging as _logging
        _logging.getLogger(__name__).warning(
            "SECRET_KEY is using a default value; each restart generates a new key, invalidating all JWTs. "
            "It is recommended to set a fixed SECRET_KEY in .env."  # FIXED: i18n - replaced hardcoded Chinese with English
        )
...
if settings.BACKEND_CORS_ORIGINS:
    ...
    if _cors_env in {"prod", "production"}:
        lowered = [o.lower() for o in _cors_origins]
        if any("localhost" in o or "127.0.0.1" in o for o in lowered):
            raise RuntimeError("SECURITY: BACKEND_CORS_ORIGINS should not contain localhost/127.0.0.1 in production")  # FIXED: i18n - replaced hardcoded Chinese with English
    app.add_middleware(
        CORSMiddleware,
...
# FIXED: ZlmApiError继承RuntimeError但有status_code字段，添加异常处理器使FastAPI正确响应
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    if isinstance(detail, dict):
        payload = dict(detail)
        message = str(payload.get("message") or payload.get("detail") or "Request failed")  # FIXED: 硬编码中文→英文
        payload.setdefault("detail", message)
        payload.setdefault("message", message)
        payload.setdefault("status_code", exc.status_code)
        return JSONResponse(status_code=exc.status_code, content=payload)
    message = str(detail or "Request failed")  # FIXED: 硬编码中文→英文
    if exc.status_code == 405:
        message = "Method not allowed"
    return JSONResponse(
        status_code=exc.status_code,
        content={"detail": message, "message": message, "status_code": exc.status_code},
    )

@app.exception_handler(StarletteHTTPException)
async def starlette_exception_handler(request: Request, exc: StarletteHTTPException):
    message = str(exc.detail or "Request failed")  # FIXED: 硬编码中文→英文
    if exc.status_code == 405:
        message = "Method not allowed"
    return JSONResponse(
        status_code=exc.status_code,
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    detail_msg = str(exc) if settings.APP_ENV == "dev" else "Internal server error"  # FIXED: 硬编码中文→英文
    return JSONResponse(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        content={
            "detail": "Internal server error",  # FIXED: 硬编码中文→英文
            "message": detail_msg,
            "reason_code": "internal_server_error",
            "suggestion": "Check backend logs, media node status and device connections",  # FIXED: 硬编码中文→英文
            "retryable": True,
            "status_code": status.HTTP_500_INTERNAL_SERVER_ERROR,
        },
    )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    errors = exc.errors()
    detail = "Validation error"  # FIXED: 硬编码中文→英文
    if errors:
        loc = errors[0].get("loc", [])
        if len(loc) > 1 and isinstance(loc[-1], str):
            detail = f"Parameter '{loc[-1]}' validation failed"  # FIXED: 硬编码中文→英文
    return JSONResponse(
...
if __name__ == "__main__":
    ...
    uvicorn.run("app.main:app", host="0.0.0.0", port=port, reload=_env == "dev", loop="asyncio", timeout_keep_alive=settings.UVICORN_TIMEOUT_KEEP_ALIVE)