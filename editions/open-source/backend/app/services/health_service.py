...
class HealthService:
        ...
        async with AsyncSessionLocal() as session:
            ...
            for n in nodes:
                ...
                if getattr(n, "is_embedded", False):
                    host = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "").strip()  # FIXED: I3 回退值不再硬编码127.0.0.1，避免配置缺失时静默使用错误地址
                ...
                try:
                    ...
                    for dev in devices:
                        if dev.status == 0 and dev.gb_id:
                            subscribe_manager.remove_all_for_device(dev.gb_id)
                except Exception as e:
    ...
    async def _probe_device_before_offline(self, device) -> bool:
        try:
            ...
            sn = int(time.time() * 1000) % 100000 # FIXED: __import__ 反模式改为标准 import
