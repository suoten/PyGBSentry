...
# FIXED-P2: S-11 每个session_id一把锁，防止release和finalize并发清理
...
async def stop_cascade_push_session(db: AsyncSession, stream_session: StreamSession, reason: str) -> None:
    ...
    # FIXED: close_stream suppress is acceptable - best-effort cleanup, failure logged by close_stream itself
...
async def release_stream_session(db: AsyncSession, stream_session: StreamSession, reason: str = "", wait_bye_response: bool = True) -> None:
    ...
    # FIXED-P2: S-11 获取session级别锁，防止与finalize并发清理
    ...
    async with lock:
        try:
            ...
            try:
                ...
                # FIXED: contextlib.suppress(Exception) → explicit log for Asset query failure
            ...
            if reason == "media_stream_not_ready" or reason == "media_node_unreachable" or reason == "invite_timeout":
                ...
                try:
                    ...
                    # FIXED: contextlib.suppress(Exception) → explicit log for stream health record
            ...
            if stream_session.resource_id:
                try:
                    ...
                    # FIXED: contextlib.suppress(Exception) → explicit log for Resource query failure
            ...
            if asset and sip_invite_module.sip_invite:
                try:
                    ...
                    # FIXED: contextlib.suppress(Exception) → explicit log for BYE send failure
            ...
            if ssrc_val:
                try:
                    ...
                    # FIXED: contextlib.suppress(Exception) → explicit log for SSRC release failure
                    logger.warning(f"[ReleaseStreamSession] Failed to release SSRC {ssrc_val}: {e}")
            # FIXED-P0: S-05 释放流会话时终止Dialog，防止僵尸Dialog干扰后续INVITE
            try:
            ...
            # FIXED-P1: C-24 使用SQL delete替代ORM delete，避免commit后对象detached
...
async def finalize_stream_session(db: AsyncSession, stream_session: StreamSession, reason: str = "") -> None:
    ...
    # FIXED-P2: S-11 获取session级别锁，防止与release并发清理
    _sid = str(stream_session.id)
    lock = _session_locks.get(_sid)
    if lock is None:
        lock = asyncio.Lock()
