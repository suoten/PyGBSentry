...
def recommend_stream_mode(
    ...
    if consecutive_failures >= 2:
        return "UDP", "High consecutive failures, recommend UDP for stability", "high"  # FIXED: 硬编码中文→英文
    if failure_rate >= 35:
        return "UDP", "High failure rate, recommend UDP", "high"  # FIXED: 硬编码中文→英文
    if auto_switch_count > 0 and failure_rate >= 15:
        return "UDP", "Self-heal history with unconverged failure rate, recommend UDP", "medium"  # FIXED: 硬编码中文→英文
    if last in {"TCP_PASSIVE", "TCP_ACTIVE"} and consecutive_failures == 0 and failure_rate <= 10:
        return "TCP_PASSIVE", "TCP quality stable, recommend TCP_PASSIVE", "low"  # FIXED: 硬编码中文→英文
    if mode == "TCP_ACTIVE" and failure_rate > 20:
        return "TCP_PASSIVE", "TCP_ACTIVE variance high, recommend downgrade to TCP_PASSIVE", "medium"  # FIXED: 硬编码中文→英文
    return mode, "Maintain current mode", "low"  # FIXED: 硬编码中文→英文
...
def should_probe_back_to_tcp_passive(
    ...
    if datetime.now(timezone.utc) - updated_at > timedelta(minutes=max_idle_minutes):
        return False, ""
    return True, f"Switch-back probe condition met: success={success_total}, failure_rate={failure_rate}%"  # FIXED: 硬编码中文→英文


# FIXED: 码流自适应自动执行 — 从策略推荐升级为自动执行切换
...
# FIXED: GB11 码流自适应切换 — 根据网络质量自动切换主/子码流
class StreamStrategy:
    ...
    async def _switch_to_sub_stream(self, session, db) -> bool:
        ...
        try:
            ...
            from app.sip.invite import send_invite_for_channel
            from app.models.resource import Resource
            from app.models.asset import Asset
            from sqlalchemy import select
            ...
            await send_invite_for_channel(asset, resource, stream_type="sub", db=db)
            return True
        except Exception as e:
    ...
    async def _switch_to_main_stream(self, session, db) -> bool:
        ...
        try:
            ...
            if not resource or not asset:
                return False
            await release_stream_session(db, session, reason="bitrate_adaptive_switch")
            await send_invite_for_channel(asset, resource, stream_type="main", db=db)
            return True
        except Exception as e:
