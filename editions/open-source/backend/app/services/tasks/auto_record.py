...
async def _select_media_node(db) -> tuple[str, int, str]:
    proxy_host = getattr(settings, "MEDIA_SERVER_HOST", "127.0.0.1")
    proxy_http_port = int(getattr(settings, "MEDIA_SERVER_HTTP_PORT", 0) or 0)
    proxy_secret = getattr(settings, "MEDIA_SERVER_SECRET", "")
...
async def _start_record(proxy_host: str, proxy_http_port: int, proxy_secret: str, stream: str) -> None:
    ...
    async with httpx.AsyncClient(timeout=5) as client:
        r = await client.get(url, params={"secret": proxy_secret, "vhost": "__defaultVhost__", "app": "live", "stream": stream, "type": 1})
