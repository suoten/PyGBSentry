...
class AuditCenterService:
    async def _push_to_external_siem(self, payload: dict) -> None:  # FIXED-P2: W-05 改为async，使用httpx异步请求替代同步requests.post
        """
    ...
    async def log(
        ...
        # 异步推送至态势感知/SIEM
        try:
            asyncio.create_task(self._push_to_external_siem(payload))  # FIXED-P2: W-05 直接异步调用，不再需要to_thread
        except Exception as e:
