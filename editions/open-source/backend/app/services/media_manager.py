...
class MediaManager:
    ...
    def __init__(self):
        ...
        self.zlm_path = ""
        self.config_path = ""
        # FIXED: W-15 _effective_rtp_range 未初始化 — 在 __init__ 中初始化为 None，访问时检查
    ...
    async def _download_package(self, url: str, dest_file: str) -> None:
        ...
        # FIXED: 添加try-except，下载失败时抛出明确错误而非让异常泄露
        try:
    ...
    async def start(self):
        ...
        """
        # FIXED-P2: N-07 monitor重启前检查_running标志，防止与stop()竞态
        ...
        # FIXED: _generate_config contains time.sleep() in _ensure_port_free, must run in executor to avoid blocking event loop
    ...
    async def _probe_webrtc_after_start(self) -> None:
        ...
        try:
            ...
            host = str(getattr(settings, "MEDIA_SERVER_HOST", "") or "").strip()  # FIXED: I3 回退值不再硬编码127.0.0.1
            url = f"http://{host}:{port}/index/api/webrtc"
            # FIXED: SDP o= line hardcoded 127.0.0.1 → use host from settings
            placeholder_sdp = (
                "v=0\r\n"
                f"o=- 0 0 IN IP4 {host}\r\n"
                "s=-\r\n"
