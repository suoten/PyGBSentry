...
async def init_db():
    ...
    async with AsyncSessionLocal() as session:
        ...
        # FIXED: removed --password= CLI support to prevent password exposure in process list (ps aux)
        ...
        if not user:
            ...
            # FIXED: removed password fingerprint from logs to prevent offline brute-force
            logger.warning("=" * 60)
        ...
        try:
            ...
            if not any_mn:
                ...
                embedded = MediaNode(
                    ip=str(getattr(settings, "MEDIA_SERVER_HOST", "127.0.0.1") or "127.0.0.1"),
                    public_ip=None,
...
def _parse_password_from_args():
    # FIXED: removed CLI password parsing to prevent exposure in process list
