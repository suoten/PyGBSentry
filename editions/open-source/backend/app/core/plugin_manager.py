...
# FIXED-P0: S-06 — 应用源码包根目录，用于校验 app.* 模块是否为真正的内部模块
...
class _PluginSandboxImportHook:
    ...
    def load_module(self, fullname):
        raise ImportError(
            ...
            f"which is in the dangerous API blacklist and has been blocked by runtime sandbox"  # FIXED: i18n - replaced hardcoded Chinese with English
        )
...
# FIXED: P4 死代码 — 删除重复定义（第一个 _install_plugin_sandbox_builtin_guard 已被下面的完整版本替代）
...
# FIXED-P0: S-06 — 移除 "app." 前缀白名单，改为 _is_internal_caller 中文件路径校验
...
def _is_internal_caller(caller: str, frame) -> bool:
    ...
    FIXED-P0: S-06 — 对 app/app.* 模块增加文件路径校验，
...
_sandbox_guard_lock = threading.Lock()  # FIXED-P0: 改为threading.Lock，_load_module是sync函数不能用async with
                        ...
                        try:
                            ...
                            if mode not in ("r", "rb"):
                                raise RuntimeError(
                                    ...
                                    f"file writing is a dangerous API and has been blocked by runtime sandbox"  # FIXED: i18n - replaced hardcoded Chinese with English
                                )
                            return orig(*args, **kwargs)
                        except RuntimeError:
                            raise
...
def _hook_process_runner(
    ...
    - FIXED-P2: W-13 使用JSON序列化替代pickle传递args/kwargs，防止反序列化漏洞
    """
    try:
        # FIXED-P2: W-13 使用安全的JSON反序列化替代pickle
...
class PluginManager:
    ...
    def _load_module(self, module_name, meta=None):
        _was_in_modules = module_name in sys.modules  # FIXED-P2: W-04 记录导入前状态，失败时清理残留
        try:
            ...
            if module_name in sys.modules:
                ...
                if vendor_dir and os.path.isdir(vendor_dir):
                    ...
                    inserted = True
                try:
                    with _sandbox_guard_lock:  # FIXED-P0: 修复async with语法错误（sync函数内不能用async with）+ FIXED-P2: W-03 加锁保护guard安装/卸载
            ...
            if hasattr(module, "register"):
                # FIXED-P2: S-21 热重载前清理旧Hook，防止回调重复执行
            ...
            # FIXED-P2: W-04 清理新导入但加载失败的模块残留，防止下次reload使用损坏的模块对象
        ...
        try:
            ...
            if requirements_file and os.path.isfile(requirements_file):
                ...
                if result.returncode != 0:
                    return {"success": False, "venv_dir": venv_dir, "error": f"pip install failed: {result.stderr[:200]}"}  # FIXED: 中文错误消息→英文

            return {"success": True, "venv_dir": venv_dir, "error": None}
        except subprocess.TimeoutExpired:
            return {"success": False, "venv_dir": venv_dir, "error": "Dependency installation timed out"}  # FIXED: 中文错误消息→英文
        except Exception as e:
    ...
    def _paid_plugin_license_currently_valid(self, plugin_id: str, meta: dict) -> bool:
        ...
        if not ok:
            logger.warning(
                "Paid plugin %s hook suspended: license verification failed (will retry per PLUGIN_PAID_HOOK_LICENSE_RECHECK_SECONDS)",  # FIXED: i18n - replaced hardcoded Chinese with English
                pid,
            )
        return ok
    ...
    def _paid_plugin_license_recheck_now(self, plugin_id: str, meta: dict) -> bool:
        ...
        if not ok:
            logger.warning(
                "Paid plugin %s hook suspended: license verification failed (background sync triggered recheck)",  # FIXED: i18n - replaced hardcoded Chinese with English
                pid,
            )
        return ok
    ...
    def _invalidate_plugin_menu_cache(self) -> None:
        ...
        logger.debug("_invalidate_plugin_menu_cache: menu cache invalidated")  # FIXED: i18n - replaced hardcoded Chinese with English
    ...
    async def emit(self, hook_name: str, *args, **kwargs):
        ...
        if hook_name.upper() == "ON_SIP_SEND" or hook_name.upper() == "ON_SIP_RECV":  # FIXED: Hook名称大小写不匹配修复，统一转大写比较
            try:
                msg = args[0]
        ...
        async def _invoke_one(callback: Callable):
            ...
            async def _run():
                ...
                if vendor_dir:
                    sys.path.insert(0, vendor_dir)
                    inserted = True
                with _sandbox_guard_lock:  # FIXED-P2: W-03 emit()中沙箱guard安装/卸载加锁，防止并发修改builtins/os全局状态
                ...
                try:
                    if timeout_seconds > 0:
                        if inspect.iscoroutinefunction(callback):
                            ...
                            if hook_mode == "process":
                                ...
                                if module_name and func_name:
                                    try:
                                        import json as _json
                                        _json.dumps(args)  # FIXED-P2: W-13 用JSON可序列化检查替代pickle检查
                                        _json.dumps(kwargs)
                                    except Exception:
                                        hook_mode = "thread"
                                    try:
                                ...
                                if hook_mode == "process":
                                    def _run_in_subprocess():
                                        ...
                                        q = ctx.Queue()
                                        # FIXED-P2: W-13 使用JSON序列化替代pickle
                        ...
                        # FIXED: S-05 sync callbacks always run in thread pool to prevent
                    ...
                    with _sandbox_guard_lock:  # FIXED-P2: W-03 guard卸载也加锁
                        _uninstall_plugin_sandbox_hook(sandbox_hook)
                        _uninstall_plugin_sandbox_builtin_guard(saved_builtins)
                        _uninstall_plugin_sandbox_os_attr_guard(saved_os_attrs)
    ...
    async def register_oss_instance(self, machine_code: str, description: str | None = None) -> dict:
        ...
        if not server_url:
            logger.warning("[OSS-Register] PLUGIN_MARKETPLACE_SERVER_URL not configured, skipping instance registration")  # FIXED: i18n - replaced hardcoded Chinese with English
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, data=body_bytes, headers={
                        ...
                        if resp.status == 200:
                            ...
                            logger.info(f"[OSS-Register] Instance registered successfully: {self._oss_instance_id}")  # FIXED: i18n - replaced hardcoded Chinese with English
                            return {"ok": True, **data}
                        else:
                            text = await resp.text()
                            logger.warning(f"[OSS-Register] Registration failed HTTP {resp.status}: {text[:200]}")  # FIXED: i18n - replaced hardcoded Chinese with English
                            return {"ok": False, "error": f"HTTP {resp.status}: {text[:200]}"}
            except aiohttp.ClientError as exc:
                logger.warning(f"[OSS-Register] Network error (attempt {attempt + 1}): {exc}")  # FIXED: i18n - replaced hardcoded Chinese with English
                if attempt == 1:
                ...
                async with session.post(url, data=body_bytes, headers={
                    ...
                    **headers,
                    ...
                    if resp.status == 200:
                        ...
                        logger.info("[OSS-Register] Instance deregistered successfully")  # FIXED: i18n - replaced hardcoded Chinese with English
                        return {"ok": True}
                    return {"ok": False, "error": f"HTTP {resp.status}: {text[:200]}"}
        except aiohttp.ClientError as exc:
            return {"ok": False, "error": str(exc)}
        ...
        for candidate in [path, path + ".bak"]:
            try:
                ...
                if data.get("instance_id") and data.get("instance_secret"):
                    ...
                    logger.info(f"[OSS-Register] Restored instance info from {candidate}: {self._oss_instance_id}")  # FIXED: i18n - replaced hardcoded Chinese with English
                    return
            except FileNotFoundError:
                continue
            except Exception as exc:
                logger.warning(f"[OSS-Register] Failed to load instance info ({candidate}): {exc}")  # FIXED: i18n - replaced hardcoded Chinese with English
                continue
    ...
    def save_oss_instance_info(self):
        ...
        try:
            ...
            logger.info(f"[OSS-Register] Instance info saved to {path}")  # FIXED: i18n - replaced hardcoded Chinese with English
        except Exception as exc:
            logger.warning(f"[OSS-Register] Failed to save instance info: {exc}")  # FIXED: i18n - replaced hardcoded Chinese with English
    ...
    def record_plugin_error(self, plugin_id: str, error: str = "") -> None:
        ...
        if threshold > 0 and entry["errors"] >= threshold:
            entry["disabled"] = True
            logger.warning(
                "Plugin %s has %d consecutive errors (threshold %d), auto-disabled",  # FIXED: i18n - replaced hardcoded Chinese with English
    ...
    def check_plugin_disk_limit(self, plugin_id: str) -> bool:
        ...
        if usage_mb > limit_mb:
            logger.warning("Plugin %s disk usage %dMB exceeds limit %dMB", plugin_id, usage_mb, limit_mb)  # FIXED: i18n - replaced hardcoded Chinese with English
            return False
        return True
    ...
    async def start_license_refresh_subscriber(self) -> None:
        ...
        if not bool(getattr(settings, "PLUGIN_MARKETPLACE_ENABLED", False)):
            ...
            return
        redis_url = getattr(settings, "REDIS_URL", None) or getattr(settings, "REDIS_HOST", "")  # FIXED: I3 回退值不再硬编码localhost
        if not redis_url:
            return
        try:
            import aioredis
