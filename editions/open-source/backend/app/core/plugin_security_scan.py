...
def scan_zip_for_security_risks(zip_ref: zipfile.ZipFile, file_list: list[str]) -> list[str]:
    ...
    """
    if not bool(getattr(settings, "PLUGIN_SECURITY_SCAN_ENABLED", False)):
        return []

    max_files = int(getattr(settings, "PLUGIN_SECURITY_SCAN_MAX_FILE_COUNT", 200) or 0)
    max_bytes = int(getattr(settings, "PLUGIN_SECURITY_SCAN_MAX_FILE_BYTES", 200_000) or 0)
    max_hits = int(getattr(settings, "PLUGIN_SECURITY_SCAN_MAX_HITS", 20) or 0)
