...
class RedisDict:
    ...
    async def keys(self) -> list[str]:
        ...
        # FIXED: replaced KEYS with SCAN to avoid blocking Redis
        all_keys = []
        cursor = 0
        while True:
    ...
    async def size(self) -> int:
        _ensure_redis()
        # FIXED: replaced KEYS with SCAN count to avoid blocking Redis
        count = 0
        cursor = 0
        pattern = f"{self._prefix}:*"
        while True:
...
class FallbackDict:
    ...
    def __init__(self, key_prefix: str, ttl: int | None = None):
        ...
        self._prefix = key_prefix
        self._was_fallback: bool = False  # FIXED: P2 状态一致性 — Redis恢复后同步内存数据

    def _redis_available(self) -> bool:
        return redis_client is not None

    async def _sync_to_redis(self) -> None:
        """FIXED: P2 状态一致性 — Redis恢复后同步内存数据"""
