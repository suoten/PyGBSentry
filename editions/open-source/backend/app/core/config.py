...
class Settings(BaseSettings):
    ...
    PROJECT_VERSION: str = "1.0.0"  # FIXED: I5 版本号应从 pyproject.toml 或 __version__.py 同步；当前开源版无 pyproject.toml，此值为唯一来源，变更时需同步更新
    ...
    ENABLE_AUTO_DISCOVERY: bool = True  # 是否允许未知设备自动注册并建库
    ENABLE_OPENAPI_DOCS: bool = True  # FIXED-P4: 默认开启API文档，与wvp-GB28181-pro对齐
    ...
    # CORS
    BACKEND_CORS_ORIGINS: List[AnyHttpUrl] = []  # FIXED: 移除硬编码CORS，强制env配置
    ...
    ENSURE_EMBEDDED_MEDIA_NODE_ON_STARTUP: bool = True  # FIXED-P4: 默认开启，启动时自动补全内置ZLM节点，与wvp对齐
    ...
    def assemble_db_connection(cls, v: str | None, info: ValidationInfo) -> str | None:
        ...
        if db_type in {"postgres", "postgresql", "kingbase", "kingbasees"}:  # FIXED: 移除中文别名"人大金仓"，保留英文别名
        ...
        if db_type in {"sqlite"}:
            ...
            return f"sqlite+aiosqlite:///{normalized}"
        if db_type in {"dm", "dameng", "damengdb"}:  # FIXED: 移除中文别名"达梦"，保留英文别名
            _safe_user = quote_plus(str(user or ""), safe="")
            _safe_pwd = quote_plus(str(password or ""), safe="")
            return f"mysql+aiomysql://{_safe_user}:{_safe_pwd}@{host}:{port}/{name}"
        return PostgresDsn.build(
    ...
    # FIXED: Redis max_connections now configurable via env (default 50)
    ...
    REDIS_SENTINEL_HOSTS: str = ""  # FIXED: Redis Sentinel地址列表，格式: host1:port1,host2:port2,host3:port3
    REDIS_SENTINEL_MASTER: str = "mymaster"  # FIXED: Redis Sentinel master名称
    REDIS_SENTINEL_PASSWORD: str = ""  # FIXED: Redis Sentinel密码
    REDIS_CLUSTER_MODE: bool = False  # FIXED: 是否使用Redis Cluster模式
    ...
    SIP_PORT: int = 5060
    SIP_WS_PORT: int = 0  # FIXED: SIP over WebSocket端口(0=禁用)
    # FIXED: 移除重复的GB28181_VERSION定义（L162），保留L198的完整注释版本
    # SIPS (TLS) Support
    ...
    GB28181_VERSION: str = "2016"  # FIXED: GB28181协议版本(2016/2022)，影响SDP a=track行等
    GB28181_STREAM_SWITCH_USE_TRACK_SUBJECT: bool = True  # FIXED-P4: 默认开启GB28181-2022码流切换track标识，与wvp对齐
    GB28181_PLAYBACK_SDP_TIME_FORMAT: str = "iso"  # FIXED: 回放SDP时间格式(iso/epoch)
    GB28181_VIDEO_QUALITY: int = 0  # FIXED: 视频质量等级(0=主码流,1=子码流)
    GB28181_CATALOG_SUBSCRIBE_EXPIRES: int = 3600  # FIXED: 目录订阅过期时间(秒)
    GB28181_MOBILE_POSITION_SUBSCRIBE_INTERVAL: int = 5  # FIXED: 移动位置订阅间隔(秒)
    GB28181_DEVICE_STATUS_QUERY_INTERVAL: int = 300  # FIXED: 设备状态查询间隔(秒)
    GB28181_ALARM_SUBSCRIBE_EXPIRES: int = 3600  # FIXED: 报警订阅过期时间(秒)
    ...
    # FIXED: S-03 default RTP range aligned with Docker deployment (30000-30199);
    # non-Docker deployments can override via env var to use wider range
    MEDIA_SERVER_RTP_PROXY_PORT_RANGE: str = "30000-30199"
    ...
    SNAPSHOT_REFRESH_ENABLED: bool = True  # FIXED-P4: 默认开启定时快照刷新，与wvp对齐
    ...
    AUTO_DETECT_PUBLIC_IP: bool = True  # FIXED-P4: 默认开启公网IP探测，支持公网部署，与wvp对齐
    ...
    CHANNEL_TREE_INFER_REGION_PLACEMENT: bool = True  # FIXED-P4: 默认开启注册即归类，与wvp对齐
    ...
    ZLM_STREAM_NONE_READER_DELAY_MS: int = 10000  # FIXED: reduced from 600000ms (10min) to 10s to let app-layer control delay
    ...
    PLAY_ALLOW_NO_TOKEN: bool = True  # FIXED-P4: 默认开启无Token播放，支持通过流地址直接观看，与wvp对齐
    AUTO_PLAY_ENABLED: bool = True  # FIXED-P4: 默认开启自动邀请，访问流URL时自动INVITE，与wvp对齐
    ...
    ZLM_WRITE_RTC_SECTION: bool = True  # FIXED-P4: 默认开启WebRTC配置段，与wvp对齐
    # 仅当 ZLM_WRITE_RTC_SECTION=true 时生效
    MEDIA_SERVER_RTC_PORT: int = 8000  # FIXED-P4: 默认WebRTC端口8000，与wvp/ZLM默认配置对齐
    ...
    # VOD / 录像配置
    VOD_ENABLE_PRELOAD: bool = True  # FIXED-P4: 默认开启VOD预加载，提升回放体验，与wvp对齐
    ...
    # Performance
    SIP_WORKER_CONCURRENCY: int = 200  # FIXED: .env.example中写1000，需保持一致
    ...
    PLUGIN_MARKETPLACE_BASE_URL: str = ""  # FIXED: 移除硬编码域名，强制env配置
    ...
    PAYMENT_SUCCESS_RETURN_URL: str = ""  # FIXED: 移除硬编码域名，强制env配置
...
# FIXED: SQLite not suitable for production - block startup in production environment
if _db_type == "sqlite":
    _app_env = (getattr(settings, "APP_ENV", "dev") or "dev").lower()
    if _app_env in {"prod", "production"}:
        import logging as _logging
...
# SIP_DEFAULT_PASSWORD 生产环境空值阻断  # FIXED: was warning-only, now blocks startup in production
...
# FIXED: SIP_STATE_BACKEND=local in production warns about multi-instance nonce/replay inconsistency
...
# FIXED: 启动时检测默认SIP_ID/SIP_DOMAIN，发出告警避免多套部署信令冲突
...
# FIXED: RTP端口范围一致性校验 — 检测 MEDIA_SERVER_RTP_PROXY_PORT_RANGE 与 Docker 端口映射是否匹配
