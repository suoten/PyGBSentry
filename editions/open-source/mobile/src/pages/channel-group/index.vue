...
const treeOptions = computed(() => flattenTree(treeNodes.value).filter((x) => x.id && x.id !== "root"));
...
const summaryText = computed(() => {
  return `业务分组通道：模式=${mode.value === "added" ? "已挂载" : "未挂载"}；目标=${selectedTargetId.value || "无"}；总数=${total.value}；本页=${channels.value.length}；已选=${selectedIds.value.length}`;
});

const nextStepAdvice = computed(() => {
  if (mode.value === "added" && !selectedTargetId.value) return "下一步建议：先选择业务节点，再查看已挂载通道。";
  if (selectedIds.value.length <= 0) return "下一步建议：勾选通道后可批量挂载或移除。";
  return mode.value === "added" ? "下一步建议：可批量移除选中通道，治理冗余挂载。" : "下一步建议：可批量挂载选中通道到当前业务节点。";
});
...
async function loadTree() {
  try {
    const rows = await fetchDeviceTree("business");
...
async function mountSelected() {
  if (!selectedTargetId.value) {
    uni.showToast({ title: "请先选择业务节点", icon: "none" });
    return;
  }
...
function copySummary() {
  ...
  uni.setClipboardData({
    data: text,
    success: () => uni.showToast({ title: "业务分组摘要已复制", icon: "none" }),
...
<template>
  <view class="app-page app-gap-12">
    <view class="app-title">业务分组</view>
    ...
    <view class="app-card app-gap-12">
      ...
      <picker mode="selector" :range="treeOptions.map((x)=>x.label)" :value="Math.max(0, treeOptions.findIndex((x)=>x.id===selectedTargetId))" @change="(e:any)=>onTargetChange(Number(e?.detail?.value || 0))">
        <view class="app-subtext">业务节点：{{ selectedTargetLabel }}</view>
      </picker>
      ...
      <view v-if="channels.length" class="app-gap-12">
        <view v-for="row in channels" :key="row.id" class="app-row">
          <view style="flex:1">
            ...
            <text class="app-subtext">业务父节点：{{ row.parent_gb_id || "-" }}</text>
          </view>
