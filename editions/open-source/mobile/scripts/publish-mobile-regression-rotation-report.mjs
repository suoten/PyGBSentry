...
const statusPath = process.env.MOBILE_REGRESSION_ROTATION_PUBLISH_STATUS_PATH || "scripts/mobile-regression.rotation-publish-status.json";
const webhookUrl = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_URL || "";
const webhookToken = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_TOKEN || "";
const webhookUrlProd = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_URL_PROD || "";
const webhookUrlCanary = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_URL_CANARY || "";
const webhookTokenProd = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_TOKEN_PROD || "";
const webhookTokenCanary = process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_TOKEN_CANARY || "";
const extraWebhookUrls = String(process.env.MOBILE_REGRESSION_ROTATION_REPORT_WEBHOOK_URLS || "")
  .split(",")
  .map((x) => x.trim())
  .filter((x) => !!x);
const retries = Math.max(0, Number(process.env.MOBILE_REGRESSION_ROTATION_PUBLISH_RETRIES || 1));
const retryDelayMs = Math.max(0, Number(process.env.MOBILE_REGRESSION_ROTATION_PUBLISH_RETRY_DELAY_MS || 1000));
const signingSecret = process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNING_SECRET || "";
const signatureHeader = process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_HEADER || "X-PyGBSentry-Signature";
const signatureTimestampHeader =
  process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_TIMESTAMP_HEADER || "X-PyGBSentry-Signature-Timestamp";
const signatureNonceHeader = process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_NONCE_HEADER || "X-PyGBSentry-Signature-Nonce";
const signatureAlgorithmHeader =
  process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_ALGORITHM_HEADER || "X-PyGBSentry-Signature-Algorithm";
const signatureEncodingHeader =
  process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_ENCODING_HEADER || "X-PyGBSentry-Signature-Encoding";
const signatureVersionHeader =
  process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_VERSION_HEADER || "X-PyGBSentry-Signature-Version";
const signatureKeyIdHeader =
  process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_KEY_ID_HEADER || "X-PyGBSentry-Signature-Key-Id";
...
const signatureVersion = process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_VERSION || "v1";
const signatureKeyId = process.env.MOBILE_REGRESSION_ROTATION_REPORT_SIGNATURE_KEY_ID || "";

function log(message) {
  console.log(`[publish-mobile-regression-rotation-report] ${message}`);
}
...
const envRouteToken = policyEnv === "prod" ? webhookTokenProd || webhookToken : policyEnv === "canary" ? webhookTokenCanary || webhookToken : webhookToken;
...
if (targets.length === 0) {
  writeStatus({ ok: false, skipped: true, reason_code: "WEBHOOK_NOT_CONFIGURED" });
  log("skip: webhook not configured");
  process.exit(0);
}
...
for (const target of targets) {
  ...
  for (let attempt = 1; attempt <= retries + 1; attempt += 1) {
    ...
    throw new Error(`publish failed: http ${res.status} target=${target.url} channel=${target.channel}`);
  }
  if (sent) {
    log(`report published target=${target.url} channel=${target.channel}`);
  }
}
