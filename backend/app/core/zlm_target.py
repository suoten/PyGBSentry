...
async def _resolve_from_db(db: AsyncSession, node_id: str | None = None):
    """Try to resolve ZLM target from database nodes."""
    ...
    if node_id:
        ...
        if db_node:
            return (
                ...
                # db_node 为 RuntimeMediaNode，.secret 已是解密后的明文（由 _to_runtime 解密）
    ...
    try:
        if db_session is None:
            async with AsyncSessionLocal() as db:
                result = await _resolve_from_db(db, node_id)
        else:
            result = await _resolve_from_db(db_session, node_id)
        if result:
            return result
