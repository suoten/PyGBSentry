...
# ---------------------------------------------------------------------------
# SQL DDL statement groups — organized by domain/table
# ---------------------------------------------------------------------------

_USER_AND_TENANT_STMTS = [
...
_MEDIA_NODES_AND_PLATFORMS_STMTS = [
    """
...
# All statement groups in execution order
_ALL_STMT_GROUPS = [
    _USER_AND_TENANT_STMTS,
    _MEDIA_AND_STREAM_STMTS,
    _DEVICE_RECORD_STMTS,
...
def _build_all_statements():
    """Flatten all statement groups into a single list in execution order."""
    return [stmt for group in _ALL_STMT_GROUPS for stmt in group]
...
async def _execute_ddl_statements(statements, dialect, conn):
    """Execute a list of DDL statements with dialect-aware error handling.

    This helper handles SQLite's lack of ``ADD COLUMN IF NOT EXISTS`` and
    MySQL's lack of ``CREATE INDEX IF NOT EXISTS``, skipping benign duplicate
    errors so the migration is idempotent across all supported databases.
    """
        ...
        try:
            ...
            if dialect == "sqlite":
                ...
                if "duplicate column name" in msg:
                    _log_skip(idx, total, started, "duplicate column", stmt_preview)
                    continue
                if "no such column" in msg and "create index" in exec_stmt.strip().lower():
                    _log_skip(idx, total, started, "index (no such column)", stmt_preview)
                    continue
                if "no such table" in msg and "alter table" in exec_stmt.strip().lower():
                    _log_skip(idx, total, started, "alter table (no such table)", stmt_preview)
                    continue
                if "no such table" in msg and "create index" in exec_stmt.strip().lower():
                    _log_skip(idx, total, started, "index (no such table)", stmt_preview)
                    continue
            if is_mysql_create_index:
                msg = str(getattr(e, "orig", e)).lower()
                if "duplicate key name" in msg or "duplicate index name" in msg:
                    _log_skip(idx, total, started, "duplicate index", stmt_preview)
                    continue
            elapsed_ms = int((time.perf_counter() - started) * 1000)
            logger.exception(
...
def _log_skip(idx, total, started, reason, stmt_preview):
    """Log a skipped DDL statement with elapsed time."""
    elapsed_ms = int((time.perf_counter() - started) * 1000)
    logger.debug(
...
async def ensure_business_schema():
    ...
    This function runs at startup to guarantee the database schema matches
    the ORM models. It performs:

    1. **Column additions**: Add missing columns to existing tables using
    ...
    DDL statements are organized into domain-specific groups for
    maintainability. See ``_ALL_STMT_GROUPS`` for the execution order.
    """
    ensure_model_registry_loaded()
    statements = _build_all_statements()
    dialect = (getattr(engine, "dialect", None) and engine.dialect.name) or ""
    dialect = str(dialect).lower()
    ...
    async with engine.begin() as conn:
        ...
        await _execute_ddl_statements(statements, dialect, conn)
