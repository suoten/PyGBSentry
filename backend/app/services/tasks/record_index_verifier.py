...
async def _run_loop():
    interval = max(5, settings.RECORD_INDEX_VERIFY_INTERVAL_SECONDS)
    batch_size = max(1, min(settings.RECORD_INDEX_VERIFY_BATCH_SIZE, 200))
    max_age_days = max(1, settings.RECORD_INDEX_VERIFY_MAX_AGE_DAYS)
    while True:
        if not settings.RECORD_INDEX_VERIFY_ENABLED:
