...
async def _refresh_once():
    if not bool(getattr(settings, "SNAPSHOT_REFRESH_ENABLED", False)):
        return
    now = datetime.now()
    if not _in_window(
        now,
        int(getattr(settings, "SNAPSHOT_REFRESH_WINDOW_START_HOUR", 0) or 0),
        int(getattr(settings, "SNAPSHOT_REFRESH_WINDOW_END_HOUR", 0) or 0),
    ):
        return
    ttl = int(getattr(settings, "SNAPSHOT_REFRESH_TTL_SECONDS", 86400) or 86400)
    limit = max(1, int(getattr(settings, "SNAPSHOT_REFRESH_MAX_PER_CYCLE", 20) or 20))
    prefer_existing = bool(getattr(settings, "SNAPSHOT_REFRESH_PREFER_EXISTING", True))
    allow_invite = bool(getattr(settings, "SNAPSHOT_REFRESH_ALLOW_INVITE", False))
...
async def _loop():
    interval = max(30, int(getattr(settings, "SNAPSHOT_REFRESH_INTERVAL_SECONDS", 1800) or 1800))
    while True:
        try:
            await _refresh_once()
