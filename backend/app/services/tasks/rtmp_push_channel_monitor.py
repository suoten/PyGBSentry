...
async def _run_loop():
    interval = max(2, settings.PUSH_CHANNEL_MONITOR_INTERVAL_SECONDS)
    grace = max(0, settings.PUSH_CHANNEL_OFFLINE_GRACE_SECONDS)
    while True:
        if not settings.PUSH_CHANNEL_MONITOR_ENABLED:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                enforce_stopped = settings.PUSH_CHANNEL_ENFORCE_STOPPED
