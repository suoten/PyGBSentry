...
async def _run_loop():
    interval = max(2, settings.PULL_PROXY_MONITOR_INTERVAL_SECONDS)
    grace = max(0, settings.PULL_PROXY_OFFLINE_GRACE_SECONDS)
    auto_retry = settings.PULL_PROXY_AUTO_RETRY_ENABLED
    max_retry = max(0, settings.PULL_PROXY_AUTO_RETRY_MAX_COUNT)
    while True:
        if not settings.PULL_PROXY_MONITOR_ENABLED:
