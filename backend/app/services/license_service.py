    ...
    if USE_NATIVE_VERIFY:
        try:
            return _native_verify_license_core(
                ...
                machine_code_enabled=settings.PLUGIN_LICENSE_MACHINE_CODE_ENABLED,
                activation_token_enabled=settings.PLUGIN_LICENSE_ACTIVATION_TOKEN_ENABLED,
                grace_period_seconds=settings.PLUGIN_LICENSE_OFFLINE_GRACE_PERIOD_SECONDS,
            )
...
def _verify_license_payload_python(
    ...
    if expires_at and now > expires_at:
        grace_seconds = settings.PLUGIN_LICENSE_OFFLINE_GRACE_PERIOD_SECONDS
        if grace_seconds == -1:
    ...
    payload["sig_alg"] = "ed25519"
    if settings.PLUGIN_LICENSE_MACHINE_CODE_ENABLED:
        ...
        if license_machine_code:
            ...
            if license_machine_code != current_machine_code:
                ...
                if instance_id:
                    ...
                    return False, "machine_code_mismatch"
    if settings.PLUGIN_LICENSE_ACTIVATION_TOKEN_ENABLED:
...
def plugin_manifest_signing_public_key_pem() -> str:
    ...
    m = (settings.PLUGIN_MANIFEST_ED25519_PUBLIC_KEY or "").strip()
    if m:
        return m
    return (settings.LICENSE_ED25519_PUBLIC_KEY or "").strip()
...
def manifest_signature_install_error(metadata: dict[str, Any] | None) -> str | None:
    ...
    req = settings.PLUGIN_MANIFEST_SIGNATURE_REQUIRED
