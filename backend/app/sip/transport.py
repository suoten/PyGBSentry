"""P2-1: 已废弃模块 — 仅向后兼容保留，SipServer 不使用本模块。

SipServer 使用自有的 UdpProtocol 和 _handle_tcp_client 处理传输层。
本模块将在未来版本中删除，请勿在新代码中引用。
"""
    ...
    def datagram_received(self, data: bytes, addr: tuple):
        ...
        try:
            ...
            if coro is not None:  # FIXED: message_callback 可能返回 None
                fire_and_forget(coro)  # P0-16: 保存引用防 GC + 异常日志
    ...
    def data_received(self, data: bytes):
        ...
        while b"\r\n\r\n" in self.buffer:
            ...
            for line in headers_str.split("\r\n"):
                if line.lower().startswith("content-length:"):
                    ...
                    try:
                        ...
                        logger.warning("Invalid Content-Length value in SIP TCP message, closing connection")
                        self._closing = True
                        if self.transport:  # FIXED: transport 可为 None
                            self.transport.close()
                        return
            ...
            if not found_content_length:
                logger.warning("Missing Content-Length header in SIP over TCP message, closing connection per RFC 3261")
                self._closing = True
                if self.transport:  # FIXED: transport 可为 None
                    self.transport.close()
                return
