...
class SipMessage:
    def __init__(self):
        ...
        self._body_cache: str | None = None  # I11 body解码缓存，避免每次访问重新解码
    ...
    def body(self, value: Union[str, bytes]):
        self._body_cache = None  # I11 body被重新赋值时清除缓存
    ...
    def parse(data: bytes) -> 'SipMessage':
        ...
        if content_length:
            try:
                ...
                if n >= 0:
                    # Content-Length不匹配时记录告警，便于排查NVR报文问题
    ...
    _MULTI_VALUE_HEADERS = frozenset({"via", "contact", "record-route", "route", "www-authenticate"})  # R-02 添加www-authenticate支持RFC 8760多认证方案
