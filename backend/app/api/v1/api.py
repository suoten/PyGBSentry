...
if not is_server_edition:
    ...
    api_router.include_router(sip_trace_ws.router, tags=["sip-trace"])
    ...
    api_router.include_router(network_diagnostics.router, prefix="/network-diagnostics", tags=["network-diagnostics"])  # A-12 前缀从/network改为/network-diagnostics，避免与network路由冲突
    ...
    api_router.include_router(ssl_cert.router, prefix="/ssl-cert", tags=["ssl-cert"])  # 硬编码中文tag→英文
