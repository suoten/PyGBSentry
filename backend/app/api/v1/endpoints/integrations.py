...
def _build_hook_base_url(node: MediaNode | None) -> str:
    ...
    def _ensure_hook_suffix(base_url: str) -> str:
    ...
    # 不应跳过。只有外置节点才需要跳过 loopback URL 回退到公网地址。
    is_embedded = bool(getattr(node, "is_embedded", False)) if node else False
    if node:
        hook_base = (getattr(node, "hook_base_url", None) or "").strip()
        if hook_base:
            if is_embedded or not _is_loopback_url(hook_base):
                return _ensure_hook_suffix(hook_base)
        hook_ip = (getattr(node, "hook_ip", None) or "").strip()
        if hook_ip:
            if is_embedded or not _is_loopback_host(hook_ip):
                return f"http://{hook_ip}:{settings.BACKEND_PUBLIC_PORT}{settings.API_V1_STR}/hook"
        node_ip = (getattr(node, "ip", None) or "").strip()
        if node_ip:
            if is_embedded or not _is_loopback_host(node_ip):
                return f"http://{node_ip}:{settings.BACKEND_PUBLIC_PORT}{settings.API_V1_STR}/hook"
    global_base = (settings.MEDIA_SERVER_HOOK_BASE_URL or "").strip()
    if global_base:
        if is_embedded or not _is_loopback_url(global_base):
            return _ensure_hook_suffix(global_base)
    if not _is_loopback_host(settings.BACKEND_PUBLIC_HOST):
...
def _build_zlm_hook_urls(base: str, secret: str) -> dict[str, str]:
    """生成 ZLM hook URL 字典，每个 URL 携带 URL-encoded secret 查询参数。

    FIX [2026-07-17 P0]: 原 [2026-07-16] 注释声称 "secret 通过请求体传递" 是错误的——
    FastAPI 的 ``verify_zlm_secret(secret: str | None = None)`` 作为 ``Depends`` 时，
    ``secret`` 参数被 FastAPI 自动绑定为 URL 查询参数（简单类型 + 默认值）；
    且 hook.py 内所有端点都通过 ``request.query_params.get("secret")`` 读取 secret。
    原实现返回不含 secret 的 URL，导致外置 ZLM 节点发起的 hook 回调全部 403。

    硬约束 "ZLMediaKit API 调用必须通过 POST 请求体传递 secret" 仅适用于
    PyGBSentry → ZLM 的 *出站* API 调用（如 openRtpServer），不适用于
    ZLM → PyGBSentry 的 *入站* hook 回调（ZLM 框架无法在 hook POST body 中追加 secret，
    只能通过 hook URL 的查询字符串传递）。参考内置 ZLM 的实现
    ``media_manager.py:1281-1290``，同样在 hook URL 后追加 ``?secret=...``。

    ZLM 框架对 hook URL 中含 ``?`` 与 ``&`` 的处理方式：原样发起 HTTP POST，
    不会丢弃 query string。FastAPI ``request.query_params.get("secret")``
    ...
    return {
        ...
        "on_server_keepalive": f"{base}/on_server_keepalive?secret={_q}",
        "on_play": f"{base}/on_play?secret={_q}",
        "on_publish": f"{base}/on_publish?secret={_q}",
        "on_stream_changed": f"{base}/on_stream_changed?secret={_q}",
        "on_stream_none_reader": f"{base}/on_stream_none_reader?secret={_q}",
        "on_record_mp4": f"{base}/on_record_mp4?secret={_q}",
    }
...
def _build_zlm_config_snippet(node: MediaNode, hook_base_url: str) -> str:
    ...
    """
    # FIX [2026-07-17 P0]: node.secret 列存 AES-256-GCM 密文，必须用 decrypted_secret 取明文，
    # 否则生成的 config.ini 片段 [api] secret 为密文，ZLM 以密文鉴权 → API 401。
    secret = (getattr(node, "decrypted_secret", None) or "").strip()
    ...
    record_file_second = int(getattr(node, "record_file_second", 0) or settings.ZLM_RECORD_FILE_SECOND)
    record_sample_ms = int(getattr(node, "record_sample_ms", 0) or settings.ZLM_RECORD_SAMPLE_MS)
    protocol_mp4_max_second = int(getattr(node, "protocol_mp4_max_second", 0) or settings.ZLM_PROTOCOL_MP4_MAX_SECOND)
        ...
        out.append(
            {
                ...
                # FIX [2026-07-17 P0]: 必须导出明文 secret，外置 ZLM 节点才能用此值鉴权；
                # node.secret 列存 AES-256-GCM 密文，直接导出会导致外置 ZLM 配置错误。
                "secret": getattr(node, "decrypted_secret", None) or "",
            }
        )
    return {"items": out}
        ...
        items.append(
            {
                ...
                "public_host": public_host,
                "public_http_port": public_http_port,
                # FIX [2026-07-17 P0]: env 导出同样需要明文 secret
                "secret": getattr(node, "decrypted_secret", None) or "",
            }
        )
...
async def create_media_node(
    ...
    # FIX: [2026-07-16 P0] 原直接赋值明文到密文列 secret，绕过加密 setter，
    # 导致数据库存储明文而非 AES-256-GCM 密文，违反等保 2.0 要求。
    # 改用 decrypted_secret setter 触发自动加密。
    node.decrypted_secret = payload.secret.strip()
    db.add(node)
    await db.commit()
...
async def update_media_node(
    ...
    # FIX: [2026-07-16 P0] 使用 decrypted_secret setter 加密，禁止直接赋值明文到密文列
    node.decrypted_secret = payload.secret.strip()
...
async def get_media_node_zlm_ssl_status(
    ...
    result = await db.execute(select(MediaNode).where(MediaNode.id == node_id))
    node = get_or_404(result, detail="MediaNode not found")  # ORM查询结果空值判断
    configured = bool((getattr(node, "zlm_ssl_merged_pem", None) or "").strip())
    return {
        "configured": configured,
        "is_embedded": bool(node.is_embedded),
        "hint": "Certificate is written to the binary directory with -s flag when built-in ZLMediaKit starts; restart backend/media service to apply.",  # i18n
    }
...
async def put_media_node_zlm_ssl(
    ...
    if not bool(node.is_embedded):
        ...
        raise HTTPException(
            status_code=400,
            detail="Only embedded media nodes can save ZLM SSL: certificates are loaded by the embedded MediaServer at startup.",  # i18n
        )
    try:
...
async def get_media_node_zlm_hook_urls(
    ...
    return {
        "node_id": node.id,
        "hook_base_url": base,
        # FIX [2026-07-17 P0]: node.secret 列存 AES-256-GCM 密文，传入 _build_zlm_hook_urls
        # 应使用明文（虽然当前实现未将 secret 拼入 URL，但保留明文以备未来按 ZLM 规范扩展）。
        "hook_urls": _build_zlm_hook_urls(base, getattr(node, "decrypted_secret", None) or ""),
    }
...
async def create_access_source(
    ...
    source = AccessSource(
        ...
        # FIX: [2026-07-16 P0] password 列在模型上声明为加密密文列，
        # 直接赋值明文会绕过加密。先创建对象再通过 setter 赋值加密。
    ...
    # FIX: [2026-07-16 P0] 通过 setter 触发 AES-256-GCM 加密
    source.decrypted_password = (payload.password or "").strip() or None
    db.add(source)
    await db.commit()
...
async def update_access_source(
    ...
    if password:
        # FIX: [2026-07-16 P0] 使用 setter 加密，禁止直接赋值明文到密文列
        source.decrypted_password = password
...
async def get_rtmp_push_url(
    ...
    if not current_user.is_superuser:
        stmt = stmt.where(AccessSource.tenant_id == (current_user.tenant_id or "default"))
    result = await db.execute(stmt)
    source = get_or_404(result, detail="AccessSource not found")  # ORM查询结果空值判断
    ...
    rtmp_host = settings.STREAM_PUBLIC_HOST or settings.MEDIA_SERVER_HOST
...
async def set_access_source_enabled(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Save failed")  # i18n
    await safe_auth_audit(
        db,
        module="integrations",
        action="set_access_source_enabled",
...
async def play_access_source(
    ...
    if protocol == "RTSP":
        ...
        media_host = settings.STREAM_PUBLIC_HOST or settings.MEDIA_SERVER_HOST
        media_port = settings.STREAM_PUBLIC_HTTP_PORT or settings.MEDIA_SERVER_HTTP_PORT
        selection_reason = "global"
        try:
            active_id = await get_active_media_node_id(db)
            db_node = await get_db_media_node_by_id(db, active_id) if active_id else None
