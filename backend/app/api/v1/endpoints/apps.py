...
async def create_app_log(
    ...
    current_user: User = Depends(deps.get_current_active_user),  # FIX: [2026-07-16 P1] 原无认证，任何人可写入日志导致 DoS
):
    """
    接收 App 端上报的崩溃/行为日志。
    """
...
async def list_app_logs(
    ...
    skip: int = Query(0, ge=0),  # 添加Query约束
    limit: int = Query(50, ge=1, le=10000),  # 添加Query约束
    db: AsyncSession = Depends(get_db),
...
async def app_log_stats(
    request: Request,
    plugin_id: Optional[str] = None,
    days: int = Query(1, ge=1, le=365),  # 添加Query约束
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
):
