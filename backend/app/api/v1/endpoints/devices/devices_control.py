...
router = APIRouter()
# FIX [2026-07-22 P0]: 使用 loguru logger（L29 已导入），不覆盖为 stdlib logging。
# stdlib logging 的日志不会写入 app.log（main.py 只配置了 loguru sink），
# 导致 [SYNC_CHANNELS] 等诊断日志丢失。loguru logger 会写入 app.log。
# logger = logging.getLogger(__name__)  # 已禁用
...
async def _try_snap_async(host: str, http_port: int, secret: str, app: str, stream: str, cache_file: str, attempts: int = 2) -> bool:
    ...
    for _ in range(zlm_attempts):
        ...
        try:
            ...
            r = await (await get_http_client()).post(api, data=params, timeout=6)  # 同步requests→异步httpx，避免阻塞事件循环
...
async def get_channel_snapshot(
    ...
    existing_snap_timeout = settings.SNAPSHOT_EXISTING_TIMEOUT_SECONDS
    invite_snap_timeout = settings.SNAPSHOT_INVITE_TIMEOUT_SECONDS
    db_query_ms = 0.0
    ...
    async def _find_existing_stream_node() -> tuple[dict | None, str, str]:
        ...
        for n in nodes:
            ...
            try:
                http_port = int(n.get("http_port") or 0)  # 防止http_port非数字时ValueError
            except ValueError:
                http_port = 0
...
async def batch_channel_snapshot(
    ...
    item_timeout_seconds: float = Query(
        settings.SNAPSHOT_BATCH_ITEM_TIMEOUT_SECONDS,
        ge=5.0,
        le=120.0,
    ),
):
    ...
    """
    if payload is None:
        raise HTTPException(status_code=400, detail='Request body cannot be empty, example: {"channel_ids":["34020000001320000001"]}')  # i18n
    channel_ids = list(dict.fromkeys(str(x).strip() for x in payload.channel_ids if str(x).strip()))
    if not channel_ids:
        return {"status": "ok", "count": 0}

    sem = asyncio.Semaphore(max(1, settings.SNAPSHOT_BATCH_CONCURRENCY))
    stats = {"ok": 0, "failed": 0}
    ...
    # FIX [2026-07-17 P1-E2]: 启用 return_exceptions=True，单个截图失败不取消其他并发任务
    _snap_results = await asyncio.gather(*tasks, return_exceptions=True)
    for _r in _snap_results:
        if isinstance(_r, Exception):
            logger.warning(f"Batch snapshot task failed: {_r}")
...
async def device_time_sync(
    ...
    return {"status": "ok", "message": "Time sync command sent"}  # i18n
...
async def sync_device_channels(
    ...
    # FIX [2026-07-22 P0]: asset.ip_addr 为空时，按优先级从多个来源恢复地址：
    #   1. ParentPlatform.server_ip（REGISTER 成功后由 enqueue_register_update 更新）
    #   2. Keepalive 源地址缓存（MESSAGE 处理器收到 Keepalive 时缓存的源 IP:port）
    # 根因：EasyGBS 注册时 enqueue_register_update 可能因事务回滚/并发未更新 Asset.ip_addr，
    # 而 Keepalive 处理器出于安全考虑不更新 ip_addr（防设备劫持），导致 ip_addr 永远为空。
    # 修复：从后备来源获取地址后，回填 Asset.ip_addr，避免下次再走兜底逻辑。
    if not asset.ip_addr:
        _recovered_ip = ""
        _recovered_port = 0
        _recovered_proto = ""

        # 来源1：ParentPlatform 表
        platform_stmt = select(ParentPlatform).where(
            or_(
                ParentPlatform.server_gb_id == device_id,
                ParentPlatform.client_gb_id == device_id,
            )
        )
        platform_result = await db.execute(platform_stmt)
        platform = platform_result.scalars().first()
        if platform and platform.server_ip:
            _recovered_ip = platform.server_ip
            _recovered_port = platform.server_port or 5060
            _recovered_proto = "UDP"
            logger.info(f"[SYNC_CHANNELS] device_id={device_id} recovered addr from ParentPlatform: {_recovered_ip}:{_recovered_port}")

        # 来源2：Keepalive 源地址缓存
        if not _recovered_ip:
            _last_seen = get_device_last_seen_addr(device_id)
            if _last_seen:
                _recovered_ip, _recovered_port, _recovered_proto = _last_seen
                logger.info(f"[SYNC_CHANNELS] device_id={device_id} recovered addr from Keepalive cache: {_recovered_ip}:{_recovered_port}")

        if _recovered_ip:
            asset.ip_addr = _recovered_ip
            asset.port = _recovered_port or asset.port or 5060
    ...
    # FIX [2026-07-22 P0]: 改为后台执行，避免阻塞 HTTP 请求导致前端超时。
    # _schedule_device_catalog_retry 内部重试延迟 [5, 5, 5] 秒总等待 15 秒，
    # 同步 await 会导致 HTTP 请求阻塞 15 秒，前端通常 10~15 秒就超时。
    # 改为 fire-and-forget 后台执行，API 立即返回，前端通过 catalog_sync_runtime
    # 状态（progress/sync_state）轮询进度。
    logger.info(f"[SYNC_CHANNELS] device_id={device_id} ip={asset.ip_addr}:{asset.port} proto={asset.transport}")

    # FIX [2026-07-22 P2]: 统一使用项目 fire_and_forget（自带异常回调+任务名+防 GC），
    # 替代裸 asyncio.create_task + 手工 done_callback。
    fire_and_forget(
        _schedule_device_catalog_retry(device_id, transport_info),
        name=f"catalog_sync:{device_id}",
    )
...
async def config_download(
    ...
    """
    # 补充ConfigDownload API端点，SIP层已有但API层缺失，与wvp对齐
    import app.sip.commander as sip_commander_module
