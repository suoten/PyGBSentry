...
async def _try_snap_async(host: str, http_port: int, secret: str, app: str, stream: str, cache_file: str, attempts: int = 2) -> bool:
    ...
    for _ in range(zlm_attempts):
        ...
        try:
            r = await (await get_http_client()).get(api, params=params, timeout=6)  # 同步requests→异步httpx，避免阻塞事件循环
...
async def get_channel_snapshot(
    ...
    async def _find_existing_stream_node() -> tuple[dict | None, str, str]:
        ...
        for n in nodes:
            ...
            try:
                r = await (await get_http_client()).get(url, params={"secret": sec, "app": "live"}, timeout=0.6)  # 同步requests→异步httpx，避免阻塞事件循环
                ...
                with contextlib.suppress(Exception):
                    ...
                    async def _get_reader_count():
                        try:
                            ...
                            r = await (await get_http_client()).get(url, params={"secret": sec, "app": invited_app, "stream": invited_stream}, timeout=1)  # 同步requests→异步httpx，避免阻塞事件循环
                            if r.status_code == 200:
                                data = r.json()
