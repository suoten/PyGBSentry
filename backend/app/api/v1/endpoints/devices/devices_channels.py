...
async def get_channels_flat(
    ...
    if keyword:
        cond = or_(Resource.gb_id.ilike(f"%{_escape_ilike(keyword)}%"), Resource.name.ilike(f"%{_escape_ilike(keyword)}%"))
...
async def get_business_tree(
    ...
    # /tree/business 端点全量查询无缓存 → 使用内存缓存，TTL 30秒
    ...
    # /tree/business 端点结果无大小限制 → 添加最大节点数限制
    if len(resources) > _MAX_TREE_NODES:
...
async def create_directory(
    ...
    if not gb_id or not name:
        raise HTTPException(status_code=400, detail="Node code and node name cannot be empty")  # i18n
    if len(gb_id) > 20:
...
async def rename_directory(
    ...
    name = (payload.name or "").strip()
    if not gb_id or not name:
        raise HTTPException(status_code=400, detail="Node code and name are required")  # i18n
