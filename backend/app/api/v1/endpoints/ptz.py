...
async def control_ptz(
    ...
    if not sent_ok:
        await safe_auth_audit(
            ...
            extra_summary=(f"channel_id={channel_id}; lr={left_right}; ud={up_down}; io={in_out}; move_speed={move_speed}; zoom_speed={zoom_speed}"),
        )
...
async def absolute_ptz(
    ...
    sent_ok = await sip_commander_module.sip_commander.send_absolute_ptz_cmd(
        device_id=asset.gb_id, channel_id=resource.gb_id, transport_info=transport_info, pan=pan, tilt=tilt, zoom=zoom
    )
    if not sent_ok:
        await safe_auth_audit(
            db,
...
async def control_preset(
    channel_id: str,
    cmd_type: int = Query(
        ..., description="0x81=设置预置位 0x82=调用预置位 0x83=删除预置位 (兼容历史值 8=设置)"
    ),  # FIX: [2026-07-03] 对齐 GB28181 标准字节 [全栈工程师]
    ...
    sent_ok = await sip_commander_module.sip_commander.send_raw_ptz_cmd(
        device_id=asset.gb_id, channel_id=resource.gb_id, transport_info=transport_info, ptz_cmd=ptz_cmd
    )
    if not sent_ok:
        await safe_auth_audit(
            db,
