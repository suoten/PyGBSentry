...
async def _stream_record_download(
    ...
    if parsed.scheme in {"http", "https"}:
        ...
        def _iter_remote():
            try:
                for chunk in upstream.iter_bytes(chunk_size=1024 * 256):  # S-08-01 httpx使用iter_bytes而非iter_content(requests API)
                    if chunk:
                        yield chunk
            finally:
                upstream.close()
    ...
    if not local_path.exists() or not local_path.is_file():
        ...
        raise HTTPException(status_code=404, detail="Record file not found (may be on external media node or already cleaned)")  # i18n
    ...
    def _iter_file():
        try:
            ...
            raise HTTPException(status_code=404, detail="Record file not found") from e  # 文件I/O异常保护
...
async def query_device_record(
    ...
    if not row:
        await _record_query_audit(
            ...
            result="failed",
            status_code=404,
            detail="Device or channel not found or permission denied",  # i18n
            extra_summary=f"device_id={device_id}; channel_id={channel_id}",
        )
        raise HTTPException(status_code=404, detail="Device or channel not found or permission denied")  # i18n
...
async def download_record(
    ...
    res = get_or_404(await db.execute(select(Resource).where(Resource.id == row.resource_id)), detail="Resource not found")  # ORM查询结果空值判断
...
async def verify_records_batch(
    ...
    ids = ids[:200]
    if not ids:
        raise HTTPException(status_code=400, detail="ids cannot be empty")  # i18n
    tenant_id = current_user.tenant_id or "default"
    ...
    for r in rows:
        ...
        ok, status_code, error = await _verify_url_or_path(url, getattr(r, "zlm_file_path", None))  # 同步requests→异步httpx，避免阻塞事件循环
...
async def repair_record_url(
    ...
    if not new_url:
        raise HTTPException(status_code=400, detail="Cannot repair: missing media_node_id or cannot deduce path")  # i18n
