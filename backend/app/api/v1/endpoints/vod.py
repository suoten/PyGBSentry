...
async def vod_play(
    ...
    # 1. 查询录像记录
    stmt = (
        select(Record, Resource, Asset)
        .join(Resource, Resource.id == Record.resource_id)
        .join(Asset, Asset.id == Record.asset_id)
    ...
    if not current_user.is_superuser and asset.tenant_id != (current_user.tenant_id or "default"):
        raise HTTPException(status_code=403, detail="Permission denied to access this recording")  # i18n
    ...
    # 5. 创建监控会话
    session_id = await vod_quality_monitor.create_session(record_id=record_id, device_id=asset.gb_id, channel_id=resource.gb_id, sources=sources)
    ...
    return VodPlayResponse(
        ...
        sources={"primary": best_source, "alternates": [s for s in sources if s.get("url") != best_source.get("url")], "all": sources},
...
async def get_vod_sources(
    ...
    checked_sources = []
    for src in sources:
        candidate = {**src, "available": src.get("available", True), "health_score": src.get("health_score", 100.0)}
        checked_sources.append(candidate)

    # 推荐源
    recommended = await vod_source_selector.select_best_source(sources)

    return {"record_id": record_id, "sources": checked_sources, "recommended": recommended, "count": len(checked_sources)}
...
async def _build_vod_sources(db: AsyncSession, record: Record) -> list[dict]:
    ...
    if url_lower.startswith("s3://"):
        sources.append({"url": file_path, "protocol": "s3", "type": "s3", "priority": 1, "estimated_bitrate": 5000, "quality": "ultra"})
    ...
    # HTTP/HTTPS 源
        ...
        sources.append(
            {"url": file_path, "protocol": protocol, "type": source_type, "priority": priority, "estimated_bitrate": 3000, "quality": "high"}
        )

    # 本地文件源
    elif file_path.startswith("/") or (len(file_path) > 2 and file_path[1] == ":"):
        sources.append({"url": file_path, "protocol": "file", "type": "local", "priority": 0, "estimated_bitrate": 10000, "quality": "ultra"})
    ...
    if base_url:
        ...
        sources.insert(1, {"url": hls_path, "protocol": "hls", "type": "http", "priority": 3, "estimated_bitrate": 2500, "quality": "high"})

        # HTTP-FLV 地址
        flv_path = f"{base_url}/flv/{record.id}.live.flv"
        sources.insert(2, {"url": flv_path, "protocol": "flv", "type": "http", "priority": 4, "estimated_bitrate": 3000, "quality": "high"})
...
async def get_optimized_vod_url(
    ...
    if protocol != "auto":
        ...
        # 智能选择
        selected_url = urls.get("mp4") or urls.get("flv") or urls.get("hls") or urls.get("raw") or file_path
...
def _get_play_suggestion(urls: dict) -> dict:
    ...
    if "mp4" in urls:
        suggestions.append({"protocol": "mp4", "label": "MP4 直连", "description": "最适合点播，加载最快，画质无损", "score": 100})

    if "flv" in urls:
        suggestions.append({"protocol": "flv", "label": "HTTP-FLV", "description": "低延迟，支持H.265，FLV封装", "score": 95})

    if "hls" in urls:
        suggestions.append({"protocol": "hls", "label": "HLS", "description": "兼容性最好，支持所有浏览器", "score": 90})

    if "webrtc" in urls:
        suggestions.append({"protocol": "webrtc", "label": "WebRTC", "description": "超低延迟，需要HTTPS环境", "score": 85})

    return {"recommended": suggestions[0] if suggestions else None, "alternatives": suggestions[1:] if len(suggestions) > 1 else []}
