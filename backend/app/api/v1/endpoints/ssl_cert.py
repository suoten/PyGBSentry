...
async def cert_status(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"SSL cert status check failed: {str(e)}") from e  # 异常保护
...
async def cert_renew(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"SSL cert renewal failed: {str(e)}") from e  # 异常保护
