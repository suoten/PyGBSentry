...
class AlarmManager:
    ...
    async def broadcast_alarm(self, alarm_data: dict, tenant_id: str):
...
_ALLOWED_ALARM_WS_ROLES = {"admin", "owner", "operator"}
...
async def _ensure_schema_ready(db: AsyncSession) -> None:
    global schema_ready
    if schema_ready:
        return
    try:
        ...
        raise HTTPException(status_code=503, detail="Alarm escalation schema not ready")
...
async def get_alarms(
    ...
    min_escalation_level: int = Query(0, ge=0),  # 添加Query约束
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
):
    await _ensure_schema_ready(db)
    ...
    if start_time > end_time:
        raise HTTPException(status_code=400, detail="start_time cannot be greater than end_time")  # i18n
    if (end_time - start_time) > timedelta(days=7):
        raise HTTPException(status_code=400, detail="Time range too large, please limit to 7 days")  # i18n
...
async def get_sla_overview(
    ...
    await _ensure_schema_ready(db)
    _ALARM_LIST_HARD_LIMIT = 1000  # 魔法数字提取为常量
...
async def get_sla_compare(
    days: int = Query(7, ge=1, le=365),  # 添加Query约束
...
async def acknowledge_alarm(
    ...
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.require_permission("alarms.handle")),
):
    await _ensure_schema_ready(db)
    ...
    if not alarm:
        ...
        raise HTTPException(status_code=404, detail="Alarm not found")  # i18n
...
async def escalate_alarm(
    ...
    if escalation.state == "acknowledged":
        ...
        raise HTTPException(status_code=400, detail="Acknowledged alarm cannot be escalated")  # i18n
...
async def update_alarm_link_rule(
    ...
    if not item:
        ...
        raise HTTPException(status_code=404, detail="Rule not found")  # i18n
