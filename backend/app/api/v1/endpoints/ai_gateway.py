...
def _maybe_save_snapshot(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    snap_dir = _safe_str(settings.AI_GATEWAY_SNAPSHOT_DIR or "ai_gateway_snapshots")
    max_kb = settings.AI_GATEWAY_MAX_SNAPSHOT_KB
...
async def _forward_to_upstream(payload: dict[str, Any], headers: dict[str, str]) -> tuple[int, str]:
    url = str(settings.AI_GATEWAY_FORWARD_UPSTREAM_URL or "").strip()
    if not url:
        return 204, "no_upstream_config"
    timeout = settings.AI_GATEWAY_FORWARD_TIMEOUT_SECONDS
...
async def analyze_ai_callback(
    ...
    forward_url = str(settings.AI_GATEWAY_FORWARD_UPSTREAM_URL or "").strip()
