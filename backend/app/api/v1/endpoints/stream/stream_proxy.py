...
async def broadcast_start(
    ...
    media_server_ip = settings.MEDIA_SERVER_IP or settings.SIP_IP
    media_server_port = settings.MEDIA_SERVER_BROADCAST_PORT
    ...
    if media_node:
        ...
        if payload.audio_file_url:
            ...
            _fallback_host = str(settings.MEDIA_SERVER_HOST or "")  # I3 回退值不再硬编码127.0.0.1
        ...
        _fallback_host = str(settings.MEDIA_SERVER_HOST or "")  # I3 回退值不再硬编码127.0.0.1
        _rtmp_port = media_node.rtmp_port or settings.STREAM_PUBLIC_RTMP_PORT
        ...
        # FIX [2026-07-17 P0]: media_node 为 ORM MediaNode，.secret 列存 AES-256-GCM 密文，
        # 必须用 decrypted_secret 取明文，否则 ZLM API 鉴权失败返回 401。
        ffmpeg_key = await add_ffmpeg_source(
            host=media_node.host,
            http_port=media_node.http_port or 80,
            secret=(getattr(media_node, "decrypted_secret", None) or settings.MEDIA_SERVER_SECRET),
            src_url=src_url,
            dst_url=dst_url,
            timeout_ms=10000,
            enable_hls=0,
...
async def talk_start(
    ...
    try:
        ...
        if payload.sdp_body:
            ...
            _fallback_rtp_port = zlm_rtp_port or settings.MEDIA_SERVER_RTP_PROXY_PORT
            # P1-fix: GB28181 §A.1 规定所有 RTP 流必须携带 y=SSRC 标识，否则设备无法匹配 RTP 流与 SIP 会话
