...
def _ptz_type_text(ptz: int | None) -> str:
    m = {
        ...
        3: "Fixed box",  # i18n
        4: "PTZ box",  # i18n
        5: "PTZ half-dome",  # i18n
        6: "Multi-sensor panoramic/stitch channel",  # i18n
        7: "Multi-sensor split channel",  # i18n
    }
    if ptz is None:
        return "Unknown"  # i18n
    return m.get(int(ptz), "Unknown")  # i18n
...
def _parse_time_to_unix(value: str) -> int:
    ...
    if not v:
        raise HTTPException(status_code=400, detail="Time parameter is required")  # i18n
    if v.isdigit():
        return int(v)
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S"):
        try:
            ...
            continue
    raise HTTPException(status_code=400, detail="Invalid time format, expected yyyy-MM-dd HH:mm:ss or unix seconds")  # i18n
...
async def _resolve_resources_by_numeric_ids(
    ...
    if len(rows) != len(want):
        ...
        if missing_ids:
            ...
            if found_after != want:
                raise HTTPException(status_code=400, detail="Some channel IDs do not exist or are not authorized")  # i18n
    return rows
...
async def region_add_by_device(
    ...
    if not device_ids:
        raise HTTPException(status_code=400, detail="deviceIds is required")  # i18n
...
async def channel_update(
    ...
    if target_id is None:
        raise HTTPException(status_code=400, detail="Missing channel identifier (id or gbId)")  # i18n
    ...
    if body.gb_device_id is not None:
        ...
        if not new_gb_device_id:
            raise HTTPException(status_code=400, detail="gbDeviceId is required")  # i18n
        tenant_id = current_user.tenant_id or "default"
    ...
    if new_ptz_type is not None:
        ...
        changed += 1
    if changed <= 0:
        raise HTTPException(status_code=400, detail="No changes made")  # i18n
    await db.commit()
    return {"status": "ok"}
...
async def channel_reset(
    ...
    for field in body.channel_fields:  # chanel_fields拼写错误→channel_fields
...
async def channel_add(
    ...
    if not name:
        raise HTTPException(status_code=400, detail="gbName is required")  # i18n
...
async def channel_map_save_level(
    ...
    _user: User = Depends(deps.require_permission("devices.manage")),  # 角色检查→权限码检查
):
...
async def channel_map_thin_save(
    ...
    if not await _map_thin_jobs.get(id):
        raise HTTPException(status_code=404, detail="Thinning task not found")  # i18n
...
async def channel_map_thin_tile(
    ...
    if thin_id and not await _map_thin_jobs.get(thin_id):
        raise HTTPException(status_code=404, detail="thinId not found")  # i18n
...
async def channel_type_list(_user: User = Depends(deps.get_current_active_user)):
    return [
        {"code": "1", "name": "Camera"},  # i18n
        {"code": "2", "name": "Audio Device"},  # i18n
    ]
...
async def channel_play(
    ...
    if not device_id:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid channelId format")  # i18n
...
@router.post("/play/stop")  # I9 play/stop使用GET方法改为POST
async def channel_play_stop(
    payload: StopStreamRequest,  # I9 GET→POST，参数从Query改为Body
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(deps.get_current_active_user),
):
    stream = payload.stream
    if not stream and payload.app:
        # S-10 当 stream 未提供时，尝试通过 app+device_id+channel_id 查找活跃流（兼容旧调用方式）
        try:
            from app.models.stream_session import StreamSession
            from app.models.asset import Asset
            from app.models.resource import Resource
            ...
            # 导致 channel_play_stop 在 stream 未提供时永远无法回退查找活跃流，
            # 前端调用 stop 接口不传 stream 时会直接返回 400 错误。
            # 修复：通过 JOIN Asset（gb_id=device_id）和 Resource（gb_id=channel_id）关联查询。
            _q = select(StreamSession.stream).where(StreamSession.app == payload.app)
            if payload.device_id:
                _q = _q.join(Asset, StreamSession.asset_id == Asset.id).where(Asset.gb_id == payload.device_id)
            if payload.channel_id:
                _q = _q.join(Resource, StreamSession.resource_id == Resource.id).where(Resource.gb_id == payload.channel_id)
            _q = _q.limit(1)
    ...
    if not stream:
        raise HTTPException(status_code=400, detail="stream parameter is required")  # i18n
...
async def channel_stream_status(
    ...
    if not raw_ids:
        return {"channels": []}
    if len(raw_ids) > 50:
        raise HTTPException(status_code=400, detail="Maximum 50 channels can be probed simultaneously")  # i18n
