...
def build_required_port_rules() -> tuple[list[PortRule], tuple[int, int] | None]:
    ...
    rules.append(PortRule(int(getattr(settings, "BACKEND_PUBLIC_PORT", 8000) or 8000), "tcp", "Backend API"))

    # SIP（GB28181 注册/信令）
    rules.append(PortRule(int(getattr(settings, "SIP_PORT", 5060) or 5060), "udp", "SIP (GB28181)"))
    rules.append(PortRule(int(getattr(settings, "SIP_PORT", 5060) or 5060), "tcp", "SIP (GB28181)"))

    # ZLM/MediaServer（若你在该机上跑内置或外置同机）
    rules.append(PortRule(int(getattr(settings, "MEDIA_SERVER_HTTP_PORT", 8880) or 8880), "tcp", "ZLM HTTP API/Play"))
    rules.append(PortRule(int(getattr(settings, "MEDIA_SERVER_RTSP_PORT", 554) or 554), "tcp", "ZLM RTSP"))
    rules.append(PortRule(int(getattr(settings, "MEDIA_SERVER_RTMP_PORT", 1935) or 1935), "tcp", "ZLM RTMP"))
    rules.append(PortRule(int(getattr(settings, "MEDIA_SERVER_RTC_PORT", 8000) or 8000), "udp", "ZLM WebRTC (RTC UDP)"))
    rules.append(PortRule(int(getattr(settings, "MEDIA_SERVER_RTC_TCP_PORT", 8001) or 8001), "tcp", "ZLM WebRTC (RTC TCP)"))

    rtp_range = _parse_range(str(getattr(settings, "MEDIA_SERVER_RTP_PROXY_PORT_RANGE", "") or ""))
    if not rtp_range:
        # 兜底：单端口（历史行为）
        p = int(getattr(settings, "MEDIA_SERVER_RTP_PROXY_PORT", 30000) or 30000)
        rtp_range = (p, p)
...
def ensure_firewall_ports() -> None:
    ...
    # 2) 可选：自动放行（只在 Linux）
    if not bool(getattr(settings, "AUTO_OPEN_PORTS", False)):
        return
    if platform.system() != "Linux":
        logger.warning("AUTO_OPEN_PORTS is enabled but current OS is not Linux. Skip auto firewall changes.")
        return

    dry_run = bool(getattr(settings, "AUTO_OPEN_PORTS_DRY_RUN", True))
    provider = str(getattr(settings, "AUTO_OPEN_PORTS_PROVIDER", "auto") or "auto").strip().lower()
